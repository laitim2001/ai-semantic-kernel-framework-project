...
- Open (unchanged): `AD-ChatV2-Resume-Replay-Drive-Through` (🟡, 57.132 Leg-2 fixture). Pre-existing: `AD-Billing-Outbox-Drain-Test-Flake` (Risk Class C, did NOT surface).
...
- Open (unchanged): `AD-ChatV2-Resume-Replay-Drive-Through` (🟡, 57.132 Leg-2 deterministic escalate fixture). Pre-existing: `AD-Billing-Outbox-Drain-Test-Flake` (Risk Class C, did NOT surface — FE-only).
...
- **NEW** `AD-ChatV2-Resume-Replay-Drive-Through` (🟡) — a deterministic output/verification-escalate drive-through for Leg-2 (needs a controllable `escalate_output_phrases` admin surface OR a verification-fail fixture; the real-LLM `confidential` trigger is non-deterministic + content-filter-prone). No admin endpoint exists today for `escalate_output_phrases` (HarnessPolicy).
