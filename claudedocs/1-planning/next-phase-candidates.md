...
48.5. 🆕 **`AD-Pre-Push-Lint-Silent-Suppression-Anti-Pattern`** (FIX-015 CI fail carryover; sibling AD #48) — Local `npm run lint --silent` swallowed 28 ESLint `no-restricted-syntax` errors in FIX-015 PR #183; only caught when CI failed in 30s. Root cause: agent + main thread both used `--silent` for "clean" output, which also suppresses error lines. Fix shape: (a) document in `.claude/rules/sprint-workflow.md` Pre-Push checklist that lint MUST run **without `--silent`** to catch warnings/errors, OR (b) modify `frontend/package.json` `lint` script to always be verbose (drop `--silent`-equivalent flags), OR (c) add `npm run lint:check` alias that's guaranteed non-silent. Estimated ~15-30 min. Class: `sprint-meta + micro-fix` 0.65 candidate. Recommended bundled with Item #4 (`AD-Sprint-Plan-Agent-Delegation-Factor-Modifier`) since both are sprint-workflow process improvements.
...
49.5. 🆕 **`AD-RouteSweep-Auto-Derive`** (FIX-016 carryover) — Long-term automate `APPSHELL_ROUTES` derivation from `routes.config.ts` to prevent future PROP→real promotion drift (every promotion currently requires manual `route-sweep.mjs` update). Decision needed between (a) `tsx`/`ts-node` dev-dependency for TS-from-MJS import, (b) regex text-parse of routes.config.ts (fragile), (c) Vite pre-build extraction step (more complex). Estimated ~2-3 hr. Class: `frontend-refactor-mechanical` 0.80 candidate.
...
- **`AD-RouteSweep-Auto-Derive`** (FIX-016 carryover; auto-sync APPSHELL_ROUTES from routes.config.ts; ~2-3 hr `frontend-refactor-mechanical` 0.80)
- **`AD-Pre-Push-Lint-Silent-Suppression-Anti-Pattern`** (FIX-015 CI fail lesson; bundle with #4 below; ~15-30 min `sprint-meta + micro-fix` 0.65)
- **`AD-ApprovalList-Risk-Color-Tailwind-Hex-Sentinels`** (FIX-015 carryover; bundle with chat_v2 risk map; ~30-45 min)
- **`AD-Sprint-Plan-Agent-Delegation-Factor-Modifier`** (#4 of Sprint 57.39 4-AD sequence — quantify ~3-5× agent speedup as calibration matrix multiplier-on-multiplier; ~2 hr)
