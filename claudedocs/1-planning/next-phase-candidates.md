...
- Remaining Skills epic ADs (unchanged): `AD-Skills-Authoring-UI` · `AD-Skills-Per-Tenant-Quota` · `AD-Skills-Bundled-Scripts` · `AD-Skills-SlashMenu-Mockup` · multi-skill-per-command (YAGNI).
...
- **`AD-Skills-Per-Tenant-Quota`** (🟢, NEW Sprint 57.114) — per-tenant skill-count quota + `instructions` body-size limit (today `TEXT` unbounded, `min_length=1` only); + a multi-worker shared cache-invalidation signal (`_SkillRegistryCache` is in-process, like `_ModelPolicyCache`).
...
- **`AD-Skills-Authoring-UI`** (🟢) — skill versioning / hot-reload / an authoring admin UI (the registry loads once at first access; no reload).
- **`AD-Skills-Bundled-Scripts`** (🟢) — CC skills can bundle executable scripts/resources; this spike is instructions-only.
