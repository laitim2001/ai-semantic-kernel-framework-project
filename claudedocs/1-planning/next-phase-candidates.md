...
## 📦 Shipped Sprints — Pointer Index (57.29 → 57.166)
...
|--------|-----------------------|----------------|
| 57.166 | cross-burst turn/token aggregate in the final `loop_end` — 4 accumulators in `_stream_loop_events` sum EVERY burst's `LoopCompleted` (swallowed + final) → the terminal `loop_end` payload `total_turns` + the `conversation_completed` audit 4 counters carry the whole-run SUM; **billing deliberately stays per-burst** (Day-0 Prong-2 proved the enqueue is un-gated → aggregate would double-bill). Drive-through PASS: 3 bursts → badge "· 7 turns" == audit 7 (per-burst ceiling 2 → 7 impossible for one burst). Closes `AD-Scheduler-Burst-Stats-Aggregate-Phase58` (57.157 carryover); **MERGED PR #395, main `a289fecf`** | memory/project_phase57_166_burst_stats_aggregate.md |
...
#### 57.157
**NEW carryover**: ~~`AD-Scheduler-Burst-Stats-Aggregate-Phase58`~~ ✅ **CLOSED Sprint 57.166** (4 accumulators in `_stream_loop_events` → final `loop_end` + audit carry the cross-burst SUM; billing stays per-burst by design) · `AD-Scheduler-Burst-Boundary-Wire-Phase58` (no wire event marks a burst boundary; a FE burst counter would need one) · `AD-Scheduler-PerTenant-Phase58` (global env, not per-tenant policy — deliberate anti-AP-6) · `AD-Scheduler-TokenBudget-Continue-Phase58` (only `max_turns` continues; `token_budget` conservatively stops) · `AD-TaskPrimitive-DAG-Enforce-Phase58` (still open — DAG + scheduler both remain guidance).

#### 57.166
**NEW carryover**: `AD-Scheduler-Burst-CacheRate-Aggregate` (`cached_input_tokens` is summable but `cache_hit_rate` is a *rate* — needs weighted re-derivation, not a sum; both still last-burst-only on the `loop_end` payload) · `AD-Scheduler-AwaitingApproval-Burst-Turn-Count` (a burst stopping at a HITL gate reports `total_turns=0`, so the aggregate silently omits the partial turn — confirm intended semantics) · `AD-DriveThrough-Determinism-Lever-Plan-At-Day0` (when acceptance needs a RARE runtime path, name the lever + its revert proof in plan §Risks at Day 0 — 57.166 improvised it on Day 3 and burned 6 legs).
