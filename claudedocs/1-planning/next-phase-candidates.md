...
- **`AD-ChatV2-Live-MultiTurn-Context`** (🟡, NEW — pre-existing, surfaced by the drive-through) — the backend does NOT rehydrate prior conversation messages for the live loop (`state_data` excludes messages), so a follow-up in the same session loses prior context (the drive-through's turn 2 lost "it"→Paris). A real product gap (multi-turn chat is effectively single-turn for context); NOT replay-related; a dedicated slice (likely needs the `messages` table to get a writer, or rehydrate from the new `message_events` user_message/llm_response rows).
