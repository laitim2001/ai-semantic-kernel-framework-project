...
- **`AD-Skills-Per-Tenant-Catalog`** (🟡) — the registry is system-bundled (no tenant_id, no DB); a per-tenant overlay would resolve like `resolve_session_persona` and merge tenant skills over the bundled set (DB-backed catalog + admin authoring).
