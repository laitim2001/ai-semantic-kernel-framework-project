/**
 * AgentActionList Test Suite
 *
 * Tests for the Agent action list component.
...
const singleAction: AgentAction[] = [
  createAction('a1', 'search', 'Searching for relevant files'),
];

const multipleActions: AgentAction[] = [
...
describe('AgentActionList', () => {
  ...
  describe('Action Icons', () => {
    it('should render correct icons for different action types', () => {
      const { container } = render(
        <AgentActionList actions={multipleActions} />
      );
  ...
  describe('Expandable Actions', () => {
    it('should show chevron for expandable actions', () => {
      const expandableAction: AgentAction[] = [
        createAction('a1', 'search', 'Search results', { expandable: true }),
      ];

      const { container } = render(
        <AgentActionList actions={expandableAction} />
      );
    ...
    it('should not show chevron for non-expandable actions', () => {
      const nonExpandableAction: AgentAction[] = [
        createAction('a1', 'think', 'Thinking', { expandable: false }),
      ];

      render(<AgentActionList actions={nonExpandableAction} />);
  ...
  describe('Description Display', () => {
    it('should display description when provided', () => {
      const actionWithDescription: AgentAction[] = [
        createAction('a1', 'code', 'Running tests', {
          description: 'unit tests for auth module',
        }),
      ];

      render(<AgentActionList actions={actionWithDescription} />);
  ...
  describe('Accessibility', () => {
    ...
    it('should have tabIndex for keyboard navigation', () => {
      const { container } = render(
        <AgentActionList actions={singleAction} />
      );
