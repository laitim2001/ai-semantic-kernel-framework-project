...
Sprint 166 — Phase 46: Complexity-aware decomposition (CC-inspired dynamic agent count).
"""
...
# Hard cap to prevent runaway decomposition regardless of LLM output
MAX_SUBTASKS = 10
...
2. 每個子任務分配一個最適合的專家角色
3. 每個子任務描述要具體、可執行
4. 如果任務很簡單，只需要一個通用助手，直接返回 1 個子任務

## 輸出格式（JSON）
{{
    "mode": "parallel",
    "reasoning": "拆解理由（包含複雜度判斷）...",
    "sub_tasks": [
        {{
            "title": "子任務標題",
            "description": "具體描述和要求",
...
class TaskDecomposer:
    ...
    async def decompose(self, task: str, complexity_hint: str = "auto") -> TaskDecomposition:
        """Decompose a complex task into sub-tasks.

        Args:
            task: The user's complex request.
            complexity_hint: Guides LLM on expected subtask count.
                "auto" — LLM decides freely based on task structure (default).
                "simple" — Steers toward 1-2 subtasks.
                "moderate" — Steers toward 2-4 subtasks.
                "complex" — Steers toward 4-8 subtasks.

        Returns:
            TaskDecomposition with sub-tasks and mode.
        """
        hint_labels = {
            "simple": "simple — 建議 1-2 個子任務",
            "moderate": "moderate — 建議 2-4 個子任務",
            "complex": "complex — 建議 4-8 個子任務",
            "auto": "auto — 請根據任務實際複雜度自行判斷",
        }
        hint_text = hint_labels.get(complexity_hint, hint_labels["auto"])
    ...
    def _build_decomposition(self, original_task: str, result: Dict[str, Any]) -> TaskDecomposition:
        ...
        # Hard cap protection — prevent runaway decomposition
        if len(sub_tasks) > MAX_SUBTASKS:
            logger.warning(
                "TaskDecomposer: LLM returned %d sub-tasks, capping at %d",
                len(sub_tasks), MAX_SUBTASKS,
            )
