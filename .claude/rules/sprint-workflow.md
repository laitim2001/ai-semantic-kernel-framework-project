...
| **`frontend-refactor-mechanical` (NEW Sprint 57.15)** | **0.50** (HYBRID weighted blend: mechanical-refactor × 0.40-0.45 ~0.70 weight + ci-config+a11y × 0.55 ~0.15 + closeout × 0.80 ~0.15) | **57.15=~1.7 (1)** | **n/a 1-data-point** | **NEW class baseline opens;1st app `actual/committed` ≈ 1.7 OVER [0.85, 1.20] band by ~0.5 BUT `actual/bottom-up` ≈ 0.89 — i.e. the *bottom-up* was accurate (mechanical refactors are low-variance/predictable), the 0.50 haircut was too aggressive. KEEP 0.50 this iteration per `When to adjust` 3-sprint window rule (single-sprint outliers ignored); if next 1-2 `frontend-refactor-mechanical` sprints also `actual/committed > 1.2` → AD-Sprint-Plan-N propose 0.50→0.70-0.80 (a mechanical class should sit near the top of the band like `medium-backend` 0.80, not the cautious mid-band — cf. `audit-cycle/docs/template` 0.40 also ran over band in 57.10).pending 2-3 sprint validation** |
