...
| **`frontend-verbatim-css-repoint` (NEW Sprint 57.29)** | **0.60** (HYBRID weighted blend: Day-0 三-prong+baseline × 0.85 + shell hotfix mechanical × 0.45 + mockup-ui primitive port × 0.55 + shell re-point × 0.55 + 3 topbar-overlay re-point × 0.55 + page re-point × 0.50 + ~19 component re-point × 0.50 + 22-route sweep+fidelity × 0.50 + closeout × 0.80 ≈ 0.60) | 57.29≈1.0 / **57.30≈0.40 (2)** | 2-pt mean **0.70** lower edge of band (1 in band + 1 below by 0.45) | **2nd app (Sprint 57.30 /chat-v2 + shell hotfix) ratio actual/committed ≈0.40 BELOW [0.85, 1.20] band by ~0.45 (estimated — agent-assisted compressed session, not rigorously per-day-tracked; same caveat as 57.13/57.27/57.28/57.29). Causes: (a) Sprint 57.21 Phase-1 structural rewrite already aligned chat-v2 structure to mockup pattern → Phase-2 became "CSS-class swap per file" not "rewrite from scratch"; (b) Sprint 57.28 verbatim CSS foundation made per-component edit one-pass-per-file; (c) bundled hotfix ~45 min actual vs ~3-4 hr estimated (Day 1 first-pass success); (d) agent-assisted pattern reuse compounded (Day 1 45 min → Day 4 8 min/file). KEEP 0.60 baseline per `When to adjust` 3-sprint window rule (2 data points insufficient for adjustment; lower-trigger needs 3+ consecutive < 0.7); NEW `AD-Sprint-Plan-frontend-verbatim-bimodal-watch` — if 3rd data point continues low, propose split between "structural-heavy re-point" (0.65) and "css-swap-only re-point" (0.40); next data point likely Sprint 57.31+ on /orchestrator or /loop-debug or another 🟡 AppShellV2 route.** |
