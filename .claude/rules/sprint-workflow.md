...
| `medium-backend` | 0.80 (0.65 base + 0.05 audit-cycle surcharge + Day 0 fixed offset) | 55.5=1.14 / 55.6=0.92 / 57.47=0.16 / 57.48=0.11 / 57.50=0.27 / 57.53=0.83 / 57.54≈1.0 / 57.55=0.79 / 57.56≈0.66 / **57.57≈0.72** (10) | 10-pt mean **0.66** (last-3 mean **~0.72**) | **KEEP 0.80 per `When to adjust` 3-sprint window rule** — last 3 (57.55=0.79 + 57.56≈0.66 + 57.57≈0.72) **1/3 < 0.7 lower-trigger NOT yet MET** (need 3+ consecutive); 10-pt mean reaches meaningful threshold at 0.66 (post-tier-4-confound-resolution tracking continues). Class baseline 0.80 well-calibrated for human-pace + Day 0 + Day 2 overhead included; agent residual captured at tier-4 sub-class layer correctly per Sprint 57.56+57.57 retro Q4 (`mechanical-greenfield-design-decisions` 0.65 1st+2nd validation CONFIRMED CLEANLY at ratio ~1.02 + ~1.15 IN BAND ✅ — 2 consec IN band tier-4 SPLIT FULLY VALIDATED stripping confound from Sprint 57.56+57.57 cases). AD-MediumBackend-AICadence-Recalibration continues Phase 58+ |
...
where agent_factor = {
  ...
  agent-delegated (tier-4 sub-class table — Sprint 57.55 retro Q4 tier-4 SPLIT ACTIVATED effective 2026-05-28 onwards):
    mechanical-pattern-reuse-heavy:               0.30   (≥ 4 mechanical repetitions of same template in 1 sprint; evidence: Sprint 57.49 retroactive — 5-tab+1-drawer migration ratio 0.21 under 0.30 vs 0.14 under 0.45)
    ...
    mechanical-greenfield-design-decisions:       0.65   (single NEW component-pair WITH NEW Pydantic schema design + NEW UX state design — e.g. edit-mode draft state — design decisions add ~30-50% wall-clock vs pure port; evidence: Sprint 57.54 HITLPolicies WRITE ratio ~1.37-2.0 at 0.50 / Sprint 57.55 FeatureFlags WRITE ratio ~1.57 at 0.50 — 2 consec > 1.20 → tier-4 SPLIT ACTIVATED; equivalent ratios under 0.65 = 1.05-1.55 / 1.21 → band top edge / IN band)
