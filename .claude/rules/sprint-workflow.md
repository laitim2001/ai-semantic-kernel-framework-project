...
| `medium-backend` | 0.80 (0.65 base + 0.05 audit-cycle surcharge + Day 0 fixed offset) | 55.5=1.14 / 55.6=0.92 / 57.47=0.16 / 57.48=0.11 / 57.50=0.27 / 57.53=0.83 / **57.54≈1.0** (7) | 7-pt mean **0.63** (last-3 mean **0.70**) | **KEEP 0.80 per `When to adjust` 3-sprint window rule** — last 3 (57.50=0.27 + 57.53=0.83 + 57.54≈1.0) only 1/3 < 0.7 lower-trigger NOT MET (further improvement from 6-pt prior; 7-pt mean 0.57→0.63). **Sprint 57.53 retro Q4 prediction validated 2x**: Sprint 57.53 0.83 cleaner under human factor + Sprint 57.54 ~1.0 IN BAND middle under tier-3 sub-class layer (`mechanical-greenfield` 0.50 stripped confound). Class baseline 0.80 confirmed well-calibrated for human-pace; agent residual captured at tier-3 sub-class layer correctly. AD-MediumBackend-AICadence-Recalibration deferred Phase 58+ — need 3-sprint window of consistent human-factor data |
| `medium-frontend` | 0.65 | 57.1=0.85 / 57.13≈0.95-1.0 / 57.49=0.064 / **57.54≈0.32** (4 — Sprint 57.13 added retroactively per memory subfile; 57.54 frontend track sub-portion) | 4-pt mean **~0.56** (last-3 mean ~0.46) | **KEEP 0.65 per `When to adjust` 3-sprint window rule** — last 3 (57.13≈0.95 + 57.49=0.064 + 57.54≈0.32) 2/3 < 0.7 lower-trigger NOT MET (need 3+ consecutive); however 4-pt mean 0.56 below band edge by 0.09 — `AD-medium-frontend-Baseline-Recalibration` continues for Sprint 57.55+ 4th-data-point-of-window (5th total) decision; if 5th continues < 0.7 → 3+ consecutive lower-trigger MET → propose 0.65 → 0.50 lift. Confound resolved at tier-2/3 sub-class layer (`mechanical-greenfield` 0.50 sub-class applies in 57.54 case) |
...
where agent_factor = {
  human (default):      1.0
  agent-delegated (tier-2 sub-class table — Sprint 57.50 retro Q4 tier-2 ESCALATION effective 2026-05-27 onwards):
    ...
    mechanical-greenfield:             0.50   (single NEW component-pair; < 4 mechanical repetitions of same template; evidence: Sprint 57.50 retroactive — 1-endpoint+1-hook+1-refactor ratio 0.54 under 0.50 vs 0.58 under 0.45)
