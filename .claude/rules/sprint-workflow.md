...
where agent_factor = {
  human (default):      1.0
  agent-delegated (sub-class table — Sprint 57.48 retro Option B structural split effective 2026-05-26 onwards):
    mechanical-single-domain:    0.45   (high pattern reuse OR mechanical port; single-domain backend OR mockup-strict-rebuild; evidence: Sprint 57.40-44 + 57.47 + 57.48)
    mixed-multidomain-bundle:    0.65   (3+ independent tracks with context-switching; evidence: Sprint 57.46)
  partial (20-79% via agent):    0.75   (linear interpolation)
  human (<20% via agent):        1.0
  History: 0.55 (Sprint 57.42 activated) → 0.45 (Sprint 57.44 tighten) → 0.65 (Sprint 57.46 rollback) → sub-class split (Sprint 57.48 Option B; closes AD-AgentFactor-Sub-Class-Calibration ACTIVATED; parallel to Sprint 57.38 `-simple/-with-extras` precedent)
}
```
