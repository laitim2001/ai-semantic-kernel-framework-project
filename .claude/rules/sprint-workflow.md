...
| `frontend-feature-with-event-wire-addition` | 0.55 | n/a (1 pt) | KEEP pending 2-3 sprint validation (Sprint 57.100 ratio ~1.0 IN band; the A2 reviewer-UI follow-up — thin additive `kind` field on the `approval_requested` event + codegen regen + a verification-gated chat-v2 reject-with-note + cross-stack parity, Cat 12×9×frontend, parent-direct agent_factor 1.0; no new event type so no 22→23 bump) |
...
| `config-tiering-model-policy-spike` | 0.60 | n/a (1 pt) | KEEP pending 2-3 sprint validation (Sprint 57.104 C1 ratio ~0.9-0.95 IN band; full-stack new-domain config-tiering — neutral `ModelPolicy` value object + TTL-cached resolver (Risk Class C) + `build_azure_model_profile` signature reshape + per-request wiring + pricing-validated admin PUT/GET + a tenant-settings tab mirroring QuotasTab; backend parent-direct + FE agent-delegated-then-parent-re-verified = blended full-stack so NO single agent_factor; loop.py diff 0, no migration, no new event; drive-through PASS proved per-tenant model + cache invalidation in cost_ledger nano→mini) |
