...
| **`frontend-verbatim-css-repoint -with-extras` (Sprint 57.38 Option 2 split — applies when ANY hold: multi-file batched >3 / AP-2 BackendGapBanner addition / dual-mount preservation / playback/filter/inspector widgets / HEX_OKLCH_BASELINE bump ≥4)** | **0.65** | 57.35≈1.7 (8-file batched) / 57.36≈1.42 (1-file AP-2+dual-mount) / 57.37B≈1.33 (1-file oklch-heavy +6 bump) (3 historical pre-split data points at 0.50; equivalent ratios at 0.65 are ~1.31 / ~1.09 / ~1.02) | 3-pt mean **1.48 at 0.50** → equivalent **~1.14 at 0.65** in [0.85, 1.20] band | **No new data point Sprint 57.38** (Domain B classified `-simple` per Day 3 strict criteria; 0/5 extras met). Baseline 0.65 derived from 57.35/36/37B mean 1.48 at 0.50 × (0.50/0.65) = ~1.14 expected — within band. Validates next sprint when true `-with-extras` candidate ships. |
