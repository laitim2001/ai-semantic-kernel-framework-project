...
| `medium-backend` | 0.80 (0.65 base + 0.05 audit-cycle surcharge + Day 0 fixed offset) | 55.5=1.14 / 55.6=0.92 / 57.47=0.16 / 57.48=0.11 / 57.50=0.27 / 57.53=0.83 (6) | 6-pt mean **0.57** (last-3 mean **0.40**) | **KEEP 0.80 per `When to adjust` 3-sprint window rule** — last 3 (57.48=0.11 + 57.50=0.27 + 57.53=0.83) only 2/3 < 0.7 lower-trigger NOT MET (improvement from 5-pt-window 3/3 < 0.7 prior); **Sprint 57.50 retro Q4 prediction "6th data point under tier-2 will be cleaner signal" validated** — Sprint 57.53 ratio 0.83 under `human` 1.0 factor (parent-assistant-direct per Sprint 57.45 Path B precedent) IS the clean class-baseline data point (no agent_factor confound). 6-pt mean improvement 0.52 → 0.57 confirms confound-resolution direction. Sub-lesson candidate: if next 2-3 human-factor sprints continue at 0.70-0.85, baseline 0.80 may be slightly too high for AI-cadence parent-assistant-direct work → Phase 58+ AD-MediumBackend-AICadence-Recalibration (deferred — need 3-sprint window of consistent human-factor data) |
