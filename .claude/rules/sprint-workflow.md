...
| **`frontend-mockup-strict-rebuild` (NEW Sprint 57.23)** | **0.60** (HYBRID weighted blend per Sprint 57.22 AUDIT-REPORT §Sprint 57.23+ Recommendation: auth-flow IAM-ish × 0.55 ~25% + mockup-direct-port-structural × 0.85 ~30% + 4-step wizard × 0.50 ~15% + 6-digit input grid × 0.55 ~15% + AP-2 banner reuse × 0.40 ~15% = ~0.60 mid-band) | 57.23=0.59 / 57.24=1.19 / **57.25=0.88** (3) | 3-pt mean **0.89** in band lower; rich-only 2-pt mean **1.04** in band middle | **3rd app (Sprint 57.25 sla-dashboard rebuild) ratio 0.88 ✅ in-band lower edge of [0.85, 1.20] band; rich-dashboard 2-data-point mean 1.04 (57.24 v2 1.19 + 57.25 0.88) sits in-band middle — rich-dashboard sub-class hypothesis from Sprint 57.24 v2 retro Q4 NOT confirmed; **KEEP 0.60 baseline + DEFER sub-classification (AD-Sprint-Plan-rich-dashboard-sub-class-DEFER #41)** per `When to adjust` 3-sprint window rule (3 data points insufficient to justify split when rich mean is in-band). Reasons for 57.25 0.88: reuses 7 Sprint 57.24 v2 primitives without API change (no primitive extraction cost; ~1.5 hr saved validating Karpathy §2 + Sprint 57.24 v2 Q3 winning bet); 1 NEW feature-scoped LatencyChart inline (~110 lines; not extracted per Karpathy §2 single-consumer); 14 NEW Vitest cases (vs 57.24 v2's 43; minus 5 deleted SLAMetricsCard via Karpathy §3 orphan delete); 3 of 3 expected BackendGapBanner instances per AP-2; bottom-up was 1.9× generous; 0.60 haircut still slightly aggressive (`actual/bottom-up` = 0.53). Next data point: Sprint 57.26 admin-tenants list rebuild — if 4th rich-dashboard ratio lands in band → DROP sub-class proposal entirely; if > 1.20 → reconsider rich sub-class higher (~0.70-0.75).** |
