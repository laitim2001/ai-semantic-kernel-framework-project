...
| `medium-backend` | 0.80 (0.65 base + 0.05 audit-cycle surcharge + Day 0 fixed offset) | 55.5=1.14 / 55.6=0.92 / 57.47=0.16 / 57.48=0.11 / 57.50=0.27 / 57.53=0.83 / 57.54≈1.0 / **57.55=0.79** (8) | 8-pt mean **0.65** (last-3 mean **0.87**) | **KEEP 0.80 per `When to adjust` 3-sprint window rule** — last 3 (57.53=0.83 + 57.54≈1.0 + 57.55=0.79) **3/3 IN band lower-edge or middle** → lower-trigger NOT MET; upper-trigger NOT MET. 8-pt mean improved 0.63→0.65 (post-tier-4-confound-resolution tracking continues). Class baseline 0.80 well-calibrated for human-pace + Day 0 + Day 2 overhead included; agent residual captured at tier-4 sub-class layer correctly per Sprint 57.55 retro Q4 SPLIT ACTIVATION (`mechanical-greenfield-design-decisions` 0.65 stripped confound from 57.54+57.55 cases). AD-MediumBackend-AICadence-Recalibration continues Phase 58+ |
| `medium-frontend` | 0.65 | 57.1=0.85 / 57.13≈0.95-1.0 / 57.49=0.064 / 57.54≈0.32 / **57.55=0.53** (5 — Sprint 57.13 added retroactively per memory subfile; 57.54+57.55 frontend track sub-portion) | 5-pt mean **~0.55** (last-3 mean ~0.30) | **KEEP 0.65 per `When to adjust` 3-sprint window rule + confound-resolved-at-sub-class-layer discipline** — last 3 (57.49=0.064 + 57.54≈0.32 + 57.55=0.53) **3/3 < 0.7 lower-trigger CRITERIA MET (3+ consecutive < 0.7)** BUT per Sprint 57.50 retro discipline: class baseline holds when confound stripped at sub-class layer; Sprint 57.55 frontend at tier-4 `mechanical-greenfield-design-decisions` 0.65 layer = human-equivalent 50 min / class-committed 46.8 min = **1.07 IN BAND middle** when agent_factor stripped. Class baseline 0.65 CONFIRMED well-calibrated for human-pace; AD-medium-frontend-Baseline-Recalibration continues Sprint 57.56+ 6th data point (need consistent human-factor data to recalibrate; agent-delegated confound persists across 57.49+57.54+57.55) |
