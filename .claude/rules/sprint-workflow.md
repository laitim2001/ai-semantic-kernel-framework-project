...
| **`frontend-mockup-strict-rebuild` (NEW Sprint 57.23)** | **0.60** (HYBRID weighted blend per Sprint 57.22 AUDIT-REPORT §Sprint 57.23+ Recommendation: auth-flow IAM-ish × 0.55 ~25% + mockup-direct-port-structural × 0.85 ~30% + 4-step wizard × 0.50 ~15% + 6-digit input grid × 0.55 ~15% + AP-2 banner reuse × 0.40 ~15% = ~0.60 mid-band) | 57.23=0.59 / **57.24=1.19** (2) | 2-data-point span **0.59→1.19** (crosses entire band — high variance) | **2nd app (Sprint 57.24 v2) ratio 1.19 ✅ top of [0.85, 1.20] band — KEEP 0.60 baseline per `When to adjust` 3-sprint window rule (2 data points insufficient). 2-point span 0.59 (auth-flow 7 small routes) → 1.19 (1 rich dashboard page) crosses entire band → possible class needs sub-classification by rebuild shape: 7-small-routes vs 1-rich-page. If 3rd app (Sprint 57.25 sla-dashboard rebuild — same rich-dashboard shape) continues high variance → propose split into `frontend-mockup-strict-rebuild-auth-flow` (0.55) vs `frontend-mockup-strict-rebuild-dashboard-rich` (0.65). Reasons for 57.24 v2 1.19: v1 ABORTED Day 1 (§Step 2.5) absorbed ~50 min redraft overhead; v2 plan was tight; 6 widget groups + 7 reusable primitives extraction; one mid-Day-1.3 STYLE.md §1 lint fix; one Day 3 e2e selector adapt for TenantTopTable row count drift. Bottom-up was 1.4× generous; 0.60 haircut close to right size (`actual/bottom-up` = 0.71).** |
