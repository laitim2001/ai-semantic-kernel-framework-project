...
| **`frontend-mockup-strict-rebuild` (NEW Sprint 57.23)** | **0.60** (HYBRID weighted blend per Sprint 57.22 AUDIT-REPORT §Sprint 57.23+ Recommendation: auth-flow IAM-ish × 0.55 ~25% + mockup-direct-port-structural × 0.85 ~30% + 4-step wizard × 0.50 ~15% + 6-digit input grid × 0.55 ~15% + AP-2 banner reuse × 0.40 ~15% = ~0.60 mid-band) | 57.23=0.59 / 57.24=1.19 / 57.25=0.88 / 57.27≈0.95 / **57.37A≈1.18** (5) | 5-pt mean **0.96** in band middle | **5th app (Sprint 57.37 Domain A — /loop-debug full rebuild: 18-event fixture + playback strip + filter pills + LoopInspector pane + AP-2 banner correction; closes Sprint 57.36 §Frontend Mockup-Fidelity gap) ratio actual/committed ~1.18 ✅ in [0.85, 1.20] band top edge. 5-pt mean 0.96 squarely in-band middle. KEEP 0.60 baseline per `When to adjust` 3-sprint window rule (class healthy across 5 apps; no lift trigger). Sprint 57.37 was 2-domain batched (Domain A `frontend-mockup-strict-rebuild` 5th app + Domain B `frontend-verbatim-css-repoint` 8th app); sprint total HYBRID blend ratio ~1.0 in-band middle. **/loop-debug user-reported empty-state issue FULLY RESOLVED — page now visually parity with mockup `localhost:8080/#loop-debug`.** Agent-assisted Day 1-2 via code-implementer (4th consecutive). Prior 4-app data: 57.23=0.59 / 57.24=1.19 / 57.25=0.88 / 57.27≈0.95 — 4-pt mean 0.90.** |
...
| **`frontend-verbatim-css-repoint` (NEW Sprint 57.29; baseline LIFTED 0.60→0.50 Sprint 57.31 Day 4; bimodal-by-shape watch Sprint 57.34 → CLOSED REJECTED Sprint 57.36; scale-overhead watch Sprint 57.35 → WEAKENED Sprint 57.36; multi-dimensional-variance watch Sprint 57.36 → confirmed Sprint 57.37; **class-split-proposal candidate Sprint 57.37 (3-consecutive-above-band lift trigger MET — Sprint 57.38 decision pending**))** | **0.50** (was 0.60 Sprint 57.29-57.31; lifted Day 4 per `When to adjust` after 57.30+57.31 both below band by 0.45+); **Sprint 57.38 candidate Option 1 lift 0.50 → 0.60 class-wide OR Option 2 class split `-simple` (0.50) vs `-with-extras` (0.65)** | rich (1-file each): 57.29≈1.0 / 57.30≈0.40 / 57.31≈0.35 / 57.32~0.40-0.55 — 3-pt mean ≈0.40; non-rich (1-file no AP-2 no dual-mount): 57.34≈0.95-1.05; non-rich (8-file batched): 57.35~1.65-1.75; non-rich (1-file AP-2 + dual-mount): 57.36~1.42; non-rich (1-file verbatim oklch-heavy port): **57.37B~1.33 (8 total)** | 8-pt span 0.35-1.75; 4-pt non-rich mean **1.36**; **3-consecutive-above-band lift trigger MET this sprint (57.35 + 57.36 + 57.37B all > 1.20 ABOVE band); Sprint 57.38 retro decision needed** | **8th app (Sprint 57.37 Domain B — /state-inspector verbatim CSS re-point per page-platform.jsx:21-155) ratio actual/committed ~1.33 ABOVE [0.85, 1.20] band by 0.13. **4th non-rich-shape data point — decisive 3-consecutive-above-band lift trigger MET**: 57.35 (1.7) + 57.36 (1.42) + 57.37B (1.33). Per `When to adjust` rule (3+ consecutive > 1.20 → raise multiplier). **Two options for Sprint 57.38 retro**: (Option 1) class-wide baseline lift 0.50 → 0.60 — simpler, but over-corrects truly simple 57.34 baseline (only in-band non-rich data point at 1.0); (Option 2 recommended) class split `-simple` (0.50): pure 1-file CSS swap no extras — Sprint 57.34 baseline only; `-with-extras` (0.65): + any of {AP-2 banner, dual-mount, playback/filter/inspector widgets, verbatim oklch-heavy port with HEX_OKLCH_BASELINE bumps, multi-file batched > 3 files} — Sprints 57.35/57.36/57.37B mean 1.48. Sprint 57.37 was 2-domain batched; Domain B classic 1-file Tailwind utility → mockup verbatim swap; Sprint 57.19 US-B3 backend wiring preserved; HEX_OKLCH_BASELINE +6 Day 3. **D-DAY3-1 positive surprise: StateInspector spec needed NO update** (text-based assertions class-swap-resilient — Sprint 57.38+ convention candidate: prefer `getByText`/`getByRole`/`data-testid` over class-name selectors). Agent-assisted Day 3 (4th consecutive code-implementer). KEEP 0.50 baseline this sprint; Sprint 57.38 retro decides class split or lift.** |
| **`frontend-verbatim-css-repoint` historical** | (collapsed) | n/a | n/a | **7th app (Sprint 57.36 /loop-debug LoopVisualizer.tsx) ratio actual/committed ~1.42 ABOVE [0.85, 1.20] band by 0.22. Single-file scope SAME as Sprint 57.34 (1.0 in band) but ratio diverged sharply — 1-D hypotheses (shape OR scale) insufficient. Emerging compound drivers: AP-2 BackendGapBanner addition (57.36 yes; 57.34 no), dual-mount preservation (mode="inline" + "standalone" branching), spec adapt overhead, drift handling (D-DAY1-1 src/tests glob coverage + D-DAY1-2 ESLint body-blind escape hatch). Agent-assisted Day 1-2 (3rd consecutive code-implementer). 22-route sweep 19 IDENTICAL + 3 CHANGED (loop-debug +22.5KB expected; chat-v2 +18B cascade ε; overview +70B time-text noise). Vitest 456/456 + mockup-fidelity 41/41 preserved. NEW `AD-Sprint-Plan-frontend-verbatim-css-repoint-multi-dimensional-variance-watch` — if Sprint 57.37+ continues > 1.20, propose (a) baseline lift 0.50 → 0.60 OR (b) class split `-simple` (0.50 no AP-2 no dual-mount) vs `-with-ap2-or-dual-mount` (0.65). CLOSE `shape-bimodal-watch` REJECTED (3 non-rich pts 1.0/1.7/1.42 = not bimodal). WEAKENED `scale-overhead-watch` (1-file also above band). KEEP 0.50 this iteration per `When to adjust` 3-sprint window rule (need 1 more above-band sprint for formal lift trigger).** |
