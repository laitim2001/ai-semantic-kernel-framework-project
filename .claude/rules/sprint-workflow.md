...
| **`frontend-verbatim-css-repoint` (NEW Sprint 57.29; baseline LIFTED 0.60→0.50 Sprint 57.31 Day 4; bimodal-by-shape watch Sprint 57.34; scale-overhead watch Sprint 57.35)** | **0.50** (was 0.60 Sprint 57.29-57.31; lifted Day 4 per `When to adjust` after 57.30+57.31 both below band by 0.45+); HYBRID weighted blend unchanged | rich (1-file each): 57.29≈1.0 / 57.30≈0.40 / 57.31≈0.35 / 57.32~0.40-0.55 — 3-pt mean ≈0.40; non-rich (1-file): 57.34≈0.95-1.05; non-rich (8-file): **57.35~1.65-1.75 (6 total)** | 6-pt span 0.35-1.75; **bimodal-by-shape WEAKENED; file-count scale-overhead emerging as 2nd variance driver** | **6th app (Sprint 57.35 AuthShell + 7 auth routes) ratio actual/committed ~1.65-1.75 ABOVE [0.85, 1.20] band by ~0.45-0.55 — 3rd validation data point for `AD-Sprint-Plan-frontend-verbatim-css-repoint-baseline-lift` AND 2nd bimodal data point. 8-file batched sprint (1 AuthShell + 7 routes + 4 Vitest spec updates + 4 drifts D-DAY1-1..D-DAY3-1). User-reported `/auth/login` drift (2026-05-24 SSO buttons unstyled / Continue no fill / dev-login orange missing) fully RESOLVED. Closes Sprint 57.23 vintage HSL-translation epic gap. Combined with 57.34 (1-file non-rich ≈1.0 in band) + 57.35 (8-file non-rich ~1.7 above band), bimodal-by-shape hypothesis WEAKENED — file-count + Vitest-spec-update overhead is plausibly the dominant variance driver. NEW `AD-Sprint-Plan-frontend-verbatim-css-repoint-scale-overhead-watch` — if Sprint 57.36+ multi-file sprints again > 1.20 → propose file-count surcharge (e.g. 0.50 + 0.05/extra-file beyond 3). Update `AD-Sprint-Plan-frontend-verbatim-css-repoint-shape-bimodal-watch` → broaden to scale-and-shape; don't propose class split yet. Sprint 57.35 agent-assisted (code-implementer Day 1-3). KEEP 0.50 baseline per `When to adjust` 3-sprint window rule (3-pt span 0.40/1.0/1.7 is inconclusive — need 4th data point to discriminate shape vs scale).** |
