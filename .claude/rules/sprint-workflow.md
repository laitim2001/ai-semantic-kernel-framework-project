...
| `medium-backend` | 0.80 (0.65 base + 0.05 audit-cycle surcharge + Day 0 fixed offset) | 55.5=1.14 / 55.6=0.92 / 57.47=0.16 / 57.48=0.11 / 57.50=0.27 / 57.53=0.83 / 57.54≈1.0 / 57.55=0.79 / **57.56≈0.66** (9) | 9-pt mean **0.65** (last-3 mean **~0.82**) | **KEEP 0.80 per `When to adjust` 3-sprint window rule** — last 3 (57.54≈1.0 + 57.55=0.79 + 57.56≈0.66) **2/3 < 0.7 lower-trigger NOT yet MET** (need 3+ consecutive); 9-pt mean steady 0.65 (post-tier-4-confound-resolution tracking continues). Class baseline 0.80 well-calibrated for human-pace + Day 0 + Day 2 overhead included; agent residual captured at tier-4 sub-class layer correctly per Sprint 57.56 retro Q4 (`mechanical-greenfield-design-decisions` 0.65 1st validation CONFIRMED CLEANLY at ratio ~1.02 IN BAND middle ✅ stripping confound from Sprint 57.56 case). AD-MediumBackend-AICadence-Recalibration continues Phase 58+ |
| `medium-frontend` | 0.65 | 57.1=0.85 / 57.13≈0.95-1.0 / 57.49=0.064 / 57.54≈0.32 / 57.55=0.53 / **57.56≈0.50** (6 — Sprint 57.13 added retroactively per memory subfile; 57.54+57.55+57.56 frontend track sub-portion) | 6-pt mean **~0.54** (last-3 mean ~0.45) | **KEEP 0.65 per `When to adjust` 3-sprint window rule + confound-resolved-at-sub-class-layer discipline** — last 3 (57.54≈0.32 + 57.55=0.53 + 57.56≈0.50) **3/3 < 0.7 lower-trigger CRITERIA MET 4th consecutive sprint** BUT per Sprint 57.50/57.55 retro discipline: class baseline holds when confound stripped at sub-class layer; Sprint 57.56 frontend at tier-4 `mechanical-greenfield-design-decisions` 0.65 layer = human-equivalent IN BAND middle when agent_factor stripped (tier-4 1st validation cleanly CONFIRMED at ratio ~1.02 sprint-level). Class baseline 0.65 CONFIRMED well-calibrated for human-pace; AD-medium-frontend-Baseline-Recalibration continues Sprint 57.57+ 7th data point (need consistent human-factor data to recalibrate; agent-delegated confound persists across 57.49+57.54+57.55+57.56) |
...
where agent_factor = {
  ...
  agent-delegated (tier-4 sub-class table — Sprint 57.55 retro Q4 tier-4 SPLIT ACTIVATED effective 2026-05-28 onwards):
    ...
    mixed-multidomain-bundle-mechanical:          0.65   (3+ independent tracks WITH mechanical pattern reuse component — e.g. backend ORM + Pydantic + tests bundle; evidence: Sprint 57.46 ratio 1.60 at 0.45→rollback to 0.65 — UNCHANGED from Sprint 57.48 Option B)
