...
| **`frontend-verbatim-css-repoint` (NEW Sprint 57.29; baseline LIFTED 0.60→0.50 Sprint 57.31 Day 4; bimodal-by-shape watch Sprint 57.34)** | **0.50** (was 0.60 Sprint 57.29-57.31; lifted Day 4 per `When to adjust` after 57.30+57.31 both below band by 0.45+); HYBRID weighted blend unchanged | rich: 57.29≈1.0 / 57.30≈0.40 / 57.31≈0.35 / 57.32~0.40-0.55 — 3-pt mean ≈0.40 (ex-57.29 anchor); non-rich: **57.34≈0.95-1.05 (5 total)** | 5-pt mean ≈0.60; **bimodal-by-shape emerging** — rich-dashboard 3-pt ≈0.40 below band, non-rich-dashboard 1-pt ≈1.0 in band middle | **5th app (Sprint 57.34 /orchestrator) ratio actual/committed ≈0.95-1.05 — 2nd validation data point for `AD-Sprint-Plan-frontend-verbatim-css-repoint-baseline-lift` AND 1st non-rich-dashboard shape in epic (config/tabbed-forms with 6 tabs + Tabs/Field/Switch primitive promotions to mockup-ui). Lands in [0.85, 1.20] band middle. Combined with prior 4 rich-dashboard apps (3-pt mean ≈0.40 below band ex-57.29 anchor), bimodal-by-shape hypothesis emerging → NEW `AD-Sprint-Plan-frontend-verbatim-css-repoint-shape-bimodal-watch`. 2-data-point span (57.32 rich ≈0.40-0.55 + 57.34 non-rich ≈0.95-1.05) suggestive but insufficient per `When to adjust` 3-sprint window rule. KEEP 0.50 baseline this iteration. If Sprint 57.35 (another non-rich-dashboard shape; e.g. /loop-debug or /state-inspector) lands in band → propose class split `-rich-dashboard` (0.40) vs `-config-form` (0.50). If lands below band → class-wide variance after all → 0.50 → 0.40 lift. Sprint 57.34 agent-assisted (code-implementer agent for Day 1-3) — ratio estimated from human-equivalent effort caveat per Sprint 57.13/57.27/57.28/57.29 convention.** |
