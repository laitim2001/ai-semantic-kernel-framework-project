...
| **Current Sprint** | **Sprint 57.150 — memory-formation dedup: `UserLayer.write` upsert-by-key — MERGED** (PR #354, main `32ae4489`; branch from main `7460f23b`, CI all-green). Closes `AD-Memory-User-Upsert-By-Key`: `write()` always INSERTed → 57.149's auto_extract (every send) piled up duplicate user-fact rows that diluted 57.148 `profile()` top-k. Fix: `MemoryUser.dedup_key VARCHAR(32) = md5(normalize(content))` + `(tenant,user,dedup_key)` unique constraint + `pg_insert … on_conflict_do_update` (greatest confidence / latest content / bump updated_at / KEEP first-writer source) + migration **0032** (add col → backfill → dedup existing → constraint). All 3 writers (57.148 nudge / 57.149 auto_extract / agent `memory_write`) dedup at the one `UserLayer.write` chokepoint. Exact-normalized dedup (semantic = CARRY-026). Backend-only, NO wire(26)/frontend/new-contract. **Drive-through PASS** (real chat-v2 + Azure gpt-5.2, dan): send-1 agent `memory_write` (conf 0.6, source none) + auto_extract (conf 0.99) of the SAME verbatim fact → **1 row** (greatest conf 0.99, updated bumped, first-writer source kept) = INTRA-send dedup LIVE; send-2 repeat → STILL 1 row (cross-send); new-session single clean recall. Pre-57.150 = 2-4 rows. pytest 3022 (+9) · mypy 0/393 · run_all 11/11 · migration up/down/up clean · FE untouched. NEW calib `memory-upsert-dedup-spike` 0.60 (1st pt ~1.07 IN band). CHANGE-117 (no design note). NEW carryover `AD-Memory-Ops-Emit-Dedup-On-Reaffirm`. (Prior: Sprint 57.149 MERGED PR #352, main `02ca23ba`.) Detail: `memory/project_phase57_150_memory_upsert_dedup.md`. |
...
**Last Updated**: 2026-06-30 (Sprint 57.150 — memory-formation dedup: `UserLayer.write` upsert-by-key — `dedup_key=md5(normalize(content))` + `(tenant,user,dedup_key)` unique + `pg_insert … on_conflict_do_update` (greatest conf / keep first-writer source) + migration 0032; a repeated fact UPDATEs one row instead of accumulating dups that diluted 57.148 `profile()` top-k; all 3 writers dedup at one chokepoint; drive-through PASS real chat-v2 + Azure gpt-5.2 — memory_write + auto_extract of the same verbatim fact → 1 row (intra-send), send-2 → still 1 row (cross-send), single clean recall; CHANGE-117; MERGED PR #354 main `32ae4489` closes `AD-Memory-User-Upsert-By-Key`); see `memory/` for sprint history
