...
| **Current Sprint** | Sprint 57.127 **PR-pending** (branch `feature/sprint-57-127-chatv2-live-multiturn-context`, from main `c1d3d1be`) — **chat-v2 live multi-turn context** (closes `AD-ChatV2-Live-MultiTurn-Context` — the real product bug 57.126's drive-through surfaced: a follow-up send started `loop.run()` with no prior turns → "its population?" → "what is 'it'?"). User picked **Approach A** (new `messages`-table writer). NEW provider-neutral `MessageStore` ABC (sibling to `Checkpointer`, bound to session+tenant) + `DBMessageStore` (load order-by-seq / append seq-from-MAX / best-effort SAVEPOINT / tenant-scoped) + serde relocated to `_contracts/message_serde.py`; the loop **self-loads** prior at `run()` start + persists user-prompt-at-start + final-answer-at-2-end_turn-sites; **`loop.run()` ABC signature UNCHANGED**; `make_chat_message_store` factory + `build_handler` injects the MAIN loop only. Pure backend; NO migration (default partitions exist); wire 24. Day-1 de-risk: dropped the plan's side-list (FINAL branch doesn't append the answer) → simpler 2-point persist; intra-turn tool round-trips deferred → `AD-ChatV2-Ledger-Tool-RoundTrips`. mypy 0/372 · run_all 10/10 · pytest 2720+5skip · Vitest 904 · mockup 51. **Drive-through PASS** (real Azure gpt-5.2, session 9e89775d: turn 2 "its population?" → "Paris ~2.1M", `messages_count` 4→6 rehydrated, DB ledger seq 1→4). Detail: `memory/project_phase57_127_chatv2_live_multiturn_context.md` + CHANGE-094 + design note 38. |
...
**Last Updated**: 2026-06-16 (Sprint 57.127 — chat-v2 live multi-turn context: a NEW provider-neutral `MessageStore` ABC (sibling to `Checkpointer`) + `DBMessageStore` (tenant-scoped, seq-from-MAX, best-effort SAVEPOINT) + serde relocated to `_contracts/message_serde.py`; the loop self-loads prior at `run()` start + persists user-prompt-at-start + final-answer-at-end_turn, `loop.run()` ABC signature UNCHANGED; `make_chat_message_store` factory + `build_handler` injects the MAIN loop only; closes `AD-ChatV2-Live-MultiTurn-Context` (the 57.126 drive-through-surfaced bug); pure backend / NO migration / wire 24; gates green (mypy 0/372 / pytest 2720+5skip / Vitest 904 / mockup 51) + drive-through PASS (real Azure: turn 2 "its population?" → "Paris ~2.1M", `messages_count` 4→6 rehydrated, DB ledger seq 1→4); intra-turn tool round-trips deferred → `AD-ChatV2-Ledger-Tool-RoundTrips`; CHANGE-094 + design note 38); see `memory/` for sprint history
