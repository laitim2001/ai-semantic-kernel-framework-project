...
| **Current Sprint** | **Sprint 57.138 PR-pending** (branch `feature/sprint-57-138-verification-key-condition`, from main `57423a80`) — **verification key-condition judge spike** — closes `AD-Verification-KeyCondition-PerTask` (research #8, 3rd in the canonical order after #6+#3). NEW `key_condition.txt` judge template extracts the request's per-task must-satisfy conditions (count/format/ordering/inclusion) + checks each (superset of the generic `output_quality` floor), selectable via the EXISTING `chat_verification_judge_template` lever (DEFAULT unchanged). A permanent real-Azure A/B harness measured it: **instruction_violation 83% (generic) → 100% (key_condition), gain +16.67pp** BUT **false_positive_rate 20%** + ~1.8× tokens → overall accuracy **tie 90.91%** → does NOT clear the gain≥30%/fp≤20% thresholds → **keep `output_quality` default; `key_condition` ships as a selectable opt-in**. NOTE: the generic judge is less blind than theory (83% not ~0%) because A3 trace-awareness already infers "asked 3, got 5 → contradicts-trace". Backend-only, ZERO src code change (template = data file; lever auto-selectable via glob), NO migration/wire/frontend. pytest 2797+5skip (+11) · mypy 0/374 · run_all 10/10. **Drive-through PASS (兩者結合)** (chat-v2 real Azure gpt-5.2: ARM A key_condition active → "exactly 3 colors"→"Red, blue, yellow"→verification_passed 0.99 + ARM B default → "capital of France"→"Paris"→verification_passed 0.99; the CATCH = the A/B harness, the UI = selectability + no-regression). NEW class `verification-keycondition-spike` 0.60 (1st pt ~0.98 IN band). CHANGE-105 + design note 42. Detail: `memory/project_phase57_138_verification_key_condition.md`. |
...
**Last Updated**: 2026-06-24 (Sprint 57.138 — verification key-condition judge spike: a NEW `key_condition.txt` template extracts per-task must-satisfy conditions + checks each (superset of the generic `output_quality` floor), selectable via the existing `chat_verification_judge_template` lever, DEFAULT unchanged; closes `AD-Verification-KeyCondition-PerTask` (#8); a real-Azure A/B showed instruction_violation 83%→100% (gain +16.67pp) BUT 20% false-positive + ~1.8× tokens → net accuracy tie → does NOT clear gain≥30%/fp≤20% thresholds → keep output_quality default, key_condition = opt-in; the generic judge is less blind than theory ∵ A3 trace-awareness; backend-only ZERO src code change (template = data file, lever auto-selectable via glob), NO migration/wire/frontend; pytest 2797+5skip (+11); drive-through PASS (chat-v2 ARM A key_condition active + ARM B default both verification_passed; CATCH = A/B harness, UI = selectability + no-regression); NEW calib class `verification-keycondition-spike` 0.60; CHANGE-105 + design note 42); see `memory/` for sprint history
