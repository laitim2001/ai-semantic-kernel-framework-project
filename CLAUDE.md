...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.139 MERGED** (PR #331, main `ac8c5ff3`; branch from main `3bf4a727`) — **layered compaction: tool-result preclear yield + ACON-band spike** — closes `AD-Context-Layered-Compaction-ACON` (research #4, Cat 4; next after #6/#3/#8). NEW LLM-free `PreClearCompactor` (standalone tool-result clear at a LOWER trigger, reuses `DefaultObservationMasker`, real token-reduction-ratio `tokens_after`) + `ChainedCompactor` (preclear→hybrid, threads state so the cheap layer defers semantic; `loop.py` UNTOUCHED) + a default-OFF env lever `CHAT_COMPACTION_PRECLEAR_RATIO` (factory returns the bare HybridCompactor when unset = byte-identical) + a permanent per-layer yield harness `benchmark_layered_compaction.py`. A real-Azure A/B (6 cases): **mean tool_clear_reduction 33.72% → IN ACON band [26-54%]**, captures ~83% of total yield for free + **semantic_deferred_rate 66.67%** → keep default OFF, ship preclear as selectable opt-in (composition-dependent + user-anchored NO-OP single-send). The "different axis from C2 (57.109)": C2 made the summarizer cheap; #4 runs the free layer FIRST to defer it. Backend-only, NO migration/wire/frontend. pytest 2825+5skip (+28) · mypy 0/376 · run_all 10/10. **Drive-through PASS (兩者結合)** (chat-v2 real Azure gpt-5.2: ARM A preclear ON → 3 turns TCP/TLS/QUIC no-regression w/ ChainedCompactor active + ARM B default OFF → "Tokyo" bare HybridCompactor byte-identical; CATCH = the harness yield, UI = no-regression + chain wiring — masking NOT UI-triggerable on plain tool-less chat). NEW class `layered-compaction-spike` 0.60 (1st pt ~0.97 IN band). CHANGE-106 + design note 43. Detail: `memory/project_phase57_139_layered_compaction.md`. |
...
**Last Updated**: 2026-06-24 (Sprint 57.139 — layered compaction tool-result preclear spike: a NEW LLM-free `PreClearCompactor` (standalone tool-result clear at a lower trigger, reuses `DefaultObservationMasker`, real token-reduction-ratio `tokens_after`) + `ChainedCompactor` (preclear→hybrid, threads state to defer semantic; `loop.py` UNTOUCHED) + a default-OFF env lever `CHAT_COMPACTION_PRECLEAR_RATIO` (factory returns bare HybridCompactor when unset = byte-identical) + a permanent per-layer yield harness; closes `AD-Context-Layered-Compaction-ACON` (#4); a real-Azure A/B (6 cases) showed mean tool_clear_reduction 33.72% IN the ACON band [26-54%] capturing ~83% of total yield for free + semantic_deferred_rate 66.67% → keep default OFF, preclear = selectable opt-in (composition-dependent + user-anchored NO-OP single-send); the different axis from C2 (57.109) = run the free layer FIRST to defer the cheap summarizer; backend-only NO migration/wire/frontend; pytest 2825+5skip (+28); drive-through PASS (chat-v2 ARM A preclear ON 3 turns no-regression + ARM B default OFF byte-identical; CATCH = harness yield, UI = no-regression + chain wiring); NEW calib class `layered-compaction-spike` 0.60; CHANGE-106 + design note 43); see `memory/` for sprint history
