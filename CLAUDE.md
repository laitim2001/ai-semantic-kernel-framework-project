...
| **Current Sprint** | Sprint 57.78 (PR pending) — **Subagents Registry real list (closes `AD-Subagent-RealList-Phase58` → 🎉 Area-A program COMPLETE, last item)**: re-pointed `GET /subagents` STUB (never-persisted invocations) → real per-tenant `agent_catalog` (57.70) registry view + wired mockup-ported `/subagents` page (real role←key/model/modes←allowed_modes/status; KPI counts derived; detail spec/budget/tools real; usage metrics honest-gapped "—", AP-4 no fabrication; removed 8-row fixture + carryover banner). Backend re-point + FE wire (sequential 2-agent); no new migration; feature-continuation (no design note). User-locked Catalog/Registry view (AskUserQuestion) over runtime invocations. D-DAY1-1 parent fix (rewrote existing `test_subagent_registry.py` into catalog contract, Never Delete respected) + D-DAY1-2 (i18n zh-TW English→繁中). Parent re-verify: backend mypy 0/331 + pytest 2109 + run_all 10/10; frontend build tsc 0 + Vitest 744 + check:mockup-fidelity baseline 50 + Playwright subagents 2. (Sprint 57.77 Memory ops-history FE — `AD-Memory-OpsHistory-Backend` FULLY CLOSED — merged via PR #243.) Detail: `memory/project_phase57_78_subagents_registry_real_list.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-04 (Sprint 57.78 — Subagents Registry real list, closes `AD-Subagent-RealList-Phase58` → 🎉 Area-A program COMPLETE: re-point GET /subagents STUB → agent_catalog registry view + wire /subagents page (real role/model/modes/status, KPI derived, honest-gap usage metrics, removed 8-row fixture + banner); backend re-point + FE wire (sequential 2-agent), no migration, feature-continuation; D-DAY1-1 rewrote existing test into catalog contract (Never Delete respected) + D-DAY1-2 i18n zh-TW English→繁中; backend mypy 0/331 + pytest 2109 + run_all 10/10; frontend build tsc 0 + Vitest 744 + check:mockup-fidelity baseline 50 + Playwright subagents 2; CHANGE-046; PR pending. Prior: Sprint 57.77 Memory ops-history FE merged via PR #243 — AD-Memory-OpsHistory-Backend FULLY CLOSED); see `memory/` for sprint history
