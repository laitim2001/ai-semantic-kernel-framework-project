...
| **Current Sprint** | Sprint 57.126 (PR-pending, NOT pushed) — **chat-v2 session history replay, arc slice 2/2: complete the backend transcript foundation + the frontend click→replay** (closes `AD-ChatV2-Session-History-Replay-Phase58` → the 2-sprint arc is COMPLETE). Day-0 三-prong caught the 57.125 foundation was incomplete: user prompts are persisted NOWHERE (`state_data` excludes messages; messages table no writer) + a latent multi-turn `main_seq` collision. User picked Option B (2 AskUserQuestions). Backend (`router.py`): `_max_main_seq` seeds `main_seq` from MAX (multi-turn ordering fix) + persist the user prompt as a `user_message` row per send (persist-only, reuses `_persist_main_event`). Frontend: `fetchSessionEvents` + `loadSessionHistory` (conversation-only reset → replay through the EXISTING `mergeEvent` + a new persist-only `user_message` case) + the `SessionList` click rewire. ZERO new CSS; wire 24; no migration/codegen. mypy 0/370 · run_all 10/10 · pytest 2712+5skip · Vitest 904 · mockup 51. **Drive-through PASS** (real Azure gpt-5.2: 2-turn chat → reload → click → complete replay (user+agent) → continue same session; seq 1..51 monotonic). Detail: `memory/project_phase57_126_chatv2_session_history_replay.md` + CHANGE-093. |
...
**Last Updated**: 2026-06-16 (Sprint 57.126 — chat-v2 session history replay, arc slice 2/2: complete the backend transcript foundation (`_max_main_seq` multi-turn seed + persist a per-send `user_message` row, persist-only, reuses `_persist_main_event`) + the frontend click→replay (`fetchSessionEvents` + `loadSessionHistory` replays `/events` through the EXISTING `mergeEvent` + a persist-only `user_message` case + the `SessionList` click rewire); closes `AD-ChatV2-Session-History-Replay-Phase58` → arc COMPLETE; ZERO new CSS / wire 24 / no migration; gates green (pytest 2712+5skip / Vitest 904 / mockup 51) + drive-through PASS (real Azure: 2-turn chat → reload → click → complete replay → continue same session, seq 1..51 monotonic); CHANGE-093, NO design note); see `memory/` for sprint history
