...
| **Current Sprint** | Sprint 57.103 (PR pending) — **B2b chat-user inject-to-teammate: backend primitive + US-5 SHIPPED; inject UI 🚧 DEFERRED to §2.5** (harness-deepening Workflow B slice B2, second half). SHIPPED + proven: US-1 `POST /chat/{id}/subagents/{sid}/inject` endpoint (B1 sibling) + US-2 `TeammateInboxScope` register/unregister lifecycle (`make_teammate_inbox_scope` — the teammate's queue lives only while it runs) + US-3 `MessageInjected`-in-relay (no new event type) + US-5 inline "Teammate · peer" + real tokens (drive-through-proven). **🔴 inject UI (US-4/6) DEFERRED**: the drive-through (real UI + real Azure gpt-5.2) found the inject control can NEVER render — the Cat 11→12 SSE relay (57.95) buffers subagent events + flushes them only after the awaited teammate completes (the parent blocks in `wait_for`) → the FE never observes a teammate "running" → a `status==="running"`-gated control never appears; the live inject window needs the detached/streaming teammate (proposal §2.5). Removed the un-drivable control per Option A (no dead control); kept the proven backend primitive (reusable for §2.5). `loop.py` diff 0. Gate: mypy `src` 0/355 · run_all 10/10 (count unchanged) · full pytest **2342 +4 skip** (+9, 0 del) · FE Vitest 143 / mockup-fidelity 53. Detail: `memory/project_phase57_103_inject_to_teammate.md` + CHANGE-070. Next: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-11 (Sprint 57.103 — B2b chat-user inject-to-teammate: backend primitive + US-5 shipped (US-1 `POST /chat/{id}/subagents/{sid}/inject` endpoint + US-2 `TeammateInboxScope` register/unregister lifecycle `make_teammate_inbox_scope` + US-3 `MessageInjected`-in-relay + US-5 inline "Teammate · peer"/real-tokens drive-through-proven); inject UI (US-4/6) DEFERRED to §2.5 — the drive-through found the buffered SSE relay (57.95) flushes subagent events only after the awaited teammate completes → the FE never observes a teammate "running" → the control can never render; removed the un-drivable control per Option A, kept the proven backend primitive; `loop.py` diff 0; full pytest 2342+4skip (+9); CHANGE-070 + 17.md + design note 20 edit); see `memory/` for sprint history
