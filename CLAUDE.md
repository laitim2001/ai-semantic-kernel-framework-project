...
| **Current Sprint** | Sprint 57.117 (PR-pending) — **Skills epic catalog hardening: per-tenant quota + instructions body-size limit — drive-through BOTH legs PASS** (closes `AD-Skills-Per-Tenant-Quota`; 1st of 5 pending Skills ADs run in sequence). The 57.114 catalog (an unbounded write surface) gains 2 write-path guards: a per-tenant count quota (`SKILLS_MAX_PER_TENANT` 50, env-overridable → `SkillQuotaExceededError` 409 in `TenantSkillService.create`'s tenant-scoped `func.count`, auto-mapped by the admin POST's existing `except TenantSkillError`, no handler change) + an `instructions` body-size cap (`SKILLS_MAX_INSTRUCTIONS_CHARS` 20_000 → Pydantic `max_length` 422; DB stays `Text`, NO migration). `SkillListResponse` += `max_skills`/`max_instructions_chars` → admin Skills tab "N / max" + disable-Add + textarea cap + 409/422 render from the SERVER value (FE type fields OPTIONAL → mocks/old-cache fall back `Infinity`; `useTenantSkills`/`tenantSettingsService` UNCHANGED, 1 file lighter than plan). Gate: mypy 0/370 · run_all 10/10 · backend pytest 2630+5skip (+7) · FE Vitest 873 (+4) / mockup 51 (count 24, no codegen/migration). Drive-through (real admin tab + lowered env limit, acme-skills): Leg A "1/2"→create→"2/2"+"Skill limit reached"+Add disabled + forced POST 409 / Leg B textarea capped 200 + forced 201-char POST 422 (low env override visible in tab proves server-sourced, AP-4-safe). NEW `config-validation-hardening` 0.55 (1st pt ~0.95-1.0 IN band, parent-direct). NO design note (feature continuation). Detail: `memory/project_phase57_117_skills_per_tenant_quota.md` + CHANGE-084. Next: remaining Skills ADs (`claudedocs/1-planning/next-phase-candidates.md` — 118 bundled-scripts / 119 authoring-UI / 120 inspector-metadata / 121 slash-menu-mockup) + carryover pool. |
...
**Last Updated**: 2026-06-15 (Sprint 57.117 — Skills epic catalog hardening: per-tenant skill-count quota (`SkillQuotaExceededError` 409) + `instructions` body-size limit (Pydantic `max_length` 422), env-overridable, surfaced on `SkillListResponse` to the admin Skills tab; NO migration / NO wire (count 24), drive-through BOTH legs PASS, closes AD-Skills-Per-Tenant-Quota; CHANGE-084, no design note); see `memory/` for sprint history
