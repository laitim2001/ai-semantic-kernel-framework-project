...
| **Current Sprint** | Sprint 57.86 (PR pending) — **C-12 IAM Block B/C: local credentials + password-login vertical spike** (closes `AD-Auth-Credentials-PasswordLogin-Phase58`; 2nd C-12 spike, completes 57.85's accepted-not-stored gap): `bcrypt` + `users.password_hash` (migration 0027) + `passwords.py` (anyio-offload + DUMMY_HASH) + `CredentialsService` (generic-401 anti-enumeration) + invite-accept now stores the password + `POST /auth/password-login` (JSON body, JWT/cookie mirror dev-login, EXEMPT) + NEW mockup-faithful `/auth/password-login` page (+ route + i18n + mockup `AuthPasswordLogin`). Spike → design note `22-iam-credentials-spike.md` (8-pt gate). mypy 0/342 + pytest 2202 + run_all 10/10 + Vitest 761 + mockup-fidelity ✓ (HEX_OKLCH_BASELINE 50→53); CHANGE-053. Detail: `memory/project_phase57_86_iam_credentials.md`. Next: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-06 (Sprint 57.86 — C-12 IAM Block B/C local credentials + password-login spike: bcrypt + users.password_hash (migration 0027) + passwords.py + CredentialsService (generic-401 anti-enumeration) + invite-accept stores password + POST /auth/password-login (EXEMPT) + NEW /auth/password-login page + mockup AuthPasswordLogin; closes AD-Auth-Credentials-PasswordLogin-Phase58; 2nd C-12 spike; design note 22-iam-credentials-spike.md 8-pt gate; mypy 0/342 + pytest 2202 + run_all 10/10 + Vitest 761 + mockup-fidelity ✓ (HEX_OKLCH_BASELINE 50→53); CHANGE-053; PR pending); see `memory/` for sprint history
