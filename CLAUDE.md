...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.143 MERGED** (PR #339, main `3c1ce9c4`; branch from main `d7d97039`) — **user Stop→continue durable interrupt-resume** — closes `AD-UserStop-Resume-Context` (Scope B, full CC parity; eval `claudedocs/5-status/user-interrupt-resume-context-gap-20260625.md`). The interrupted-run ledger persisted NOTHING (per-append SAVEPOINT rode the request session → `get_db_session` rollback on the client-disconnect `CancelledError`). Fix: **US-1** `DBMessageStore` own-session-per-call (ctor factory + `set_config('app.tenant_id')` for FORCE-RLS `messages` + immediate commit) → the interrupted run's user prompt + completed tool batches survive a mid-run Stop; **US-2** `cancel_session` persists a `[Request interrupted by user]` marker (best-effort, own session) + chat-v2 Stop also `POST /cancel`. `loop.py` + `session_registry.py` UNTOUCHED; NO migration / wire stays 26 / NO mockup. pytest 2881+5skip · mypy 0/381 · Vitest 922 · v2 lints 15 (llm_sdk_leak 3/3). **Drive-through (real Azure gpt-5.2, session 35789e63)**: DB ledger seq 5 = interrupted essay prompt (DURABLE — survived Stop) + seq 6 = marker; "continue" rehydrated 19 msgs + continued the ORIGINAL task (write_todos naming the computing essay) — NOT "continue from what?". NEW calib class `chatv2-userstop-resume-durability` 0.60 (1st pt ~1.0 IN band, parent-direct). CHANGE-110 + design note 47. cancel_event-poll / partial-answer-capture deferred (Phase58 ADs). (Prior: Sprint 57.142 OTel GenAI semconv MERGED PR #337.) Detail: `memory/project_phase57_143_userstop_resume.md`. |
...
**Last Updated**: 2026-06-25 (Sprint 57.143 — user Stop→continue durable interrupt-resume; closes `AD-UserStop-Resume-Context`; own-session `DBMessageStore` (factory + set_config + immediate commit) + `/cancel` interrupt marker + chat-v2 Stop→/cancel; drive-through PASS real Azure; CHANGE-110 + design note 47); see `memory/` for sprint history
