...
**Last Updated**: 2026-06-24 (Sprint 57.140 — explicit todo-list task primitive (research #1): a full vertical slice — Cat 2 `write_todos` tool (replace-whole-list, auto-pass risk=LOW) + Cat 3/7 `session_todos` table (migration 0031, RLS) + `DBTodoStore` (upsert, mirrors DBMessageStore) + `Todo` contract + Cat 5 plan-first nudge + Cat 1 run-start `## Active Plan` re-injection (loop, async) + `TodosUpdated` emit after write_todos + Cat 12 `todos_updated` wire event (25→26) + chat-v2 Inspector 5th "Todos" tab (existing mockup classes, styles-mockup.css byte-identical); naming avoids `task` (Cat 11 collision); ~28 files; pytest 2843+5skip (+20) · mypy 0/380 · run_all 10/10 · Vitest 920 · mockup 51; **drive-through STRONG PASS** (real Azure gpt-5.2: send 1 proactive write_todos 3 items → 3/3 completed; send 2 cross-send → rehydrated 3 + added 4th → 4/4 completed — AP-4 ignores-the-list risk FALSIFIED); NEW calib class `task-primitive-spike` 0.60 (1st pt ~0.95-1.0 IN band); CHANGE-107 + design note 44); see `memory/` for sprint history
