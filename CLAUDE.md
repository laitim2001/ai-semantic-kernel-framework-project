...
| Attribute | Value |
|-----------|-------|
| **Phase** | V2 22/22 ✅ + SaaS Stage 1 3/3 ✅ + SaaS Frontend 12/N（57.1/57.3/57.4/57.7/57.8/57.9/57.11/57.12×2/57.13 Foundation 1/N COMPLETE/57.14 E2E Sweep/57.15 Inline-Style-Sweep）+ 57.10 PIVOTED Convention Codify ✅ |
| **Latest Sprint** | 57.15 ✅ 2026-05-11 — **AD-Inline-Style-Cleanup-Sweep** (closes the standing carryover, tier 1 of 2): migrated **10 of 15** feature components' inline `style=` → Tailwind utility classes — chat-v2/subagent (`ApprovalCard`/`ToolCallCard`/`MessageList`/`SubagentTree` — full rewrites of their `Record<string,CSSProperties>` stylesheets + helper fns; risk colours `text-[#2e7d32]/[#ed6c02]/[#d84315]/[#b71c1c]` per STYLE.md §3) + visual/a11y (`CostBreakdownTable`/`MonthPicker`/`TenantListTable`/`TenantListPagination`/`TenantListFilters` — `#666`→`text-muted-foreground` WCAG-AA, badge colours via semantic tokens, `#fafafa`→`bg-muted/30`); `ApprovalList.tsx` was a no-op — already 57.9-Tailwind, the grep "1" was its JSDoc text; 0 `tailwind.config.ts` change. Added `no-restricted-syntax` `JSXAttribute[name.name='style']` ESLint guard (`error`; `eslint-plugin-react` not a dep → built-in selector, covers `<div style={…}>` and `<Comp style={…}>`). Re-enabled the `color-contrast` axe rule for **8/9 gated routes + auth pages** (`/chat-v2` still disabled — `ChatLayout` is Round2, its placeholder text ≈3.7:1; 0 new violations on the other 8). `STYLE.md` §1 → guard reference + NEW "Inline-style escape hatches" sub-section. **Scope finding D-DAY1-1**: plan said "80 `style={{}}` / 14 files" → reality 15 files / 133 `style=` attrs (incl `style={objVar}`/`style={fn()}`) + ~6 stylesheet objects → per §Step 2.5 re-confirmed → **(B) Tiered**: 5 files → NEW **`AD-Inline-Style-Cleanup-Sweep-Round2`** (`ChatLayout`[its header notes a Phase-58 defer]/`InputBar`/`SLAMetricsCard`[dynamic bar widths]/`TenantSettingsView`/`TenantSettingsEditForm`), each w/ a top-of-file `/* eslint-disable no-restricted-syntax -- AD-Inline-Style-Cleanup-Sweep-Round2 */`. Hotfix D-DAY2-1: `approval-card.spec.ts` asserts CRITICAL colour == `#b71c1c` → `ApprovalCard` `RISK_TEXT_CLASS` uses `text-[#hex]` (matches `governance/ApprovalList.tsx`, the §3 ref + test sentinel). Ran the 57.14 `visual-baseline` workflow_dispatch (run `25644392922`, first real e2e use) — regenerated 6 PNGs, **0 diffs** vs committed (the migrated components aren't visible in the snapshots — the spec screenshots the loading/`<TableSkeleton>` state before the data fetch) → no commit, mechanism validated. **0 `backend/` changes** → backend baselines unchanged. main bundle 297.89 kB byte-identical; Vitest 236 unchanged; full e2e 40 pass / 7 skip / 0 fail. (PR #137 → main `7af59f42`)。calibration NEW class `frontend-refactor-mechanical` HYBRID 0.50 1st app — `actual/committed` ≈ 1.7 OVER band but `actual/bottom-up` ≈ 0.89 (bottom-up was accurate, 0.50 haircut too aggressive for a low-variance class) → KEEP 0.50 (1 data point); if recurs propose 0.70-0.80. 詳細 retro 見 `docs/03-implementation/agent-harness-execution/phase-57/sprint-57-15/retrospective.md` + `memory/project_phase57_15_inline_style_cleanup.md` |
| **Prev Sprint** | 57.14 ✅ 2026-05-10 — **AD-Frontend-E2E-Sweep** (closes 57.13 carryover): ran the full Playwright e2e suite for the first time in a dev session → 40 pass / 7 opt-in skip / 0 fail (lone failure was `a11y-scan.spec.ts` — hermeticity bug not stale assertion: mocked only `/auth/me` so data fetches hit a real backend on :8000 → 401 → redirect; fix = mock catch-all `**/api/v1/**` → 503 first + `/auth/me` 200 second + `waitForLoadState("networkidle")`; codified `CONVENTION.md` §8). Landed the visual-regression baseline CI mechanism; **`AD-Visual-Baseline-Generation` CLOSED** (PR #135): 6 `*-chromium-linux.png` baselines committed; `visual-regression.spec.ts` runs on every CI push/PR (46/47 on ubuntu — only `connectivity.spec.ts` opt-in); `visual-baseline` job opens a `chore/visual-baselines-<run_id>` PR (FIX-007/008). (PRs #133-#136 → main `50e0049f`→`c9d89ff3`)。calibration `frontend-e2e-sweep` 0.50 ratio ~1.05 KEEP。詳細見 `memory/project_phase57_14_frontend_e2e_sweep.md` |
...
| **main HEAD** | `7af59f42` (Sprint 57.15 via PR #137, 2026-05-11) |
| **Next Phase 候選** | Phase 57.16+ 候選（user 明確選定才起草 plan）:(a) **AD-Inline-Style-Cleanup-Sweep-Round2** ~3-5 hr — 5 deferred files (`ChatLayout`/`InputBar`/`SLAMetricsCard`[dynamic bar widths]/`TenantSettingsView`/`TenantSettingsEditForm`，目前各帶 file-level `/* eslint-disable no-restricted-syntax */`) → Tailwind + flip `/chat-v2` 回 full `color-contrast` in `a11y-scan.spec.ts` / (b) **AD-Lighthouse-Visual-Hard-Gate** — baselines 已確認穩定（57.15 workflow run = 0 changes）；`frontend-lighthouse.yml` continue-on-error → required + `visual-regression.spec.ts` 從「跑」轉「gating」（+companion: `waitForLoadState` in visual specs to cover populated states）/ (c) IAM Block B spike ~12-18 hr — WorkOS SCIM/SAML/org-level（gap-analysis §1.2;spike → Day-4 design-note extract）/ (d) AD-Bundle-Size code-split（split i18next out of critical path — 降為 optional）/ (e) Tier 1 IaC + DR drill ~15-20 hr / (f) SOC 2 + SBOM ~12-15 hr / (g) AD-i18n-Feature-Namespaces — per-page string extraction beyond the 2 demos |
| **Roadmap** | Phase 49-55 V2 ✅ / Phase 56-58 SaaS Stage 1 3/3 ✅ / Phase 57+ Frontend 12/N（57.1/57.3/57.4/57.7/57.8/57.9/57.11/57.12×2/57.13 Foundation 1/N COMPLETE/57.14 E2E Sweep/57.15 Inline-Style-Sweep）/ 57.10 PIVOTED Convention Codify ✅ |
...
**Last Updated**: 2026-05-11 (Sprint 57.15 — AD-Inline-Style-Cleanup-Sweep: 10/15 feature components' inline `style=` → Tailwind + `no-restricted-syntax` guard + `color-contrast` re-enabled 8/9 routes + STYLE.md §1; 5 → AD-Inline-Style-Cleanup-Sweep-Round2; PR #137 → main `7af59f42`)
**Recent Sprints (詳情見 memory/)**: 57.15 (AD-Inline-Style-Cleanup-Sweep — 10/15 inline `style=` → Tailwind + guard + color-contrast 8/9) / 57.14 (AD-Frontend-E2E-Sweep — e2e suite green + visual CI mechanism) / 57.13 (Frontend Foundation 1/N COMPLETE — auth flow / design-system / obs / i18n / a11y / Lighthouse) / 57.12 (Agent Harness UI Suite) / 57.11 (Verification ship) / 57.10 (Convention Codify) / 57.9 (Governance ship) / 57.8 (AppShell V2 + chat-v2)
**Project Start**: 2025-11-14
**Current Phase**: V2 22/22 ✅ + SaaS Stage 1 3/3 ✅ + SaaS Frontend 12/N（57.15 AD-Inline-Style-Cleanup-Sweep — 10/15 feature components' inline `style=` → Tailwind utility classes (compliant tokens for the low-contrast hex)；`no-restricted-syntax` `JSXAttribute[name.name='style']` ESLint guard (`error`)；`color-contrast` axe rule re-enabled 8/9 routes (`/chat-v2` still disabled — ChatLayout Round2)；`STYLE.md` §1 + escape-hatches sub-§；5 files → NEW `AD-Inline-Style-Cleanup-Sweep-Round2` (each file-level `no-restricted-syntax`-disabled)；visual baselines unchanged (57.14 workflow run = 0 diffs — migrated components not visible in the loading-state snapshots)；0 backend changes；main bundle 297.89 kB byte-identical；calibration NEW class `frontend-refactor-mechanical` 0.50 1st app ratio ~1.7 over band → KEEP 0.50 (1 data point)）。Carryovers: AD-Inline-Style-Cleanup-Sweep-Round2 / AD-Lighthouse-Visual-Hard-Gate (baselines confirmed stable) / AD-Bundle-Size(降 optional) / 57.13 carryover 未動。Phase 57.16+ 候選 7 條 pending user 明確選定（見上方表格 Next Phase 候選 row）。
**main HEAD**: `7af59f42` (Sprint 57.15 via PR #137, 2026-05-11)
