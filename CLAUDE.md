...
| **Current Sprint** | **Sprint 57.133 PR-pending** (committed `feature/sprint-57-133-chatv2-inspector-token-sweep` from main `3e8ee330`; push/PR awaits user confirmation) — **chat-v2 Inspector Turn tab token-sweep** — **fully closes `AD-ChatV2-Inspector-Turn-Metadata-Wire`** (token-sweep = last of 3 legs; active_skill 57.120 + model 57.131 + this). The Inspector Turn tab showed `tokens.in/out/thinking` but not per-turn prompt-cache economics; the data was ALREADY on the wire (`llm_response.cached_input_tokens`, Sprint 57.65/57.108) → FE-only (3 src + 4 test EDIT, NO backend/wire/codegen/migration): `AgentTurn += cachedInputTokens: number\|null` + `turn_start` init + `llm_response` 0-guard capture (mirrors the 57.108 tokensIn) + 2 `InspectorTurn` KV rows — `tokens.cached` + a DERIVED per-turn `cache_hit` = round(cachedInputTokens / tokensIn × 100)% (no new store state; the wire's `loop_end.cache_hit_rate` is cumulative, this is per-turn). Docstring KV count 8→12 (57.120/131 drift). +4 Vitest. Vitest 915 (+4) · lint/build clean · mockup 51 byte-identical (0 new oklch) · tsc 0 · backend UNTOUCHED. **Drive-through PASS** (real chat-v2 UI + real Azure gpt-5.2, jamie@acme.com/acme-prod): turn-1 `tokens.cached —`/`cache_hit —` (first request, 0-guard honest "—" not "0"); turn-2 (re-sent ~2,435-tok prefix) `tokens.cached 2,048`/`cache_hit 83%` (real Azure prefix-cache hit; 2048/2458 = 83% self-consistent; turn-varying real values, not fixture). NEW class `chatv2-inspector-existing-field-surface` 0.85 **3rd data point** ratio ~0.94-1.03 IN band → KEEP 0.85. CHANGE-100, NO design note. Detail: `memory/project_phase57_133_chatv2_inspector_token_sweep.md`. |
...
**Last Updated**: 2026-06-17 (Sprint 57.133 — chat-v2 Inspector Turn tab token-sweep: surfaces per-turn `cached_input_tokens` + a derived `cache_hit` (already-on-wire field, 57.65/57.108) in the Inspector Turn tab; FE-only 3 src + 4 test, +4 Vitest; fully closes `AD-ChatV2-Inspector-Turn-Metadata-Wire` 3/3 legs; drive-through PASS with a real Azure cache hit turn-2 `tokens.cached 2,048`/`cache_hit 83%`; CHANGE-100); see `memory/` for sprint history
