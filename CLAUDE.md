...
| **Current Sprint** | **Sprint 57.152 — combine post-send extract + summarize into one LLM call — local commit done, PR push PENDING USER CONFIRMATION** (branch `feature/sprint-57-152-memory-combined-formation` from main `a6e8d586`). Closes `AD-Memory-Formation-Combine-Extract-Summarize` (a memory-formation arc efficiency follow-on; the arc 57.148→151 was already closed). The chat post-send hook loaded the ledger ONCE but made TWO cheap-tier LLM calls over the SAME conversation (57.149 `MemoryExtractor` facts→UserLayer + 57.151 `SessionSummarizer` summary→memory_session_summary) = ~2× background token/latency per send. Fix: a NEW `MemoryFormationWorker` (`agent_harness/memory/formation.py`) that **COMPOSES** the two existing workers (NOT replace — `SessionSummarizer`'s only non-test caller was the chat path = AP-2/AP-4 orphan risk) and by default makes ONE combined `chat()` call returning both the facts array AND the summary object, dispatching each half to the SAME write code via extracted behavior-preserving `MemoryExtractor.write_facts` + `SessionSummarizer.store_summary`; `_form_separate` (the `chat_memory_combined_formation=false` fallback) delegates to the two full single-call methods → both stay live on the chat path. One flag `chat_memory_combined_formation` (default ON) = a one-env-var revert to the proven two-call path. Wiring: ctx `{extractor,summarizer}`→`{former}`; `_maybe_auto_extract` calls `former.form()` once (profile() gated on `former.wants_user_facts`). AP-8 allowlist += formation.py. Backend-only, NO migration/wire(26)/frontend/loop.py. parent-direct agent_factor 1.0. **Drive-through STRONG PASS** (real chat-v2 + Azure gpt-5.2): one send ("I'm Marcus … Redis→DynamoDB session store … write-through cache … TTL vs explicit invalidation") → DB shows a `memory_session_summary` row (3 cols) AND `memory_user [auto_extract]` rows written at the SAME `15:20:30` timestamp = ONE combined call (unit test proves `chat_call_count==1`); 57.150 dedup coexists. Leg-2 recall (read path unchanged this sprint): both halves trace-injected into the new session; final answer hit 2 PRE-EXISTING carryovers (cross-sprint dan identity conflict + `AD-Verification-Judge-Memory-Inject-Blind`) — NOT 57.152 regressions. pytest 3053 (+11) · mypy 0/397 · run_all 11/11 · flake8/llm_sdk_leak clean · FE untouched. NEW calib `memory-formation-combine-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-119 + design note 55. (Prior: Sprint 57.151 MERGED PR #356, main `496d8dcf`.) Detail: `memory/project_phase57_152_memory_combined_formation.md`. |
...
**Last Updated**: 2026-06-30 (Sprint 57.152 — combine post-send extract + summarize into ONE LLM call: a `MemoryFormationWorker` COMPOSES the 57.149 extractor + 57.151 summarizer and (default) makes one combined cheap-tier `chat()` returning both the facts array + the summary object, dispatching each half to the SAME write code via extracted behavior-preserving `write_facts`/`store_summary`; `_form_separate` env-fallback (`chat_memory_combined_formation=false`) delegates to the two full single-call methods → both stay live (no orphaning); halves background formation token+latency when both formation features on; backend-only NO migration/wire/frontend; parent-direct; drive-through STRONG PASS real chat-v2 + Azure gpt-5.2 — one send wrote both a `memory_session_summary` row AND `memory_user [auto_extract]` rows at the SAME timestamp = one call (unit test call-count==1), 57.150 dedup coexists; Leg-2 recall both halves trace-injected, final answer hit 2 pre-existing carryovers not 57.152 regressions; pytest 3053 (+11) · mypy 0/397 · run_all 11/11; CHANGE-119 + design note 55; local commit done, PR push PENDING USER CONFIRMATION, closes `AD-Memory-Formation-Combine-Extract-Summarize`); see `memory/` for sprint history
