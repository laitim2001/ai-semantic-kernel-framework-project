...
| **Current Sprint** | Sprint 57.97 (PR pending) — **Multi-model profile (cheap tier for verification)**: a thin provider-neutral `ModelProfile{action, cheap}` value object pairs two pre-built `ChatClient`s by role, routing the per-request verification (Cat 10 llm_judge) onto a cheaper Azure deployment (gpt-5.4-mini) while the user-facing main turn + compaction keep the strong tier (gpt-5.2). The `ChatClient` ABC fixes model at construction (no per-call `model=`) → the seam is construction-time DI → **`loop.py` diff = 0, no ABC/event/DB change**. Unset cheap env → `cheap is action` (byte-identical). **Drive-through PASS** (real UI + Azure: cost_ledger sub_type proves verification ran on gpt-5.4-mini, ~62% cheaper than gpt-5.2, answer quality unchanged). mypy 0/353 + pytest **2291** (+8) + run_all 10/10 (SDK-leak 0). Risk Class E reinforced (D-DAY3-1 orphaned spawn-worker). Detail: `memory/project_phase57_97_multi_model_profile.md` + CHANGE-064 + design note 24. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-10 (Sprint 57.97 — Multi-model profile: thin ModelProfile{action, cheap} routes verification/llm_judge to a cheap Azure tier (gpt-5.4-mini, ~62% cheaper) while the main turn stays strong; loop.py diff=0; drive-through PASS; CHANGE-064 + design note 24); see `memory/` for sprint history
