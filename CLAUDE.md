...
| **Current Sprint** | **Sprint 57.148 — memory-formation Slice 1 (user-identity write + always-on inject)** — Day 0-4 done, **PR-pending (NOT pushed)**; branch `feature/sprint-57-148-memory-formation-identity` from main `3d5a1360`. Fixes the live-drive-through gap (new session → "我不知道你是誰"): a system-prompt **formation nudge** → agent proactively `memory_write(scope=user)`, + an **always-on user-profile inject** (`MemoryRetrieval.profile()` merged into `DefaultPromptBuilder.build()`) surfacing user-scope long_term facts EVERY turn regardless of message keywords (bypasses the ILIKE query-gating that caused the bug). Backend-only, NO migration/wire(26)/frontend; `user_id=None` byte-identical. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2): jamie states identity → 2 `memory_user` rows proactively written → NEW session "你知道我是誰嗎?" (0 keyword overlap) recalls "你是 Chris…Knowledge Connector" → priya (diff user) "我不知道你是誰" (per-user isolation). pytest 2999 · mypy 0/392 · run_all 11/11 · FE untouched. NEW calib `memory-formation-identity-spike` 0.60 (1st pt ~1.0-1.1 IN band, parent-direct). CHANGE-115 + design note 52. Closes `AD-Memory-Formation-Identity` Slice 1 (Option-B extraction / upsert-by-key / cross-session conversation recall / semantic axis open). (Prior: Sprint 57.147 MERGED PR #348, main `ba8d07e1`.) Detail: `memory/project_phase57_148_memory_formation_identity.md`. |
...
**Last Updated**: 2026-06-27 (Sprint 57.148 — memory-formation Slice 1: user-identity write (system-prompt nudge → proactive `memory_write(scope=user)`) + always-on `profile()` inject (keyword-independent, bypasses ILIKE query-gating); drive-through PASS — cross-session identity recall + per-user isolation on real chat-v2 + real Azure gpt-5.2; CHANGE-115 + design note 52; PR-pending NOT pushed); see `memory/` for sprint history
