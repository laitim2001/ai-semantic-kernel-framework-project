...
| **Current Sprint** | **Sprint 57.159 — compaction L2→L3 drive-through + Inspector timeline marker — PR-pending** (branch `feature/sprint-57-159-compaction-drivethrough-inspector` from main `8eb3d261`; engine-debt program, compaction range). Raises **Cat 4 context compaction L2 → L3** (reality-audit §3 gap) + fills the deferred 57.66 "A-5c". The `context_compacted` event was fully wired loop→sse→wire→store but the chat-v2 store left it **rawEvents-only** → the token reduction rendered nowhere. **FE-only surface, ZERO backend/wire/codegen**: `+CompactionMarkerTurn` in the `Turn` union + a `context_compacted` store case pushing a persistent chat-v2 timeline marker (mirrors the `message_injected` pseudo-turn; mockup `.badge.warning` chip, compaction==warning) + `TurnList` dispatch + 2 Vitest; i18n dropped (surrounding turn components use English literals). parent-direct agent_factor 1.0. **Drive-through PASS** real chat-v2 + Azure gpt-5.2: the marker renders LIVE (no-op `4,086→4,086` … `35,144→35,144` on a long tool-using send + a REAL `4,604 → 1,770 tokens (hybrid · 8 msgs)` −62% at `keep_recent=1`) + context retained after compaction (Aurora/Oracle→Postgres/NUMBER(38,4) 0.99 + Beacon/October/Lodestar 0.99). **L2→L3 finding**: compaction TRIGGERS every over-budget turn but **0-REDUCES on the single-user-turn chat path** (`len(user_indices) > keep_recent` unreachable → context grows 4k→35k unbounded); NOT a correctness bug (retention 0.99) → carryover `AD-Compaction-NoOp-On-Single-User-Turn-Chat-Path` + `AD-Compaction-Marker-Inspector-Trace-Correlate`. Vitest 927 (+2) · mockup 51 byte-identical · run_all 11/11 · mypy `src` 400 (backend UNTOUCHED). NEW calib `chatv2-compaction-drivethrough-surface` 0.85 (1st pt ~1.06 IN band). CHANGE-126, no design note. (Prior: Sprint 57.158 memory recall A/B PR #372 merged, main `c709c3fe`.) Detail: `memory/project_phase57_159_compaction_drivethrough_marker.md`. |
...
**Last Updated**: 2026-07-07 (Sprint 57.159 — compaction L2→L3 drive-through + chat-v2 Inspector timeline marker: raises Cat 4 context compaction L2→L3 (reality-audit §3) + fills the deferred 57.66 A-5c — the `context_compacted` event was fully wired loop→sse→wire→store but left rawEvents-only so the token reduction rendered nowhere. FE-only surface (ZERO backend/wire/codegen): `+CompactionMarkerTurn` in the `Turn` union + a `context_compacted` store case pushing a persistent chat-v2 timeline marker (mirrors `message_injected`; `.badge.warning` chip) + `TurnList` dispatch + 2 Vitest; i18n dropped (surrounding turn components use English literals). **Drive-through PASS** real chat-v2 + Azure gpt-5.2: marker renders live (no-op `4,086→4,086`…`35,144→35,144` + a REAL `4,604→1,770 (hybrid · 8 msgs)` −62% at keep_recent=1) + context retained after compaction (0.99 ×2). **L2→L3 finding**: compaction TRIGGERS every over-budget turn but 0-REDUCES on the single-user-turn chat path (`user_indices > keep_recent` unreachable → context grows 4k→35k); NOT a correctness bug → carryover `AD-Compaction-NoOp-On-Single-User-Turn-Chat-Path` + `AD-Compaction-Marker-Inspector-Trace-Correlate`. Vitest 927 (+2) · mockup 51 byte-identical · run_all 11/11 · mypy `src` 400 (backend UNTOUCHED). NEW calib `chatv2-compaction-drivethrough-surface` 0.85 (1st pt ~1.06 IN band). CHANGE-126, no design note; **PR-pending** (branch from main `8eb3d261`, engine-debt program compaction range); prior Sprint 57.158 memory recall A/B PR #372 merged main `c709c3fe`; see `memory/` for sprint history
