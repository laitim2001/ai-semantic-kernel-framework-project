...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.132 MERGED** (PR #310, main `f2e243a1`) — **chat-v2 resume-path ledger persistence** (closes `AD-ChatV2-Resume-Tool-RoundTrips` 57.129 carryover + the sibling held-answer gap; user picked the comprehensive scope). The HITL **resume path** appended its out-of-loop messages to the buffer but never persisted them to the `messages` Cat-3 ledger (the 57.129 send-path persist lives INSIDE `_run_turns`), so a follow-up after a HITL resume rehydrated an incomplete conversation. 2 `loop.py` persist call sites: **Leg-1** `resume()` tool-kind APPROVED persists the paused-then-approved tool's `[assistant tool_use, *tool results]` round-trip (backward-scan, mirrors the 57.129 atomic batch); **Leg-2** `_replay_approved_output` persists the delivered held answer (output+verification APPROVE). PURE backend (1 src + 1 test EDIT, +4 unit tests), NO new ABC/event/wire 24/codegen/migration/frontend. mypy 0/372 · run_all 10/10 · pytest 2731 passed/5 skip (+4) · Vitest/mockup UNTOUCHED. **Leg-1 drive-through PASS** (real chat-v2 UI + real Azure gpt-5.2, session 02fa6bfb: python_sandbox MEDIUM → HITL pause → Approve & continue → resume → DB `messages` ledger seq-2 assistant tool_use + seq-3 tool result persisted AT RESUME; follow-up "0.031" from `duration_seconds` un-deducible → rehydration proven). **Leg-2 unit + composition verified, NOT UI-driven** (the real-LLM output-escalate trigger is non-deterministic / content-filter-prone — honest gate-only label; carryover `AD-ChatV2-Resume-Replay-Drive-Through`). CHANGE-099, NO design note. Detail: `memory/project_phase57_132_chatv2_resume_ledger_persist.md`. |
...
**Last Updated**: 2026-06-17 (Sprint 57.132 — chat-v2 resume-path ledger persistence: 2 `loop.py` persist call sites closing `AD-ChatV2-Resume-Tool-RoundTrips` + the sibling held-answer gap — Leg-1 `resume()` persists a paused-then-approved tool's round-trip, Leg-2 `_replay_approved_output` persists the held answer; PURE backend +4 unit tests; Leg-1 drive-through PASS (DB ledger seq-2/3 persisted AT RESUME + follow-up "0.031" rehydration), Leg-2 unit+gate verified honest; CHANGE-099); see `memory/` for sprint history
