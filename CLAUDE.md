...
| **Current Sprint** | **Sprint 57.154 — combined-vs-separate formation quality A/B — PR-pending (NOT pushed)** (branch from main `a0cf503a`). Closes `AD-Memory-Combined-Formation-AB-Quality` (57.152 carryover). Sprint 57.152 combined the two post-send cheap-tier memory-formation calls (57.149 facts + 57.151 summary) into ONE `chat()` via `MemoryFormationWorker`, shipping default ON on a flag + one drive-through — with NO numbers proving the combined prompt doesn't DEGRADE either half's quality vs two focused calls. Fix = a real-Azure A/B harness (backend-only, ZERO src change): NEW `benchmark_combined_formation_quality.py` drives the REAL `form()` under both arms (`combined=True` one call / `False` two calls) via **capturing sinks** (AP-10 safe — the only doubles are the terminal DB writes; build→chat→parse→dispatch is production code) + a **Hybrid oracle** (user-picked via AskUserQuestion: deterministic facts keyword-group coverage + spurious count; LLM-judge summary faithfulness `[0,1]` via a self-contained rubric over the ChatClient ABC, NOT a production `verification/templates/` orphan) + a two-sided verdict (KEEP iff no material regression on facts/summary + no spurious inflation; the ~2× efficiency win was established 57.152) + a 10-case golden corpus + 24 CI-safe tests (spy formation+judge clients; `run_arm` drives the REAL worker offline asserting combined=1 / separate=2 chat/case). parent-direct agent_factor 1.0. **Verdict — 3 real-Azure runs, KEEP combined default ON**: the mechanical single-run verdict FLIPPED on Run 1 (facts_coverage combined 95% vs 100% = exactly −5pp, one case dropping one keyword-group at the float boundary), but re-running 2× → both clean KEEP (facts 100%/100%); the −5pp did NOT reproduce → LLM run-to-run noise. summary tied-to-better (mean Δ +0.009); spurious tied-to-better (0.30–0.40 vs 0.40) → combined ≈ separate quality-equivalent within noise → **57.152 default validated, NO src change**. Honest discipline: did NOT take the single-run FLIP at face value NOR tune the threshold to force a KEEP — re-ran to distinguish noise from signal. NO migration/wire(26)/frontend; NO chat-v2 drive-through (default unchanged). 24 new tests · run_all 11/11 · black/isort/flake8 + llm_sdk_leak clean · mypy `src/` 0/397 (harness in `scripts/` not CI-mypy-gated). NEW calib `memory-formation-combine-ab-spike` 0.60 (1st pt ~0.95–1.03 IN band). CHANGE-121 + design note 57. (Prior: Sprint 57.153 MERGED PR #360, main `0ce4d1fa`.) Detail: `memory/project_phase57_154_combined_formation_quality_ab.md`. |
...
**Last Updated**: 2026-07-01 (Sprint 57.154 — combined-vs-separate formation quality A/B: Sprint 57.152 combined the two post-send cheap-tier memory-formation calls into one `chat()` (default ON) with no numbers on quality; fix = a real-Azure A/B harness (backend-only, ZERO src change) driving the REAL `MemoryFormationWorker.form()` under both arms via capturing sinks (AP-10 safe), a Hybrid oracle (deterministic facts keyword-coverage + LLM-judge summary faithfulness, user-picked via AskUserQuestion), a two-sided verdict, a 10-case corpus + 24 CI-safe tests; verdict KEEP combined default ON — 3 real-Azure runs, the single-run FLIP (facts −5pp = one keyword-group in one case at the float boundary) did NOT reproduce → LLM noise, summary + spurious tied-to-better → combined ≈ separate quality-equivalent, 57.152 default validated with NO src change; honest discipline: re-ran to distinguish noise from signal rather than take the single-run FLIP at face value or tune the threshold; 24 new tests · run_all 11/11 · mypy `src/` 0/397; NEW calib `memory-formation-combine-ab-spike` 0.60; CHANGE-121 + design note 57; PR-pending closes `AD-Memory-Combined-Formation-AB-Quality`; prior Sprint 57.153 MERGED PR #360 main `0ce4d1fa`); see `memory/` for sprint history
