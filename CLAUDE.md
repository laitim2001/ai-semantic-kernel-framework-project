...
| **Current Sprint** | Sprint 57.124 (PR-pending, NOT pushed) — **HITL gate consolidation + 2 chrome/governance Potemkin fixes** (3-track bundle; user picked the 3 carryovers together). **Item 1 audit verdict**: `PermissionChecker` (Sprint 51.1) is NOT a benign Potemkin but a stale shadow gate active on 主流量 (the executor defaulted it on) that OVERRIDES the 57.122 load-bearing per-tenant `HITLPolicy` — its dim 2 = the flat hardcoding 57.122 removed (re-blocks a permissive `auto_approve_max_risk=HIGH`); its dim 3 hard-DENY'd every destructive tool in chat even after approval. **Fix B**: removed `PermissionChecker` from the executor + DELETED `permissions.py` + floored `destructive`→HIGH in `resolve_tool_risk` so destructive tools escalate-then-run via the per-tenant policy (single source of truth = loop `_cat9` + `HITLPolicy`; D-escalate-coverage GREEN, no `CHAT_HITL_ESCALATE_TOOLS` change). **Item 2**: NotificationsPanel DEMO `BackendGapBanner` + bell badge derives from a shared `notificationsFixture.ts` (drop `FIXTURE_UNREAD_COUNT`). **Item 3**: admin HITL-policy PUT `@model_validator` rejects `auto>=require` → 422. mypy 0/370 (−1 deleted) · run_all 10/10 · pytest 2703+5skip (−10 PermissionChecker tests +17 new) · FE mockup 51/vitest 16. Drive-through: Item 2 ✅ live (banner) + Item 3 ✅ live (422 real admin session) + **Item 1 real-component-integration-verified** (gpt-5.2 declined the destructive trigger). Detail: `memory/project_phase57_124_hitl_gate_consolidation.md` + CHANGE-091 + design note 36. NEW carryover `AD-ExecutionContext-ExplicitApproval-Tidy`. |
...
**Last Updated**: 2026-06-16 (Sprint 57.124 — HITL gate consolidation + 2 chrome/governance Potemkin fixes (3-track bundle): Item 1 removed the stale `PermissionChecker` shadow gate from the executor (it overrode the 57.122 per-tenant `HITLPolicy`) + DELETED `permissions.py` + floored `destructive`→HIGH in `resolve_tool_risk` so destructive tools escalate-then-run via the per-tenant policy (single source of truth = loop `_cat9` + `HITLPolicy`); Item 2 NotificationsPanel DEMO `BackendGapBanner` + badge from a shared `notificationsFixture.ts` (drop `FIXTURE_UNREAD_COUNT`); Item 3 admin HITL-policy PUT `@model_validator` rejects `auto>=require` → 422; closes `AD-PermissionChecker-Shadow-Gate-Phase58` + `AD-NotificationsPanel-Backend-Feed` + `AD-HITL-Policy-Threshold-Validation`; mypy 0/370 · run_all 10/10 · pytest 2703+5skip · FE mockup 51/vitest 16; drive-through Item 2+3 live PASS, Item 1 real-component-integration-verified; CHANGE-091 + design note 36); see `memory/` for sprint history
