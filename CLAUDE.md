...
**Last Updated**: 2026-07-07 (Sprint 57.161 — structural compactor real token re-count: closes `AD-Compaction-Structural-RealTokenCount` (57.160 carryover). `StructuralCompactor.tokens_after` was a message-count ratio (`structural.py:192-193`) blind to in-place tombstoning → `N→N` on masking even when real tokens dropped, AND `loop.py:2282` trusts `tokens_after` as the ongoing budget (no re-count; the "Loop.run() will re-count" comment was STALE) → the 57.159 `4k→35k / 8-no-op` pathology. Fix injects a `TokenCounter` into `StructuralCompactor` mirroring `PreClearCompactor` (`preclear.py:178-181`) so tool-anchored masking surfaces its reduction WITHOUT preclear; `token_counter=None` fallback = byte-identical. Rollout A default-on (factory injects `TiktokenCounter`; AskUserQuestion pick). Backend-only, ZERO wire/codegen/frontend/migration. parent-direct agent_factor 1.0. **Drive-through STRONG PASS** real chat-v2 + Azure gpt-5.2 + 6× knowledge_search in ONE user turn (preclear OFF): the 57.159 marker renders REAL `22,925→10,584 (−54%)` + `21,754→13,650 (−37%)` where 57.160 Leg-1/2 rendered `N→N`; context bounded ~10-22k; BOREALIS-9 retained; run ended max_turns=8. Closes mechanism half of `AD-Compaction-ToolAnchored-Preclear-Phase58`. pytest 3206 (+4) · mypy `src` 400 · run_all 11/11 · Vitest 927 / mockup 51 (FE untouched). NEW calib `compaction-structural-realcount-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-128 + design note 63; carryover `AD-Compaction-Structural-TombstoneCount-Marker`; commit `d20191f9` ⏳ **PR-pending** (branch from main `204c4499`, engine-debt program compaction range); prior Sprint 57.160 tool-anchored masking PR #376 merged main `ab9b4cf5`; see `memory/` for sprint history
