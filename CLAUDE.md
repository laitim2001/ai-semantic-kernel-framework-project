...
| **Current Sprint** | Sprint 57.76 (PR pending) — **Memory ops-history backend (closes `AD-Memory-OpsHistory-Backend` backend half; Area-A remaining item)**: NEW append-only `memory_ops` table (Option B) + Alembic `0024` (RLS 2-policy + FORCE verbatim-mirror 0023) + `MemoryOp` ORM + user/tenant layer write/evict emit (**same-txn, Risk-C atomicity tested**; evict SELECT-before-DELETE for value snapshot) + `GET /memory/ops` (tenant-scoped + require_audit_role + cursor pagination, mirrors /matrix deps). **Backend-only**; frontend (RecentOps + TimeTravel wiring) → Sprint 57.77 (backend-then-FE split). role/system raise → no emit (honest, not Potemkin; D1 corrected researcher misreport). Agent-delegated (Track A backend) + parent re-verify. Detail: `memory/project_phase57_76_memory_ops_history_backend.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-04 (Sprint 57.76 — Memory ops-history backend, closes `AD-Memory-OpsHistory-Backend` backend half: NEW append-only `memory_ops` table (Option B) + Alembic 0024 (RLS 2-policy + FORCE mirror 0023) + user/tenant write/evict emit (same-txn, SELECT-before-DELETE, Risk-C atomicity tested) + `GET /memory/ops` (cursor pagination); backend-only, frontend → 57.77; role/system raise → no emit (D1 corrected researcher misreport); backend mypy 0/331 + pytest 2105 + run_all 10/10 (check_rls_policies + check_llm_sdk_leak); CHANGE-044; PR pending); see `memory/` for sprint history
