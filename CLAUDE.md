...
| **Current Sprint** | **Sprint 57.142 PR-pending** (branch `feature/sprint-57-142-otel-genai-semconv` from main `3e9d069e`) — **OTel GenAI semantic-conventions span/attr mapping (research #5)** — closes research #5 `AD-Observability-OTel-GenAI-Schema` (eval `otel-genai-schema-thin-spike-eval-20260625.md`; canonical-next after #6/#3/#8/#4/#1/#2, all CLOSED). **Key reframe**: the OTel SDK is ALREADY real (`opentelemetry-*==1.22.0` + OTLP/Prometheus), so the gap is ONLY the bespoke span/attr SCHEMA → a translation-at-tracer layer (NOT an OTel adoption). NEW `_genai_semconv.py` (pure: stop_reason→finish_reason · span_type→operation · attr renames · `to_genai_span`/`assert_genai_conformant`) applied in `OTelTracer._span_cm` at start+close; the close-time `set_attributes` re-read **fixes a discovered latent bug** (post-response token attrs never reached real exported spans). 1-line `loop.py` (`finish_reason`, decision B2); dual-view (internal/test bespoke, exported `gen_ai.*`) → loop.py + existing obs tests untouched. NEW `benchmark_otel_conformance.py` (real `InMemorySpanExporter`) + corpus + 18 CI tests. pytest 2877+5skip (1 pre-existing flake, isolation-passes) · mypy 0/381 · llm_sdk_leak 3/3 · NO migration / wire stays 26 / NO frontend / NO 17.md contract. **Drive-through (real Azure gpt-5.2)**: production-path `chat gpt-5.2` span carries `gen_ai.operation.name` + `gen_ai.request.model` + `gen_ai.usage.input/output_tokens` (**latent bug FIXED on real path**) + `gen_ai.response.finish_reasons`; GenAI 2/2 conformant, ratio 100%. NEW class `otel-genai-semconv-spike` 0.60 (1st pt ~0.95-1.0 IN band, parent-direct). CHANGE-109 + design note 46. content-capture + `gen_ai.provider.name` deferred (Phase58 ADs). (Prior: Sprint 57.141 pass^k MERGED PR #335.) Detail: `memory/project_phase57_142_otel_genai_semconv.md`. |
...
**Last Updated**: 2026-06-25 (Sprint 57.142 — OTel GenAI semantic-conventions span/attr mapping (research #5): translation-at-tracer layer mapping V2's bespoke OTel span names + attr keys to CNCF `gen_ai.*`. Reframe: the OTel SDK is ALREADY real (`opentelemetry-*==1.22.0`), gap is ONLY the schema → NOT an adoption. NEW `_genai_semconv.py` (pure mapping) applied in `OTelTracer._span_cm` start+close; the close-time set_attributes re-read FIXES a latent bug (post-response token attrs never reached real exported spans); 1-line loop.py finish_reason (B2); dual-view (internal bespoke / exported gen_ai) → loop.py + existing obs tests untouched; NEW `benchmark_otel_conformance.py` real InMemorySpanExporter harness + corpus + 18 CI tests; pytest 2877+5skip · mypy 0/381 · llm_sdk_leak 3/3 · NO migration/wire-26/frontend/17.md-contract; **drive-through real Azure gpt-5.2: production-path `chat gpt-5.2` span carries gen_ai.operation.name + request.model + usage.input/output_tokens (latent bug FIXED on real path) + response.finish_reasons; GenAI 2/2 conformant 100%**; NEW calib class `otel-genai-semconv-spike` 0.60 (1st pt ~0.95-1.0 IN band, parent-direct); CHANGE-109 + design note 46; content-capture + provider.name deferred Phase58); see `memory/` for sprint history
