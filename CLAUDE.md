...
| **Current Sprint** | Sprint 57.61 closed 2026-05-29 — **RateLimits SyntaxValidation** closes `AD-RateLimits-SyntaxValidation-Phase58`: PUT-time 422 (per-item reason) replaces silent drop — NEW `is_recognized_rate_limit_value` predicate (reuses `_VALUE_RE`+`_WINDOW_ALIASES`, only new `_CONCURRENCY_RE`; no 4th rate-regex) + `field_validator("items")` on `RateLimitsUpsertRequest` (NOT shared `RateLimitItem`) accepts enforceable rate + display-only `N concurrent`, rejects garbage/unsupported-window/non-positive/empty; US-2 parser-consistency guard (store⟺counter validity). NO schema change → frontend untouched. single agent 27th consecutive; pytest 1848→1887 (+39: 16 integration + 23 unit) / 9/9 V2 lints / 0 frontend. Calibration: `mechanical-greenfield-design-decisions` 0.65 3rd validation **1st BACKEND-ONLY** ~0.74 BELOW band → KEEP single-data-point caution (counterfactual `-port-style` 0.45 → ~1.06 IN band; NEW `AD-AgentFactor-DesignDecisions-BackendOnly-Variant-Watch`); `medium-backend` 0.80 12th data point ~0.48 confound-resolved KEEP. DUAL CLEAN 22/22 PARITY **17 consecutive 57.45-57.61**. Detail: `memory/project_phase57_61_rate_limits_syntax_validation.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-05-29 (Sprint 57.61 — RateLimits SyntaxValidation; PUT-time 422 per-item reason replaces silent drop — NEW `is_recognized_rate_limit_value` predicate + `field_validator("items")` on `RateLimitsUpsertRequest` (NOT shared `RateLimitItem`) accepts enforceable rate + display-only `N concurrent` + US-2 parser-consistency guard **closes `AD-RateLimits-SyntaxValidation-Phase58`**; no schema change → frontend untouched; `mechanical-greenfield-design-decisions` 0.65 3rd validation 1st BACKEND-ONLY ~0.74 BELOW band KEEP single-data-point caution (counterfactual `-port-style` 0.45 → ~1.06 IN band); `medium-backend` 0.80 12th ~0.48 KEEP; pytest 1848→1887 (+39); DUAL CLEAN 17 consec 57.45-57.61; CHANGE-029; commits `6bf23e63`+`093a161d` Day 0+1 + Day 2 pending); see `memory/` for sprint history
