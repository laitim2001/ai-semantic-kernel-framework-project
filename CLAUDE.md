...
> **權威排序**：`agent-harness-planning/` 21 份 V2 核心規劃文件（20 規劃 + 1 review）> 本 CLAUDE.md > 任何 V1 文件 / 既有代碼。
> 衝突時以 V2 規劃為準。
...
| **Current Sprint** | **Sprint 57.157 — Scheduler: cross-burst auto-continue — MERGED** (PR #370, main `65157788`, CI all-green; branch from main `aa0dcf13`). Closes `AD-TaskPrimitive-Scheduler-Phase58` (research #3; the **engine-debt program kickoff** — user-picked phasing: clear the 7 engine-internal ranges (memory/compaction/tool/loop-中斷/task-primitive/verification/observability) FIRST, defer knowledge+business grounding LAST; kickoff = this scheduler, breaking the `max_turns=8` short-life ceiling). **Guidance not enforcement**: when a chat `real_llm` burst ends at `max_turns=8` with an unfinished `write_todos` plan, the SSE generator auto-continues the loop for the next bounded burst (driven by a `CONTINUATION_NUDGE` user turn) instead of forcing a manual "continue" — the fix for the reality-audit §9 long-lived-agent 5% ceiling. NEW `orchestrator_loop/scheduler.py` pure `should_continue_plan()` 4-condition AND predicate + a `_scheduled_loop_events` chaining generator in `router.py` yielding `(event, swallow)` (existing `async for` gains 3 `if not _swallow` guards) + 2 default-OFF settings (`chat_scheduler_auto_continue`=False, `chat_scheduler_max_bursts`=3); **billing every burst / quota·SLA·audit·handoff final-burst-only** (Day-0 MAJOR drift `D-router-loopcompleted-shape` — repeated quota reconcile of the same reservation would over-release). Rode the 57.127 ledger + 57.156 `## Active Plan` rehydration serde-transparently → **NO migration / wire schema(26) / codegen / frontend / loop.py change** (backend delta 3 files: `scheduler.py`+`router.py`+`core/config`); parent-direct agent_factor 1.0. **Drive-through STRONG PASS** both legs (real chat-v2 + Azure gpt-5.2): ON = 1 POST → 15 gpt-5.2 turns / 2 bursts / one continuous stream (LOOP 14 turns), the `messages` ledger shows the scheduler-injected `CONTINUATION_NUDGE` user turn between bursts (DB count 1), Todos 12/12 completed with NO manual continue, Verification 0.98, billing `materialized=15`; OFF (control) = same prompt → 8 completions / 1 burst, `stop_reason=max_turns`, plan 7/12 unfinished, no nudge, billing `materialized=8` (byte-identical single-burst). 18 new tests · mypy `src` 400/0 · run_all 11/11 · 850 chat regression OFF byte-identical. NEW calib `scheduler-cross-burst-spike` 0.60 (1st pt ~1.15-1.3 near/slightly-over band top; drive-through env-resolution the variance driver). CHANGE-124 + design note 60. (Prior: Sprint 57.156 MERGED PR #366, main `78c8ff8c`.) Detail: `memory/project_phase57_157_scheduler_cross_burst.md`. |
...
### Agent Loop 能力現況（drive-through 驗證，2026-06-18；capability 清單更新至 Sprint 57.156）

chat-v2 主流量上，下列 agent loop 子能力已**實機 drive-through 驗證**（真 UI + 真後端 + 真 LLM，非僅 gate/curl）：工具執行（python_sandbox via Docker）/ subagent 任務分解（fork）/ verification self-correction / HITL pause-resume（tool-ESCALATE）/ **output-ESCALATE 投遞前暫停**（57.93）/ **verification-ESCALATE**（57.99 A2；APPROVE 照原樣投遞 + REJECT 一次教練重試）/ handoff / mid-run injection / context compaction 實際壓縮 / **顯式 task primitive**（`write_todos` Sprint 57.140，依賴 DAG 57.156，**跨-burst 自動續跑 scheduler 57.157 default-OFF**）/ **knowledge connector**（`knowledge_search` 真外部檔 + embedding/Qdrant 語意檢索，57.145-147）/ **memory formation + 語意 recall**（identity / auto-extract / session-recall / L4 vector，57.148-155）。皆已正確接線、非 Potemkin。output-ESCALATE 為確定性觸發；verification-ESCALATE 依賴 real-LLM 連 3 次 judge 失敗，非 100% 確定（與 57.132 retro 一致）。

**長運行的真實邊界（架構事實，非缺陷）**：chat real_llm loop 單次 send 上限 **`max_turns=8`**（不是 CC 式單次無界 run）。「長時間運行」靠**多次有界爆發 + 跨輪 rehydration（Cat 3 memory，Sprint 57.127）+ compaction**達成；單次 send 內若 `write_todos` plan 未完，**cross-burst scheduler（Sprint 57.157，default OFF，`CHAT_SCHEDULER_AUTO_CONTINUE`）可自動續跑下一 burst**（bounded by `max_bursts`，每 burst 仍守 `max_turns=8` 治理上限）；compaction 僅在 **≥3 user turns** 時實際壓縮（預設 75k 門檻正常聊天不觸發，env `CHAT_COMPACTION_TOKEN_BUDGET` 為 drive-through lever）。任務為本靠「對話式 loop + 工具 + subagent 分解 + CC 式 `write_todos` 顯式 task primitive（Sprint 57.140；57.156 演進為 `depends_on` 依賴 DAG，render ready/blocked 為 guidance 不強制）」。屬有意的伺服器端可治理設計。
...
> **註**：同目錄 `agent-harness-planning/` 另含 Sprint 57.x 起「thin spike → extract」產出的 spike design notes（`18-*.md` … `59-*.md`，隨 sprint 增長），**非** 21 份核心規劃的一部分。下表 21 份為最高權威 canonical set；design notes 是 per-slice 抽取紀錄。
...
建 `docs/03-implementation/agent-harness-execution/phase-XX/sprint-XX-Y/progress.md`。
...
**Last Updated**: 2026-07-06 (Sprint 57.157 — Scheduler: cross-burst auto-continue: when a chat `real_llm` burst ends at `max_turns=8` with an unfinished `write_todos` plan, the SSE generator auto-continues the loop for the next bounded burst (driven by a `CONTINUATION_NUDGE` user turn) instead of forcing a manual "continue" — **guidance not enforcement**, default OFF; NEW `orchestrator_loop/scheduler.py` pure `should_continue_plan()` 4-condition AND predicate + a `_scheduled_loop_events` chaining generator in `router.py` yielding `(event, swallow)` (existing `async for` gains 3 `if not _swallow` guards) + 2 default-OFF settings; billing every burst / quota·SLA·audit·handoff final-burst-only (Day-0 MAJOR drift `D-router-loopcompleted-shape`); rode 57.127 ledger + 57.156 `## Active Plan` rehydration serde-transparently → NO migration/wire(26)/codegen/frontend/loop.py change (backend delta 3 files); parent-direct agent_factor 1.0; drive-through STRONG PASS both legs (real chat-v2 + Azure gpt-5.2 — ON: 1 POST → 15 turns / 2 bursts / one stream, ledger shows the injected CONTINUATION_NUDGE between bursts, Todos 12/12 no manual continue, billing materialized=15; OFF: 8 turns / 1 burst, stop_reason=max_turns, plan 7/12, materialized=8, byte-identical); 18 new tests · mypy `src` 400/0 · run_all 11/11 · 850 chat regression OFF byte-identical; NEW calib `scheduler-cross-burst-spike` 0.60; CHANGE-124 + design note 60; **MERGED PR #370 main `65157788`** closes `AD-TaskPrimitive-Scheduler-Phase58` (engine-debt program kickoff — clear 7 engine-internal ranges first, defer knowledge+business last; branch from main `aa0dcf13`); prior Sprint 57.156 MERGED PR #366 main `78c8ff8c`; see `memory/` for sprint history
