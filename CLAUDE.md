...
| **Current Sprint** | **Sprint 57.135** (branch `feature/sprint-57-135-scheduled-transcript-retention`, from `main` `c98b6368`; ⏳ PR pending) — **scheduled transcript-retention background job** — closes 57.134 follow-on #1. A poll loop auto-enforces per-tenant retention: `run_transcript_retention_sweep` (enumerate all tenants → per-tenant `apply_transcript_retention` + audit + commit, fail-open) + `_transcript_retention_poll_loop`/`_start_*`/lifespan wiring mirroring the billing-outbox drainer; **DEFAULT OFF** (destructive opt-in via `TRANSCRIPT_RETENTION_JOB_ENABLED`). Backend-only, NO migration, +6 tests. mypy 0/374 · run_all 10/10 · pytest 2747+5skip (+6). **Drive-through PASS** (real backend PID 43144 + real DB; background job = runtime verification, no UI): startup sweep `tenants=5000 failed=0 messages_deleted=1`, throwaway tenant MESSAGES 2→1 + SCHEDULED_AUDITS=1, default-OFF re-confirmed. NEW class `scheduled-job-mirror-spike` 0.55→0.85 (ceremony-not-code-accelerated). CHANGE-102, no design note. (Branch commit 1 also closes stale `AD-Subagent-Child-Event-SSE-Relay` via a 2026-06-17 diagnostic — already-resolved/YAGNI.) Detail: `memory/project_phase57_135_scheduled_transcript_retention.md`. |
...
**Last Updated**: 2026-06-17 (Sprint 57.135 — scheduled transcript-retention background job: a poll loop auto-enforces per-tenant retention (`run_transcript_retention_sweep` + lifespan wiring mirroring the billing-outbox drainer), DEFAULT OFF (destructive opt-in); backend-only NO migration, +6 tests; drive-through PASS (real backend + real DB runtime verification: sweep deleted an expired transcript on a throwaway tenant, default-OFF re-confirmed); NEW calib class `scheduled-job-mirror-spike` 0.85; CHANGE-102; branch commit 1 also closed stale `AD-Subagent-Child-Event-SSE-Relay` (diagnostic: already-resolved/YAGNI)); see `memory/` for sprint history
