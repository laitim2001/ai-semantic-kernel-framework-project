...
| **Current Sprint** | **Sprint 57.164 — Tool-error taxonomy surfaced in chat-v2 UI — ✅ MERGED (PR #385, main `980b3357`)** (engine-debt **Tool range** 2nd slice). Closes ③3 `AD-Tool-Error-Taxonomy-UI` (57.144 carryover; 3 of ~4 Tool ADs done, ③2 autofix → 57.165). 3 parts (both user AskUserQuestion picks): (a) **Option B decouple** — `classify_tool_error` runs on EVERY failure, the `CHAT_TOOL_ERROR_REFLECTION` lever gates only the LLM `content` (agent byte-identical); (b) **wire** the taxonomy as an additive field on the EXISTING `tool_call_result` (count stays 26) → codegen → FE `.badge danger` chip; (c) **reachability fix** — emit `ToolCallFailed(+taxonomy)` before `LoopTerminated` at both terminate sites. **KEY drive-through finding**: the chip was WIRED-but-UNREACHABLE on the live 主流量 (tool failures `loop_terminated` at `loop.py:3113/3028` BEFORE the `ToolCallFailed` emit) — the canonical gate-green≠human-reachable catch → the (c) fix made it reachable → **drive-through PASS** real Azure gpt-5.2 (`get incident INC-99999` → mock 404 → ToolBlock `failed_api` chip + real error + `terminated` badge). pytest 3231 (+1) · Vitest 933 (+3) · mypy `src` 400 · v2 lints 11/11 · mockup 51 byte-identical · NO migration/new-wire-type/CSS. NEW calib note: `frontend-feature-with-event-wire-addition` 0.55, ratio ~1.4-1.55 OVER (drive-through-discovered mid-sprint loop.py extension; KEEP 0.55, single over-point). CHANGE-131, no design note. Carryover: ③2 autofix + `AD-Tool-{Taxonomy-HumanLabel,Failure-Terminate-vs-Recoverable-Policy}` + `AD-RequestApproval-Placeholder-Deprecated`. (Prior: Sprint 57.163 MERGED PR #383, main `08a93901`.) Detail: `memory/project_phase57_164_tool_taxonomy_ui.md`. |
...
**Last Updated**: 2026-07-10 (Sprint 57.164 — Tool-error taxonomy surfaced in chat-v2 UI: engine-debt **Tool range** 2nd slice, closes ③3 `AD-Tool-Error-Taxonomy-UI`. Option B decouple (classify always; lever gates only LLM content) + additive `error_taxonomy` on the existing `tool_call_result` (count 26) + FE `.badge danger` chip. KEY drive-through finding: chip WIRED-but-UNREACHABLE on the live 主流量 (tool failures `loop_terminated` before the `ToolCallFailed` emit) → fixed by emitting `ToolCallFailed(+taxonomy)` before terminate → drive-through PASS (`failed_api` chip on a real 404). pytest 3231 (+1) · Vitest 933 (+3) · mypy `src` 400 · v2 lints 11/11 · mockup 51. CHANGE-131, no design note. MERGED PR #385, main `980b3357`. Prior Sprint 57.163 MERGED PR #383 main `08a93901`; see `memory/` for sprint history
