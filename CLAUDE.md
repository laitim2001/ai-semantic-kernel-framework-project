...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.141 MERGED** (PR #335, main `041e9943`; branch from main `196d8892`) — **pass^k reliability eval harness (research #2)** — closes research #2 `AD-Eval-PassK-Reliability-Harness` (eval `passk-reliability-thin-spike-eval-20260624.md`; canonical-next after #6/#3/#8/#4/#1, all CLOSED). V2's first repeatability-measuring eval: a pure-offline `scripts/benchmark_pass_k.py` (mirrors `benchmark_judge.py`) running the SAME corpus input through V2's own `AgentLoopImpl.run()` k times, computing 4 axes (pass^k vs pass@1 · behavioral consistency · fault injection λ via `_FaultInjectingChatClient` wrapping the ChatClient ABC · perturbation ε), with a hybrid oracle (rules inline + `LLMJudgeVerifier` per-case criterion). 3 NEW files, **ZERO src edit** (WRAPS existing ChatClient/Verifier/AgentLoop ABCs; loop DB-less drivable per Day-0). 12-case difficulty-graded corpus + 17 CI-safe tests (spy ChatClient drives the REAL loop offline). pytest 2859+5skip (1 pre-existing flake, isolation-passes) · mypy 0/380 · v2 lints 21 (incl. llm_sdk_leak) · NO migration / wire stays 26 / NO frontend / NO loop.py edit. **Drive-through (real Azure gpt-5.2, 12 cases · k=3 · 90 runs · 94k tokens)**: pass^k 100% = pass@1 100% (**divergence 0% — corpus too easy to stress the τ-bench pass^8<25% regime; VALID verdict, either direction acceptable**) BUT 2 pass@1-invisible signals surfaced — **answer-consistency 75%** + **λ degradation 50%** (no chat-level retry). NEW class `passk-reliability-spike` 0.60 (1st pt ~1.0 IN band, parent-direct). CHANGE-108 + design note 45. (Prior: Sprint 57.140 task-primitive MERGED PR #333.) Detail: `memory/project_phase57_141_passk_reliability.md`. |
...
**Last Updated**: 2026-06-25 (Sprint 57.141 — pass^k reliability eval harness (research #2): V2's first repeatability eval — a pure-offline `scripts/benchmark_pass_k.py` (mirrors benchmark_judge) running the SAME corpus input through V2's own `AgentLoopImpl.run()` k times, computing 4 axes (pass^k vs pass@1 · behavioral consistency · λ fault-injection · ε perturbation) with a hybrid oracle (rules inline + LLMJudgeVerifier per-case criterion); 3 NEW files ZERO src edit (WRAPS ChatClient/Verifier/AgentLoop ABCs); 12-case corpus + 17 CI tests (spy ChatClient drives the REAL loop offline); pytest 2859+5skip · mypy 0/380 · v2 lints 21 · NO migration/wire-stays-26/frontend/loop.py-edit; **drive-through real Azure gpt-5.2 (12 cases · k=3 · 90 runs · 94k tokens): pass^k 100%=pass@1 100% (divergence 0% — corpus too easy to stress the τ-bench pass^8<25% regime, VALID verdict) BUT answer-consistency 75% + λ-degradation 50% (no chat-level retry) surfaced — 2 pass@1-invisible signals**; NEW calib class `passk-reliability-spike` 0.60 (1st pt ~1.0 IN band, parent-direct); CHANGE-108 + design note 45); see `memory/` for sprint history
