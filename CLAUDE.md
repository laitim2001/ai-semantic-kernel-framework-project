...
| **Current Sprint** | Sprint 57.70 closed 2026-06-02 (PR pending) — **Real Per-Tenant Agent-Spec Catalog (DB-backed) + Admin CRUD API (A-3b carryover, backend slice; FE → 57.71)**: replaces the 57.68 hardcoded persona stand-in with a real `agent_catalog` table (per-tenant, RLS 2-policy + FORCE, Group 9; fields per the mockup AgentSpec) + `AgentCatalogRepository` + an async tenant-scoped resolver (DB catalog → hardcoded `DEFAULT_AGENTS` → `None`, DB-error fail-safe) rewired into both HANDOFF consumers (None=reject / DEMO-fallback contract preserved; an empty catalog still resolves the 3 defaults) + a `0023` data-migration seeding the 3 defaults per existing tenant + an admin CRUD API (`/admin/tenants/{id}/agents`, distinct from the `/subagents` invocations STUB). A >20% Day-0 reframe (3 researcher rounds) found the "agent catalog" = the existing Subagent Registry of AgentSpec definitions (mockup + 09-schema Group 9 + a read-only fixture `/subagents` page) → plan revised + user re-confirmed **backend registry + CRUD API this sprint, FE → 57.71**. Feature-continuation (no design note). pytest 2049 / mypy src 0/329 / 10/10 V2 lints (RLS + SDK-leak green) / black 603 unchanged / Alembic 0023 up·down / no FE·wire·codegen. Detail: `memory/project_phase57_70_agent_catalog.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-02 (Sprint 57.70 — Real Per-Tenant Agent-Spec Catalog (DB-backed) + Admin CRUD API (A-3b carryover, backend slice; FE → 57.71): replaces the 57.68 hardcoded persona stand-in with an `agent_catalog` table (per-tenant, RLS 2-policy+FORCE, Group 9, fields per mockup AgentSpec) + `AgentCatalogRepository` + an async tenant-scoped resolver (DB → `DEFAULT_AGENTS` → None, DB-error fail-safe) rewired into both HANDOFF consumers (None=reject / DEMO-fallback preserved; empty catalog still resolves defaults) + a `0023` data-migration seeding 3 defaults per existing tenant + admin CRUD `/admin/tenants/{id}/agents` (distinct from the `/subagents` invocations STUB); a >20% Day-0 reframe (catalog = existing Subagent Registry of AgentSpec definitions) → user re-confirmed backend-only, FE → 57.71; feature-continuation (no design note); pytest 2049 / mypy src 0/329 / 10/10 V2 lints / black 603 unchanged / Alembic 0023 up·down / no FE·wire·codegen; CHANGE-038; PR pending); see `memory/` for sprint history
