...
| **Current Sprint** | **Sprint 57.158 — memory vector-recall precision A/B — PR-pending** (branch `feature/sprint-57-158-memory-recall-precision-ab` from main `65157788`; engine-debt program, memory range). Closes `AD-Memory-Vector-Recall-Precision-AB` (CARRY-026 semantic-axis follow-on). An **evidence-first A/B measurement spike, ZERO `src/` change**, settling 57.155's honest gap: does the semantic vector axis actually out-recall confidence-ranked `profile()` at many-fact scale? A 3-arm harness (`profile` confidence-desc / `keyword` substring / `semantic` the REAL `MemoryVectorIndex.search`, all ranking the case's `MemoryRow` list — `search(rows=…)` is a param → DB-free) + a discriminating 10-case corpus (8/10 gold-low-conf-outside-top-5 + question-query non-substring) + a CI-safe offline test (importlib-shadow + re-inlined `_FakeMemStore` + `DeterministicEmbeddingClient` — machinery/oracle only; the semantic win is real-Azure-only ∵ hash embeddings have no semantic structure). parent-direct agent_factor 1.0. **Verdict: semantic-wins (decisive)** — real `text-embedding-3-large` (3072-dim) + real Qdrant → semantic recall@k **100%** (MRR 1.0 = gold always first) vs profile **20%** vs keyword **10%** (semantic − profile **+80pp** ≫ 10% materiality); deterministic (no re-run). Validates the 57.155 axis behaviourally — the deferred semantic-axis slices are now evidence-backed. Honest caveat: corpus discriminating but non-adversarial → precision@k follow-on `AD-Memory-Vector-Recall-Adversarial-Corpus`. pytest +17 · mypy `src` 400/0 · run_all 11/11. NEW calib `memory-vector-recall-precision-ab-spike` 0.60 (1st pt ~1.0-1.05 IN band). CHANGE-125 + design note 61. (Prior: Sprint 57.157 scheduler PR #370 merged, main `65157788`.) Detail: `memory/project_phase57_158_memory_recall_precision_ab.md`. |
...
**Last Updated**: 2026-07-06 (Sprint 57.158 — memory vector-recall precision A/B: an evidence-first A/B measurement spike (ZERO `src/` change) settling 57.155's honest gap — does the semantic vector axis out-recall confidence-ranked `profile()` at many-fact scale? A 3-arm harness (profile/keyword/semantic, all ranking the case's `MemoryRow` list since `search(rows=…)` is a param → DB-free) + a discriminating 10-case corpus (8/10 gold-low-conf-outside-top-5 + question-query non-substring) + a CI-safe offline test (importlib-shadow + `DeterministicEmbeddingClient` — machinery/oracle only, the semantic win is real-Azure-only); **verdict semantic-wins (decisive)** real `text-embedding-3-large` + real Qdrant → semantic recall@k 100% (MRR 1.0) vs profile 20% vs keyword 10% (+80pp), deterministic no-re-run; validates the 57.155 axis behaviourally → the deferred semantic-axis slices now evidence-backed; honest caveat non-adversarial corpus → `AD-Memory-Vector-Recall-Adversarial-Corpus`; pytest +17 · mypy `src` 400/0 · run_all 11/11; NEW calib `memory-vector-recall-precision-ab-spike` 0.60; CHANGE-125 + design note 61; **PR-pending** closes `AD-Memory-Vector-Recall-Precision-AB` (engine-debt program, memory range; branch from main `65157788`); prior Sprint 57.157 scheduler PR #370 merged main `65157788`; see `memory/` for sprint history
