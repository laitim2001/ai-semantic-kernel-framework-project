...
| **Current Sprint** | **Sprint 57.134 PR-pending** (committed `feature/sprint-57-134-transcript-retention` from main `631f599a`; push/PR awaits user confirmation) — **per-tenant transcript retention (apply + dry-run preview)** — closes the transcript-retention (57.125) Option-A slice. **Day-1 pivot** (user AskUserQuestion "Pivot + preview"): `Tenant.retention_days` ALREADY EXISTS (Sprint 57.46 SaaS settings, settable via PATCH `/tenants/{id}`) → the planned parallel `meta_data["transcript_retention"]` config was an AP-6 trap → DROPPED → shipped enforcement on the canonical column. Backend-only, NO migration: `apply_transcript_retention(db, tid, retention_days, *, now, dry_run)` (DELETE-by-age on `messages`+`message_events`; `SET LOCAL app.tenant_id` for FORCE'd RLS + explicit `WHERE tenant_id`; `dry_run`=COUNT) + POST `/{id}/transcript-retention/apply` + GET `/{id}/transcript-retention/preview`. +10 tests (3 unit + 7 integration incl. multi-tenant isolation + audit). mypy 0/374 · run_all 10/10 · pytest 2741+5skip (+10) · FE untouched. **Drive-through PASS** (real uvicorn PID 55768 + real admin dan@acme.com platform_admin + real DB): preview on acme-prod (non-destructive, PATCH retention_days 90→1→90, cutoff tracks) + apply on a throwaway tenant (destructive primary path: seeded old now-60d/recent now-5d retention 30 → deleted 1/1, recent survived, REMAINING_MESSAGES=1, cleaned up). NEW class `transcript-retention-apply-spike` 0.60 (ratio ~1.0-1.1 IN band). CHANGE-101 + design note 39. Detail: `memory/project_phase57_134_transcript_retention.md`. |
...
**Last Updated**: 2026-06-17 (Sprint 57.134 — per-tenant transcript retention: apply (DELETE-by-age) + dry-run preview on the canonical `tenants.retention_days` column; Day-1 pivot dropped a parallel `meta_data` config (AP-6) after finding the column already exists (Sprint 57.46); backend-only NO migration, +10 tests incl. multi-tenant isolation; drive-through PASS both legs (preview non-destructive + apply destructive on a throwaway tenant: deleted 1/1, recent survived); CHANGE-101 + design note 39); see `memory/` for sprint history
