...
| **Current Sprint** | **Sprint 57.163 — Tool-error reflection rare-path verify + weaker-model A/B — MERGED PR #383, main `08a93901`** (engine-debt program, **Tool range** 1st slice). Closes 2 of ~4 Tool ADs: ③1 `AD-Tool-Error-Reflection-Loop-RarePath-DriveThrough` + ③4 weaker-model re-check (both 57.144 carryovers). **Day-0 re-scope (方案 A)**: D-executor-self-raise found the `loop.py:3068` rare branch near-unreachable on the 主流量 (executor turns every failure into a `ToolResult`, doesn't self-raise) → ③1 verified by an integration fault-inject test (`_RaisingExecutor` → assert the branch, lever ON/OFF) + honest **gate-only** label + reality finding (NOT a drive-through — a human can't reach the branch; pure-backend evidence slice, Drive-Through pure-backend exemption); ③4 added a `--answerer-tier {action,cheap}` knob + one real-Azure cheap A/B. **KEY finding — reflection is tier-dependent**: strong tier +0.00% (57.144, keep OFF) vs weak tier **+12.5%** at −6.8 tokens → refines the blanket "keep OFF". Lever default stays OFF (evidence-only sprint; an 8-case single-run is directional, not decision-grade for a global flip). pytest 3230 (+6) · mypy `src` 400 · v2 lints 11/11 · NO migration/wire(26)/codegen/frontend/loop.py-logic change. NEW calib `tool-reflection-drivethrough-evidence-spike` 0.60 1st pt (~1.0 IN band). CHANGE-130, no design note. Carryover: `AD-Tool-Reflection-{RarePath-Near-Dead-Evaluate,PerTier-Default}`. (Prior: Sprint 57.162 DAG soft-enforce MERGED PR #381, main `f4828495`.) Detail: `memory/project_phase57_163_tool_reflection_evidence.md`. |
...
**Last Updated**: 2026-07-09 (Sprint 57.163 — Tool-error reflection rare-path verify + weaker-model A/B: engine-debt **Tool range** 1st slice, closes ③1 `AD-Tool-Error-Reflection-Loop-RarePath-DriveThrough` + ③4 weaker-model re-check (57.144 carryovers). Day-0 re-scope (方案 A): D-executor-self-raise found the `loop.py:3068` rare branch near-unreachable (the executor turns every failure into a `ToolResult`) → ③1 verified by an integration fault-inject test + honest gate-only label + reality finding (NOT a drive-through; pure-backend evidence slice); ③4 added a `--answerer-tier` knob + a real-Azure cheap A/B. KEY finding — reflection is tier-dependent: strong 57.144 +0.00% keep-OFF vs weak +12.5% at −6.8 tok → refines the blanket keep-OFF; lever default stays OFF (evidence-only). pytest 3230 (+6) · mypy `src` 400 · v2 lints 11/11. NEW calib `tool-reflection-drivethrough-evidence-spike` 0.60 1st pt. CHANGE-130, no design note. Carryover: AD-Tool-Reflection-{RarePath-Near-Dead-Evaluate,PerTier-Default}. **MERGED PR #383, main `08a93901`**; prior Sprint 57.162 MERGED PR #381 main `f4828495`; see `memory/` for sprint history
