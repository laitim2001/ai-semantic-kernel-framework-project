...
| **Current Sprint** | Sprint 57.123 (PR-pending, NOT pushed) — **Tenant-display real-data wiring — closes `AD-FE-Tenant-Display-Fixture-Phase58`, a C-class 主流量 Potemkin** (the next C-class after the 57.122 HITL载重 gap). The always-visible app chrome (sidebar tenant pill + topbar tenant pill + UserMenu tenant switcher) rendered a HARDCODED fixture tenant (`acme-prod` / `tenant_01h9a2 · Pro` / a 3-tenant globex-eu/initech-jp list) regardless of the logged-in tenant. Now `/auth/me` `AuthMeTenant` += `plan` + `region` (real `Tenant` cols, at ALL 3 build sites `me()`/`dev_login()`/`issue_session()`) → the FE `AuthTenant` auto-threads (+2 fields; whole-object `set`, no codegen) → the 3 components read `authStore.tenant`; the UserMenu collapses 3→1 to the single real current tenant. NO migration/wire(24)/codegen; CSS byte-identical (mockup 51). mypy 0/371 · run_all 10/10 · pytest 2696+5skip · Vitest 892. Drive-through PASS (real chrome + 2 real logins): the chrome FOLLOWS the session (acme-prod → globex-eu), old fixtures gone. Detail: `memory/project_phase57_123_tenant_display_real_data.md` + CHANGE-090 (NO design note). Next: remaining C-class Potemkin + the carryover pool (`claudedocs/1-planning/next-phase-candidates.md`). |
...
**Last Updated**: 2026-06-15 (Sprint 57.123 — Tenant-display real-data wiring: the app chrome (sidebar/topbar tenant pill + UserMenu tenant switcher) now reads `authStore.tenant` instead of a hardcoded `acme-prod`/`tenant_01h9a2 · Pro`/3-tenant fixture; `/auth/me` `AuthMeTenant` += `plan` + `region` (real `Tenant` cols at all 3 build sites) → FE `AuthTenant` auto-threads → 3 components real + UserMenu 3→1 collapse; closes `AD-FE-Tenant-Display-Fixture-Phase58` (a C-class 主流量 Potemkin); NO migration/wire/codegen, CSS byte-identical (mockup 51); pytest 2696+5skip · Vitest 892 · mypy 0/371; drive-through PASS (chrome follows the session across 2 real logins); CHANGE-090, NO design note); see `memory/` for sprint history
