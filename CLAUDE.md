...
| **Current Sprint** | Sprint 57.82 (PR pending) — **B-8 leg-1: verification judge token → cost ledger + quota** (closes B-8 blocker A / `AD-Cat10-Judge-Cost-Ledger`): when verification is enabled, the LLM judge call's tokens are now recorded as a distinct `_verification` cost-ledger sub_type + counted vs quota (previously discarded → billing/quota under-report). Wrapper accumulates judge tokens across verifiers+attempts → `LoopCompleted.verification_*_tokens` → router records distinct entry + quota actual. Default `chat_verification_mode` UNCHANGED (`disabled`) — this is a correctness fix activating only on the enabled path; leg 2 (57.83) does blocker B (general judge template) + C (real-LLM e2e) + flip. backend+docs mypy 0/332 + pytest 2147 + run_all 10/10; no design note (17.md §1.1/§4.1 in-place). Detail: `memory/project_phase57_82_verification_judge_cost_ledger.md`. Next: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-05 (Sprint 57.82 — B-8 leg-1: verification judge token → cost ledger + quota, closes B-8 blocker A/AD-Cat10-Judge-Cost-Ledger; judge LLM tokens recorded as distinct `_verification` sub_type + counted vs quota when enabled; default chat_verification_mode unchanged (disabled) — leg 2 = 57.83 blocker B+C + flip; backend+docs mypy 0/332 + pytest 2147 + run_all 10/10; CHANGE-049; PR pending); see `memory/` for sprint history
