...
**Last Updated**: 2026-06-27 (Sprint 57.148 — memory-formation Slice 1: user-identity write (system-prompt nudge → proactive `memory_write(scope=user)`) + always-on `profile()` inject (keyword-independent, bypasses ILIKE query-gating); drive-through PASS — cross-session identity recall + per-user isolation on real chat-v2 + real Azure gpt-5.2; CHANGE-115 + design note 52; MERGED PR #350 main `4fe02965`); see `memory/` for sprint history
