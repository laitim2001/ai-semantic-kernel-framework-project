...
| **Current Sprint** | Sprint 57.59 closed 2026-05-28 (PR pending) — **RateLimits Potemkin Migration C1 two-table split (Phase 58.x deeper extensions 2/5)** closes `AD-RateLimits-Potemkin-Migration-Phase58` + **AP-4 CLOSED**: NEW `rate_limit_configs` table (durable config, replaces `meta_data` JSONB) + activate dormant `rate_limits` usage table (since Phase 49); Alembic `0019` + 2 RLS policies + additive data migration; re-point GET/PUT/usage/middleware (API shapes UNCHANGED → frontend untouched). Calibration: `mixed-multidomain-bundle-mechanical` 0.65 tier-3 2nd validation ~0.34 BELOW band → 2 consec < 0.7 → tighten 0.65→0.45 (Sprint 57.60+). DUAL CLEAN 22/22 PARITY **15 consecutive 57.45-57.59**. Detail: `memory/project_phase57_59_rate_limits_potemkin_migration.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-05-28 (Sprint 57.59 — RateLimits Potemkin Migration C1 two-table split **Phase 58.x deeper extensions 2/5**; NEW `rate_limit_configs` + activate dormant `rate_limits` usage table **AP-4 CLOSED**; Alembic 0019 + 2 RLS policies + additive data migration; re-point GET/PUT/usage/middleware API-shapes-unchanged; `mixed-multidomain-bundle-mechanical` 0.65 tier-3 2nd validation ~0.34 BELOW band → 2 consec < 0.7 → tighten 0.45 Sprint 57.60+; DUAL CLEAN 15 consec 57.45-57.59; commits `560a7697`+`195072ef` Day 0+1 + Day 2 pending); see `memory/` for sprint history
