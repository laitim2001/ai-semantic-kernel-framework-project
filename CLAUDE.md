...
| **Current Sprint** | Sprint 57.114 (PR-pending) — **Skills System epic 2nd slice: per-tenant catalog overlay — full-stack, drive-through ALL 3 legs PASS** (closes `AD-Skills-Per-Tenant-Catalog`). A tenant authors skills via a new tenant-settings "Skills" tab → stored in a NEW `tenant_skills` table (migration 0030, RLS two-policy minus sentinel) → `resolve_tenant_skill_registry` (TTL cache, fail-open) overlays them on the 57.113 bundled set per chat request via `SkillRegistry.with_overlay` (override-by-name) → a **1-line** router swap (`handler.py`/`make_default_executor`/`loop.py` UNTOUCHED). `TenantSkillService` RLS-scoped CRUD + 4 admin endpoints; FE list-CRUD tab (Add/Edit/2-step-Delete). No new wire event (count 24) / no codegen. Gate: mypy 0 · run_all 10/10 · backend pytest 2602+5skip (+36) · FE lint 0 / build / Vitest 851 (+11) / mockup-fidelity 51. Drive-through (real chat-v2 + real Azure gpt-5.2, 2 dev-login tenants): A author→`read_skill`→3-header-follow / B isolation (`read_skill` 0× + bundled intact) / C override-by-name→numbered-checklist — load+follow + isolation proven (AP-4 guard). NEW scope class `per-tenant-catalog-table-backed` 0.60 (1st pt ~0.92). Detail: `memory/project_phase57_114_skills_per_tenant_catalog.md` + CHANGE-081 + design note 32. Next: the carryover pool (`claudedocs/1-planning/next-phase-candidates.md` — Skills slash-command/Inspector/authoring-richness, MFA WebAuthn/recovery, SOC 2 + SBOM, FE mockup rebuild). |
...
**Last Updated**: 2026-06-13 (Sprint 57.114 — Skills System 2nd slice: per-tenant catalog overlay (tenant_skills table + resolver + admin tab), drive-through ALL 3 legs PASS, closes AD-Skills-Per-Tenant-Catalog; CHANGE-081 + design note 32); see `memory/` for sprint history
