...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.145** (branch `feature/sprint-57-145-knowledge-connector-spike` from main `7cedcea4`; **PR-pending**) — **first real external data-source connector** — `knowledge_search` + `LocalDocsConnector` reading real in-repo `.md`; opt-in `make_default_executor` (mirrors todo_store/skill_registry) + handler default-ON + prompt nudge. Answers reality-audit §9 gate: **vision pillar 1 (connect external systems) 0 → 1**. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2, 3 rounds): found+fixed a multi-word-query 0-hit bug (tokenize + OR match) the green gate couldn't catch → clean PASS, agent cites 3 real `.md` (`08-glossary`/`04-anti-patterns`/`00-v2-vision`); BONUS per-tenant output-ESCALATE HITL verified live. Backend-only NO migration / wire 26 / NO frontend, reuses Cat 2 ToolSpec. pytest 2947+6skip · mypy 0/385 · run_all 11/11 · Vitest 922 · mockup 51. NEW calib class `knowledge-connector-real-source-spike` 0.55 (1st pt ~1.15-1.25 upper-edge IN band, parent-direct). CHANGE-112 + design note 49. Carryover `AD-Knowledge-Connector-First-Real-Source` (Slice 2: snippet depth + embedding + external sources). (Prior: Sprint 57.144 tool-desc lint + error reflection MERGED PR #341.) Detail: `memory/project_phase57_145_knowledge_connector.md`. |
...
**Last Updated**: 2026-06-26 (Sprint 57.145 — first real external data-source connector: `knowledge_search` + `LocalDocsConnector` over real in-repo `.md`, vision pillar 1 0→1 per reality-audit §9; drive-through PASS found+fixed multi-word 0-hit bug, agent cites 3 real `.md`; CHANGE-112 + design note 49); see `memory/` for sprint history
