...
| **Current Sprint** | Sprint 57.119 (PR-pending, NOT pushed) — **Skills epic authoring-UX visibility: read-only "System Skills" section + Preview modal — drive-through PASS** (ships the system-skills-visibility leg of `AD-Skills-Authoring-UI`; 3rd of the user's "complete all Skills work" sequence; scope chosen via AskUserQuestion over the AD's versioning / hot-reload interpretations). The 57.114 admin Skills tab showed ONLY the tenant's overlay skills; the system-bundled set (`code-review`/`digest`/`summarize`, the base every tenant overlays + what the model sees) was invisible. This adds a read-only "System Skills" `<Card>` (a "🔧 script" badge for `has_script` + a "shadowed by your skill" tag for per-tenant `overridden`) + a Preview modal rendering any skill's full instructions (bundled or tenant). ONE read-only `GET /admin/tenants/{tenant_id}/skills/system` (`list_system_skills`, mirrors `list_tenant_skills` auth) over `get_default_skill_registry().list()`; `has_script = skill.script is not None` (57.118), `overridden = name in {tenant skill names}` (the tenant_id makes it meaningful). api→Cat-5 import follows the `handler.py:96`/`router.py:465` precedent (the #1 lint risk RESOLVED Day-0). FE: `SystemSkill` + `fetchSystemSkills` + `useSystemSkills` (own key) + an inline-overlay Preview modal (NO `Modal` primitive → the `TenantMembersDrawer` a11y convention: window Escape + `role="dialog"` + matching `jsx-a11y` disables; `var(--bg)`). Read-only — respects 57.118's tenant-script deferral (`has_script` is a boolean badge; the script source is never shown). NO DB / migration / wire (count 24) / codegen. Gate: mypy 0/371 · run_all 10/10 · backend pytest 2648+5skip (+4) · FE Vitest 879 (+6) · mockup 51. Drive-through (real admin Skills tab + real backend, acme-skills): the section lists 3 bundled read-only · `digest` 🔧-badge only · a live-created tenant `code-review` → "shadowed by your skill" tag (deleted 204) · Preview renders `digest.md` verbatim · Close dismisses — every control live (AP-4 clear). NEW `skills-admin-readonly-surface` 0.55 (1st pt ~0.97 IN band, parent-direct). Detail: `memory/project_phase57_119_skills_system_visibility.md` + CHANGE-086 (NO design note — feature continuation). Next: `AD-Skills-Authoring-UI` versioning + hot-reload + disable-toggle legs 🟡 carried + remaining Skills ADs (`claudedocs/1-planning/next-phase-candidates.md` — 120 inspector-metadata / 121 slash-menu-mockup) + carryover pool. |
...
**Last Updated**: 2026-06-15 (Sprint 57.119 — Skills epic authoring-UX visibility: a read-only "System Skills" section in the admin Skills tab (🔧 script badge + shadowed-by-tenant tag) + a Preview modal rendering any skill's instructions; one read-only `GET /skills/system` over `get_default_skill_registry()`; ships the system-skills-visibility leg of AD-Skills-Authoring-UI; NO DB/migration/wire (count 24), drive-through PASS; CHANGE-086, NO design note (feature continuation)); see `memory/` for sprint history
