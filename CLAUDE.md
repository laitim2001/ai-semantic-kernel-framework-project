...
| **Current Sprint** | Sprint 57.62 closed 2026-05-29 — **RateLimits Alerting** closes `AD-RateLimits-Alerting-Phase58`: server-side 80%-threshold usage alerting that PERSISTS a row when usage crosses 80% of quota — captured even when no admin watches the live-usage card. NEW `RateLimitAlert` ORM (`rate_limit_alerts`, severity lowercase warning/critical 2-tier + CHECK, UNIQUE window) + Alembic `0021` (FORCE RLS 2-policy) + stateless `RateLimitAlertStore.maybe_record` (idempotent peak/escalate `on_conflict_do_update` GREATEST, early-return quota<=0/pct<80, fail-open) hooked into `RedisRateLimitCounter._write_through` (D-DAY0-G: session+7 values in scope → no ctor DI / no main.py wiring) + `GET .../rate-limits/alerts` + frontend `useRateLimitsAlerts` (15s poll) + QuotasTab Recent alerts Card (`.badge.warning`/`.badge.danger`, 0 new oklch; existing 2 cards scope-guard). Day 0 pivot: SSE-carryover proved false (admin SSE greenfield) → user-locked Option A persisted log. Sequential 2-agent 28th backend + 29th frontend consecutive; pytest 1887→1907 (+20: 12 store + 6 endpoint + 2 migration) / mypy src 0/319 / 9/9 V2 lints (check_rls_policies 21 tables) / Vitest →686 (+17) / oklch delta 0 / Alembic 0021 down→up clean. Calibration: `mechanical-greenfield-design-decisions` 0.65 4th validation BACK TO PAIR SHAPE ~0.77 BELOW band → KEEP single-data-point-per-shape (pair sub-seq 57.56+57.57+57.62 mean ~0.98 IN band; **R6 WEAKENS** — 57.61 backend-only 0.74 + 57.62 pair 0.77 both below cross-shape; NEW `AD-AgentFactor-DesignDecisions-Below-Band-Watch`); `medium-backend` 0.80 13th ~0.50 confound-resolved KEEP. DUAL CLEAN 22/22 PARITY **18 consecutive 57.45-57.62**. Detail: `memory/project_phase57_62_rate_limits_alerting.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-05-29 (Sprint 57.62 — RateLimits Alerting; server-side 80%-threshold usage alerting persists a breach row even when unwatched — NEW `RateLimitAlert` ORM + Alembic `0021` FORCE RLS 2-policy + stateless `RateLimitAlertStore.maybe_record` (idempotent peak/escalate, fail-open) hooked into `_write_through` (D-DAY0-G no main.py wiring) + GET alerts endpoint + QuotasTab Recent alerts Card (0 new oklch) **closes `AD-RateLimits-Alerting-Phase58`**; Day 0 pivot SSE-carryover-false → Option A persisted log; `mechanical-greenfield-design-decisions` 0.65 4th validation pair shape ~0.77 BELOW band KEEP (R6 weakens — 2 consec design-decisions below cross-shape; NEW `AD-AgentFactor-DesignDecisions-Below-Band-Watch`); `medium-backend` 0.80 13th ~0.50 KEEP; pytest 1887→1907 (+20) / Vitest →686 (+17); DUAL CLEAN 18 consec 57.45-57.62; CHANGE-030; commits `79282286`+`95c65e09` Day 0+1 + Day 2 pending); see `memory/` for sprint history
