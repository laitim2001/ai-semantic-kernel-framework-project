...
| **Current Sprint** | Sprint 57.115 (PR-pending) — **Skills System epic 3rd slice: `/skill-name` slash-command force-load — full-stack, drive-through ALL 3 legs PASS** (closes `AD-Skills-Slash-Command`). The user-invoked half after 57.113 (model-invoked) + 57.114 (per-tenant): a greenfield `SkillSlashMenu` composer picker (`/`-trigger + ↑/↓/Enter/Esc + leading-token parse) → `ChatRequest.force_load_skill` → router validate-and-pass → `build_handler` appends the picked skill's full instructions under `## Active Skill` = **deterministic force-load** (model loads it WITHOUT `read_skill`); a DRY `render_skill_instructions` helper (read_skill byte-identical) + a non-admin `GET /chat/skills` picker list (name+desc only, no body leak). `make_default_executor`/`loop.py`/`read_skill` self-select/wire(24)/codegen/migration UNTOUCHED. Gate: mypy 0/370 · run_all 10/10 · backend pytest 2616+5skip (+14) · FE lint 0 / build / Vitest 863 (+12) / mockup-fidelity 51. Drive-through (real chat-v2 + real Azure gpt-5.2, tenant acme-skills): A `/release-notes`→output follows Summary/Highlights/Upgrade + **`read_skill` 0×** (Loop trace) + token stripped / B `/` lists 3 + `/re` filters / C `/nonexistent`→plain text + agent "OK" graceful. NEW scope class `skills-slash-command-fullstack` 0.55 (1st pt ~1.0). Detail: `memory/project_phase57_115_skills_slash_command.md` + CHANGE-082 + design note 33. Next: the carryover pool (`claudedocs/1-planning/next-phase-candidates.md` — Skills Inspector-affordance/authoring/quota, MFA WebAuthn/recovery, SOC 2 + SBOM, FE mockup rebuild). |
...
**Last Updated**: 2026-06-14 (Sprint 57.115 — Skills System 3rd slice: `/skill-name` slash-command force-load (`## Active Skill` deterministic injection + `GET /chat/skills` + greenfield `SkillSlashMenu`), drive-through ALL 3 legs PASS, closes AD-Skills-Slash-Command; CHANGE-082 + design note 33); see `memory/` for sprint history
