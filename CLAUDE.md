...
| **Current Sprint** | **Sprint 57.137 PR-pending** (branch `feature/sprint-57-137-sandbox-detect-to-restrict`, from main `ba4c5c13`) — **sandbox detect→restrict spike** — closes `AD-Guardrail-Detect-To-Restrict` (research #3). Reframed the Cat 9 `RiskyActionDetector` regex deny-list from "the boundary" to ESCALATE-for-visibility, and made the structural DockerSandbox isolation MANDATORY via an env-gated fail-closed lever (`SANDBOX_REQUIRE_ISOLATION`, DEFAULT OFF) — `default_sandbox()` now returns a refusing `_FailClosedSandbox` on a non-isolating backend instead of silently degrading to the production-unsafe SubprocessSandbox; the gate reads a new provider-neutral `SandboxBackend.is_structurally_isolated` property (not isinstance). A permanent real-Docker escape harness MEASURED the gap: **regex_escape_rate 60% · docker_containment_rate 100%** (deny-list misses urllib/http.client/importlib-concat/asyncio/ftplib/smtplib; `network none` contains all 10) → regex redundant-for-containment under Docker. Backend-only, NO migration / NO new wire event / NO frontend. pytest 2786+5skip (+21) · mypy 0/374 · run_all 10/10. **Drive-through PASS (兩者結合)** (backend runtime: real `_get_shared_sandbox`→`default_sandbox`→handler — ARM A require_isolation+Docker-unreachable → `_FailClosedSandbox` refusal/code-not-run + ARM B real Docker → `print(2+2)`→"4"; UI: chat-v2 real Azure gpt-5.2 → `print(31337*1337)` → HITL approve → real DockerSandbox → "41897569" + verification_passed 0.99 — no-regression). NEW class `guardrail-restrict-spike` 0.60 (1st pt ~0.97 IN band). CHANGE-104 + design note 41. Detail: `memory/project_phase57_137_sandbox_detect_to_restrict.md`. |
...
**Last Updated**: 2026-06-24 (Sprint 57.137 — sandbox detect→restrict spike: measured the regex deny-list's escape vs Docker containment + made structural isolation mandatory via an env-gated fail-closed lever; closes `AD-Guardrail-Detect-To-Restrict` (#3); a real-Docker escape harness showed regex_escape_rate 60% / docker_containment_rate 100% → regex redundant-for-containment; `SANDBOX_REQUIRE_ISOLATION` (DEFAULT OFF) makes `default_sandbox()` fail-closed (refusing `_FailClosedSandbox`) on a non-isolating backend instead of silently degrading; provider-neutral `is_structurally_isolated` property; detector reframed ESCALATE-for-visibility (check() byte-unchanged); backend-only NO migration; pytest 2786+5skip (+21); drive-through PASS (backend runtime fail-closed + chat-v2 UI no-regression); NEW calib class `guardrail-restrict-spike` 0.60; CHANGE-104 + design note 41); see `memory/` for sprint history
