...
| **Current Sprint** | Sprint 57.121 (PR-pending, NOT pushed) — **Skills slash-menu mockup + production re-point — drive-through PASS; closes `AD-Skills-SlashMenu-Mockup` = the Skills epic COMPLETE (57.113-121)**. 57.115 shipped the chat-v2 `/skill-name` picker GREENFIELD with inline token-styles + "No mockup reference exists" — the ONE chat-v2 element with no `reference/design-mockups/` entry (a Mockup-Fidelity Hard Constraint gap). This (1) AUTHORS it in the mockup (`page-chat.jsx` `SkillMenu` + `styles.css` `.skill-menu*`, CommandPalette-consistent via AskUserQuestion, token-only NO hex/oklch literal, no emoji; an interactive Composer makes the prototype demonstrable) and (2) RE-POINTS production `SkillSlashMenu.tsx` from inline-styles to the mockup classes (byte-identical CSS copy into `styles-mockup.css` → `diff` empty + a `Skills` group header + a `.kbd` footer + DROP the `eslint-disable no-restricted-syntax` = a net fidelity-debt reduction; KEEP the 57.115 testids/role/aria-selected/onMouseDown). InputBar UNTOUCHED (it owns the filter/keyboard/force-load; the mockup's interactive Composer is prototype-only). Two-step (user-directed): author mockup → review/approve → implement — the byte-identity gate couples reference+production so they can't split across PRs. 5 files ALL FE, **0 `.py`** → mypy 0/371 / pytest 2648+5skip / wire 24 UNCHANGED. Gate: Vitest 888 (+4) · `diff` empty · mockup 51 byte-identical · lint clean (no unused-directive from the dropped disable) · build ✅ · run_all 10/10. Drive-through (real chat-v2 + acme-prod, re-dev-login after the 57.120 JWT expired): `/` → menu in the NEW mockup styling + the REAL `useChatSkills` (`/code-review`(active)/`/digest`/`/summarize`, real descriptions) + footer "3 skills"; `/dig` → only `/digest` ("1 skill"); click → `/digest ` filled + closed (force-load). AP-4 clear, 3 screenshots. NEW class `mockup-author-and-port` 0.70 (1st pt ~1.17 IN band; set >0.65 repoint per the 57.120 ceremony-not-code-accelerated insight). Detail: `memory/project_phase57_121_skills_slash_menu_mockup.md` + CHANGE-088 (NO design note — feature continuation). Next: **Skills epic done** — the deferred larger Skills slices (`AD-Skills-Authoring-UI` versioning/hot-reload/disable, tenant-authored scripts, `AD-Config-Cache-MultiWorker-Invalidation`) + the 57.120 `model` row / token sweep + the carryover pool (`claudedocs/1-planning/next-phase-candidates.md`). |
...
**Last Updated**: 2026-06-15 (Sprint 57.121 — Skills slash-menu mockup + production re-point: author the chat-v2 `/`-slash picker in `reference/design-mockups/` (`page-chat.jsx` SkillMenu + `styles.css` `.skill-menu*`, CommandPalette-consistent, token-only) then re-point production `SkillSlashMenu.tsx` from inline-styles to the classes (byte-identical CSS copy into `styles-mockup.css` + group header + kbd footer + drop the `eslint-disable`); PURE FE, 0 `.py`, NO backend/wire/codegen (count 24); closes AD-Skills-SlashMenu-Mockup = **the Skills epic COMPLETE (57.113-121)**; drive-through PASS; CHANGE-088, NO design note (feature continuation)); see `memory/` for sprint history
