...
| **Current Sprint** | Sprint 57.67 closed 2026-06-02 (PR pending) — **Event Schema Codegen + CI Parity Gate (A-5b)**: a declarative single-source wire-schema registry (`event_wire_schema.py`, 18 wire-types, pure stdlib) generates the frontend event contract (`events.json` + `loopEvents.generated.ts`, consumed via a `types.ts` re-export), locked by a pytest serializer↔registry parity test (30) + a `check_event_schema_sync` lint (10th V2 lint) wired into `run_all.py` + the CI `lint.yml` `v2-lints` required check — so the serializer↔FE drift 57.66 maintained by hand is now mechanically un-mergeable. **No `loop.py`/`events.py`/`sse.py` rewrite** (serializer locked by the parity test). Architecture = user-chosen declarative registry (Day-0 D5: wire shape ≠ dataclass); Stage-1 flat-vs-nested shape error caught by Stage-2 reconciliation. pytest 1994 / mypy src 320/0 / 10/10 V2 lints / Vitest 697 / tsc 0 / build ✓. Detail: `memory/project_phase57_67_event_schema_codegen.md`. Next candidates: `claudedocs/1-planning/next-phase-candidates.md`. |
...
**Last Updated**: 2026-06-02 (Sprint 57.67 — Event Schema Codegen + CI Parity Gate (A-5b): a declarative wire-schema registry generates the FE event types + a CI parity gate, automating the 57.66 serializer↔FE double-gate so drift is mechanically un-mergeable; sse.py NOT rewritten (locked by parity test); pytest 1994 / mypy src 320/0 / 10/10 V2 lints / Vitest 697 / tsc 0 / build ✓; CHANGE-035; PR pending); see `memory/` for sprint history
