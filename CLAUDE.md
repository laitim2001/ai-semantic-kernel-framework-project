...
| **Current Sprint** | **Sprint 57.146 — knowledge Slice 2 (embedding + Qdrant semantic search)** — DONE, **PR-pending (NOT pushed)**, branch `feature/sprint-57-146-knowledge-embedding-vector` from main `03f2b79d`. Section-aware chunking (`##` split) → keyword snippet returns whole section = **fixes 57.145 R2 over-search**; + platform's first real `EmbeddingClient` ABC + Azure `text-embedding-3-large` adapter (SDK confined per 約束 3) + `QdrantVectorStore` (`query_points`; closes CARRY-026 for KB) + `KnowledgeVectorIndex` (batched ingest + cosine). Opt-in `KNOWLEDGE_VECTOR_ENABLED` (default OFF → 57.145 byte-identical) vector-primary + **keyword fail-soft fallback**. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2 + real embedding + real Qdrant; trace `f5b394db`): a query with NO literal "adapter"/"neutrality" → agent calls knowledge_search (TOOL_EXEC 1750ms) → Qdrant returns `adapters-layer.md`/`llm-provider-neutrality.md` by similarity → answer cites each real source path+section. 2 drive-through bugs the green gate missed: env-name rename (`deployment_embedding`) + **429 batching** (`_EMBED_BATCH=16`; default root recurses 418 files/3818 sec → bounded `docs/rules-on-demand` 129). Backend-only NO migration / wire 26 / NO frontend, new dep `qdrant-client`. pytest 2980+6skip · mypy 0/392 · run_all 11/11 · Vitest 922 · mockup 51. NEW calib `knowledge-embedding-vector-spike` 0.60 (1st pt ~1.05-1.15 IN band, parent-direct). CHANGE-113 + design note 50. Carryover `AD-Knowledge-Connector-RBAC-Citation-Slice3` + `AD-Knowledge-Connector-Ingest-Scale` (background ingest). (Prior: Sprint 57.145 MERGED PR #344.) Detail: `memory/project_phase57_146_knowledge_embedding_vector.md`. |
...
**Last Updated**: 2026-06-27 (Sprint 57.146 — knowledge Slice 2: section-aware snippets + first real `EmbeddingClient` ABC + Azure text-embedding-3-large + `QdrantVectorStore` semantic search; opt-in vector-primary + keyword fail-soft; drive-through PASS — no-literal-keyword query → semantic retrieval cites real source paths; 2 drive-through bugs env-name+429-batching; CHANGE-113 + design note 50); see `memory/` for sprint history
