...
| **Current Sprint** | **Sprint 57.156 — DAG task primitive: dependency-aware todos — MERGED** (PR #366, main `78c8ff8c`, CI all-green; branch from main `e7b4ba73`). Closes `AD-TaskPrimitive-DAG-Phase58` (the 57.140 DAG evolution; the memory track hit landing saturation → user-picked axis-switch to the AI-agent-system frontier). Evolves the LINEAR `write_todos` into a dependency DAG: `Todo.depends_on` end-to-end — tolerant serde + `ready_todos` (direct-dependency, cycle-safe, unknown-dep-ignored) + dependency-aware `## Active Plan` `(ready)`/`(blocked by:…)` render (byte-identical when no dep) + `write_todos` schema `depends_on` + chat-v2 Todos panel "⤷ needs" line + `blocked` badge. **Guidance not enforcement** (renders ready/blocked, never blocks the agent — the 57.140 nudge-not-force pattern). Rode the 57.140 machinery serde-transparently → **NO migration / wire schema(26) / codegen / loop.py / sse.py / store change** (backend delta 2 files `_contracts/todo.py`+`tools/todo_tools.py` + 2 FE); parent-direct agent_factor 1.0. **Drive-through STRONG PASS** both legs (real chat-v2 + Azure gpt-5.2, trace `743c72d4…`): Leg 1 the agent authored a real DAG (`2→[1]`,`3→[1]`,`4→[3]`,`5→[4]`), the `todos_updated` wire carried `depends_on` (sse.py UNCHANGED confirmed live), the agent identified the ready root, the panel rendered blocked badges + "⤷ needs" lines; Leg 2 completing step 1 LIVE-unblocked steps 2+3 (blocked badge removed) while 4+5 stayed blocked. 19 new tests · pytest 3139/6skip (+16) · Vitest 925 (+3) · mypy `src` 399/0 · run_all 11/11 · mockup 51 byte-identical. NEW calib `task-primitive-dag-spike` 0.60 (1st pt ~0.95-1.0 IN band). CHANGE-123 + design note 59. MERGED PR #366 main `78c8ff8c`. (Prior: Sprint 57.155 MERGED PR #364, main `e1083964`.) Detail: `memory/project_phase57_156_dag_task_primitive.md`. |
...
**Last Updated**: 2026-07-01 (Sprint 57.156 — DAG task primitive, dependency-aware todos: evolves the 57.140 LINEAR `write_todos` into a dependency DAG — `Todo.depends_on` end-to-end (tolerant serde + `ready_todos` direct-dependency/cycle-safe/unknown-dep-ignored + dependency-aware `## Active Plan` `(ready)`/`(blocked by:…)` render, byte-identical when no dep) + `write_todos` schema `depends_on` + chat-v2 Todos panel "⤷ needs" line + `blocked` badge; **guidance not enforcement**; rode the 57.140 machinery serde-transparently → NO migration/wire(26)/codegen/loop.py/sse.py/store change (backend delta 2 files + 2 FE); parent-direct agent_factor 1.0; drive-through STRONG PASS both legs (real chat-v2 + Azure gpt-5.2, trace `743c72d4…` — Leg 1 agent authored the DAG + panel rendered blocked badges; Leg 2 completing step 1 LIVE-unblocked steps 2+3); 19 new tests · pytest 3139/6skip · mypy `src` 399/0 · run_all 11/11 · Vitest 925; NEW calib `task-primitive-dag-spike` 0.60; CHANGE-123 + design note 59; **MERGED PR #366 main `78c8ff8c`** closes `AD-TaskPrimitive-DAG-Phase58` (branch from main `e7b4ba73`); prior Sprint 57.155 MERGED PR #364 main `e1083964`); the memory track hit landing saturation → user-picked axis-switch to the AI-agent-system frontier; see `memory/` for sprint history
