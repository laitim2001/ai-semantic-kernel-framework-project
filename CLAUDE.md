...
| **Current Sprint** | **Sprint 57.153 — memory-aware in-loop verification judge — MERGED** (PR #360, main `0ce4d1fa`, CI all-green; branch from main `5bbf1a77`). Closes `AD-Verification-Judge-Memory-Inject-Blind` (logged 57.149 carryover, re-confirmed 57.152 Leg-2). The in-loop Cat 10 judge (57.98 A1 + 57.111 A3) saw only `{output}`+`{trace}` (built from the `messages` accumulator); the `profile()`/`recent_sessions()`-injected memory lives in the per-turn prompt artifact (system prompt), never in `messages`, and `build_trace_block` drops system messages → the judge was BLIND to the agent's grounding → false-positive-REJECTed memory-grounded recalls → coached into no-recall answers (57.149/152 Leg-2). Fix = the direct parallel of 57.111 A3 (`{trace}`): a NEW `TransientState.injected_memory` field + `build_memory_block` helper + a `{memory}` judge-template placeholder + a "consistent-with-memory = GROUNDED, still flag contradictions" instruction (`output_quality`+`key_condition`); the loop captures per-turn `memory_accesses` at the PromptBuilder build branch and threads them into `_cat10_verify_gate(injected_accesses=)`, gated by a NEW default-ON env lever `CHAT_VERIFICATION_MEMORY_GROUNDING` (ctor kwarg mirrors 57.136 `correction_context_strategy`). **Key design**: thread via a `TransientState` field (the gate already builds a throwaway trace_state), NOT a `Verifier.verify` ABC kwarg (0 verifier-impls + 0 test-stubs vs ~13). `build_memory_block`/`build_trace_block` re-exported from `verification/__init__.py` (check_cross_category_import). Backend-only NO migration/wire(26)/frontend. parent-direct agent_factor 1.0. **Real-Azure A/B harness** (`benchmark_memory_grounded_judge.py`, 10 cases): `fabrication_catch_rate` bare **0%→memory 100%** (+100pp), `grounded_recall_false_reject_rate` 0%→0% → **KEEP default ON** (honest finding: the lenient `output_quality` judge doesn't false-reject grounded recalls in isolation → the fix's deterministic value is CONTRADICTION detection at zero false-reject cost). **Drive-through STRONG PASS** (real chat-v2 + Azure gpt-5.2): Leg-1 jamie "Dana Okafor / Verification Loops" → memory_write ×2 + verify 0.99; Leg-2 NEW session 0-keyword "你知道我是誰?" → the Loop trace shows the user memory injected (NEW Dana facts + accumulated 57.148 Chris facts = a CONTRADICTION); the agent recalled both grounded facts + flagged the conflict; memory-aware judge **VerificationPassed 0.98** (the grounded recall DELIVERED, vs the pre-fix blind judge that rejected→no-recall). pytest 3082 (+29) · mypy 0/397 · run_all 11/11 · flake8/llm_sdk_leak clean · FE untouched. NEW calib `verification-memory-grounding-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-120 + design note 56. (Prior: Sprint 57.152 MERGED PR #358, main `cd317ec7`.) Detail: `memory/project_phase57_153_verification_memory_grounding.md`. |
...
**Last Updated**: 2026-07-01 (Sprint 57.153 — memory-aware in-loop verification judge: the in-loop Cat 10 judge saw only `{output}`+`{trace}` (from the `messages` accumulator) but never the `profile()`/`recent_sessions()`-injected memory (it lives in the per-turn prompt artifact, and `build_trace_block` drops system messages) → it false-positive-REJECTed memory-grounded recalls as fabrication (57.149/152 Leg-2); fix = the 57.111-A3-`{trace}` parallel: a NEW `TransientState.injected_memory` field + `build_memory_block` + a `{memory}` judge-template placeholder + grounding instruction, threaded loop→`_cat10_verify_gate` (gated by default-ON env lever `CHAT_VERIFICATION_MEMORY_GROUNDING`); chose a TransientState field over a Verifier.verify ABC kwarg (0 impls/stubs vs ~13); backend-only NO migration/wire/frontend; parent-direct; real-Azure A/B harness fabrication-catch 0%→100% (+100pp) at 0% false-reject → KEEP default ON (the lenient output_quality judge doesn't false-reject grounded recalls in isolation → the fix's deterministic value is contradiction detection); drive-through STRONG PASS real chat-v2 + Azure gpt-5.2 — a NEW-session 0-keyword recall with an accumulated Dana+Chris memory contradiction was DELIVERED + VerificationPassed 0.98 (vs the pre-fix blind judge that rejected→no-recall); pytest 3082 (+29) · mypy 0/397 · run_all 11/11; CHANGE-120 + design note 56; MERGED PR #360 main `0ce4d1fa` closes `AD-Verification-Judge-Memory-Inject-Blind`); see `memory/` for sprint history
