...
| **Current Sprint** | **Sprint 57.155 — Cat 3 memory semantic axis: user-layer vector recall (CARRY-026 Slice 1) — code + drive-through DONE, PR-pending** (branch `feature/sprint-57-155-memory-vector-recall` from main `c3d1bff1`; **push/PR awaiting user confirmation**). Closes `CARRY-026` Slice 1 (logged 51.2; KB half closed 57.146, memory half open). Wires the long-stubbed `"semantic"` time-scale onto the L4 user layer: NEW `MemoryVectorIndex` (mirrors `KnowledgeVectorIndex` — reuse `EmbeddingClient` ABC + `QdrantVectorStore` + `QdrantNamespaceStrategy("user_memory")`, all pre-existing) gives per-tenant+per-user cosine recall replacing the `[]` stub in `UserLayer.read`. Opt-in `MEMORY_VECTOR_ENABLED` (default OFF → byte-identical); lazy ingest-on-search (per-user count-idempotency, `dedup_key`-derived point ids — does NOT touch the 57.150 write path); fail-soft to keyword/stub. Backend-only NO migration/wire(26)/frontend; parent-direct agent_factor 1.0. **Drive-through PASS** (real chat-v2 + Azure gpt-5.2 + real `text-embedding-3-large` 3072-dim + real Qdrant): `memory vector index built` on the main flow + per-user Qdrant ingest (jamie 5 / priya 2 pts in `tenant_09eb…_user_memory`, Cosine) + count-idempotency + **CLEAN per-user isolation** (priya cannot retrieve jamie's memory-only "Project Lodestar" nor his name despite the shared per-tenant collection). Honest caveat: Leg-2 keyword-disjoint recall is co-supported by 57.148 `profile()` + `knowledge_search` muddied attribution → the semantic axis's DISTINCTIVE value proven at the machinery/isolation layer, not the answer (many-fact A/B deferred `AD-Memory-Vector-Recall-Precision-AB`). 17 new tests · pytest 3123/6skip · mypy `src` 399/0 · run_all 11/11. NEW calib `memory-vector-recall-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-122 + design note 58. (Prior: Sprint 57.154 MERGED PR #362, main `64732999`.) Detail: `memory/project_phase57_155_memory_vector_recall.md`. |
...
**Last Updated**: 2026-07-01 (Sprint 57.155 — Cat 3 memory semantic axis, user-layer vector recall (CARRY-026 Slice 1): wires the long-stubbed `"semantic"` time-scale onto the L4 user layer via a NEW `MemoryVectorIndex` mirroring `KnowledgeVectorIndex` (reuse `EmbeddingClient` ABC + `QdrantVectorStore` + `QdrantNamespaceStrategy("user_memory")`, all pre-existing) → per-tenant+per-user cosine recall replacing the `[]` stub in `UserLayer.read`; opt-in `MEMORY_VECTOR_ENABLED` default OFF (byte-identical), lazy ingest-on-search (per-user count-idempotency + `dedup_key` point ids, write path untouched), fail-soft; backend-only NO migration/wire/frontend; drive-through PASS (real chat-v2 + Azure gpt-5.2 + real text-embedding-3-large 3072-dim + real Qdrant — machinery LIVE + CLEAN per-user isolation jamie↛priya; honest caveat: Leg-2 recall co-supported by 57.148 profile() so the semantic axis's value is proven at the machinery/isolation layer not the answer); 17 new tests · pytest 3123/6skip · mypy `src` 399/0 · run_all 11/11; NEW calib `memory-vector-recall-spike` 0.60; CHANGE-122 + design note 58; **code+drive-through DONE, PR-pending (push awaiting user confirmation)**, branch from main `c3d1bff1`, closes `CARRY-026` Slice 1; prior Sprint 57.154 MERGED PR #362 main `64732999`); see `memory/` for sprint history
