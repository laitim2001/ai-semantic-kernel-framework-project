...
| **Current Sprint** | **Sprint 57.147 — knowledge Slice 3a (per-tenant KB isolation) — code done + full gate green + drive-through PASS, commit PENDING USER PUSH** (branch `feature/sprint-57-147-knowledge-per-tenant-isolation` from main `91ade673`). Threads `tenant_id` end-to-end, **mirroring `memory_search`** (dual-arity `(ToolCall, ExecutionContext)` + forgery guard): per-tenant Qdrant collection (`tenant_<hex>_kb`) + `payload_filter` (defense-in-depth) + per-tenant corpus subfolder (`<root>/<tenant>/`, shared-root fallback) + lazy idempotent per-tenant ingest; `tenant_id=None` = 57.146 byte-identical. Every building block pre-existed (`QdrantNamespaceStrategy` / executor arity auto-dispatch / loop `ExecutionContext(tenant_id=…)`) → backend-only, NO executor/loop/migration/wire(26)/frontend. **Drive-through PASS** (real chat-v2 + real Azure gpt-5.2 + real embedding + real Qdrant, 2 tenants alpha/beta): alpha asks Falcon → only `falcon.md` cited; alpha asks Condor → **0 `condor.md`, "I did not find Project Condor"** (judge 0.98); beta asks Condor → only `condor.md` (Nightjar), 0 `falcon.md` leak; Qdrant 2 distinct collections — bidirectional isolation at BOTH agent-answer + vector-store layers. pytest 2988+6skip · mypy 0/392 · run_all 11/11 · Vitest 922 · mockup 51. NEW calib `knowledge-per-tenant-isolation-spike` 0.55 (1st pt ~1.0-1.15 IN band, parent-direct). CHANGE-114 + design note 51. Closes **isolation half** of `AD-Knowledge-Connector-RBAC-Citation-Slice3` (RBAC 3b + citation 3c open). (Prior: Sprint 57.146 MERGED PR #346, main `ee22375d`.) Detail: `memory/project_phase57_147_knowledge_per_tenant_isolation.md`. |
...
**Last Updated**: 2026-06-27 (Sprint 57.147 — knowledge Slice 3a: per-tenant KB isolation; dual-arity `tenant_id` threading (mirror memory_search) + per-tenant Qdrant collection/payload-filter/corpus + lazy ingest; drive-through PASS — bidirectional isolation A↛B + B↛A on real chat-v2 + real Azure embedding + real Qdrant; CHANGE-114 + design note 51; commit pending user push); see `memory/` for sprint history
