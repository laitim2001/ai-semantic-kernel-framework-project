...
| **Current Sprint** | **No active sprint** (awaiting next-phase selection) — last shipped **Sprint 57.140 MERGED** (PR #333, main `196d8892`; branch from main `313a3e31`) — **explicit todo-list task primitive (research #1)** — closes research #1 `Explicit task primitive` (eval `task-primitive-thin-spike-eval-20260618.md`; 1st net-new after #6/#3/#8/#4). A full vertical slice: Cat 2 `write_todos` tool (replace-whole-list, `risk=LOW`+`destructive=False` → auto-pass) + Cat 3/7 `session_todos` table (migration 0031, 1 row/session, RLS) + `DBTodoStore` (upsert, mirrors DBMessageStore) + `Todo` contract + Cat 5 plan-first nudge + Cat 1 run-start `## Active Plan` re-injection (`loop.run()` async) + `TodosUpdated` emit after write_todos + Cat 12 `todos_updated` wire event (25→26) + chat-v2 Inspector 5th "Todos" tab (existing mockup classes, styles-mockup.css byte-identical). Naming avoids `task` (Cat 11 `task_spawn` collision). ~28 files. pytest 2843+5skip (+20) · mypy 0/380 · run_all 10/10 · Vitest 920 · mockup 51 byte-identical. **Drive-through STRONG PASS** (chat-v2 real Azure gpt-5.2: send 1 → agent proactively `write_todos` 3 items → panel 3/3 completed; send 2 cross-send → rehydrated the 3 + added a 4th → panel 4/4 completed — the agent continued FROM the durable list, AP-4 "ignores the list" risk FALSIFIED). NEW class `task-primitive-spike` 0.60 (1st pt ~0.95-1.0 IN band). CHANGE-107 + design note 44. Detail: `memory/project_phase57_140_task_primitive.md`. |
