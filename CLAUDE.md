...
| **Current Sprint** | Sprint 57.113 (PR-pending) — **Skills System epic OPENED — first thin vertical (model-invoked lazy-load), drive-through ALL 3 legs PASS** (cc-parity row 9, the last systematic CC-parity gap). A system-global `SkillRegistry` (`agent_harness/skills/`, Cat 5) loads bundled `SKILL.md` (frontmatter name+description + body); a cheap `## Available Skills` block rides the `system_prompt` persona seam (`loop.py:1899`); a `read_skill` tool (Cat 2) lazy-loads a skill's full instructions when the model self-selects — `make_default_executor(skill_registry=)` opt-in (the chat path's registry-derived perm matrix auto-PASSes it) + `build_handler`/`router` wiring. 2 REAL bundled skills (`code-review`, `summarize`). No DB / no migration / no new wire event (count 24) / no FE. Gate: mypy 0/366 · run_all 10/10 · full pytest 2566+5skip (+20, 0 del). Drive-through (real chat-v2 + real Azure, dev-login): code-review→`read_skill`→Risks-table / summarize→`read_skill`→Decisions+owner—task / "2+2"→no-trigger — output shapes distinctly followed the loaded skills (load+follow, AP-4 guard) + no false-positive. NEW scope class `skills-system-spike` 0.60 (1st pt ~0.94). Detail: `memory/project_phase57_113_skills_system_spike.md` + CHANGE-080 + design note 31. Next: the carryover pool (`claudedocs/1-planning/next-phase-candidates.md` — Skills per-tenant/slash-command/Inspector, MFA WebAuthn/recovery, SOC 2 + SBOM, FE mockup rebuild). |
...
**Last Updated**: 2026-06-13 (Sprint 57.113 — Skills System epic opened, first thin vertical model-invoked lazy-load, drive-through ALL 3 legs PASS; cc-parity row 9; CHANGE-080 + design note 31); see `memory/` for sprint history
