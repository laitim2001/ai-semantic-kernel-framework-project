...
| **Current Sprint** | Sprint 57.109 (PR-pending) — **C2: compaction cheap tier — SHIPPED + drive-through PASS** (closes proposal §3.4 C2; C-family 3/3). `make_chat_compactor(profile.cheap)` retier (cheap-unset byte-identical; C1 per-tenant policy governs compaction tier too) + `_compaction` cost-ledger attribution (57.82 mirror; usage rides `ContextCompacted` server-side fields per D-DAY1-1 — NOT LoopCompleted, whose 30+ ctor sites would drop cost on early terminations) + `CHAT_COMPACTION_TOKEN_BUDGET`/`CHAT_COMPACTION_KEEP_RECENT_TURNS` env knobs threaded to all 3 compactors (D13). Backend-only: wire count 24 / codegen / FE / DB all untouched; `loop.py` diff = 1 yield site. Gate: mypy 0/359 · run_all 10/10 · full pytest 2485 (+23, 0 del). Drive-through (zero dev-login, real Azure): keep=1 + budget=2000 + ONE B1 mid-run injection → REAL summarize `context_compacted` 9824→2679 (8 msgs, 3.5s) on **gpt-5.4-mini** → priced `_compaction` cost_ledger rows ($0.000195/$0.0006705) → post-compaction answer keeps the injected fact + llm_judge 0.99. **D-DAY3-1 load-bearing: semantic compaction was a latent main-flow NO-OP since 52.1** (chat continuity lives in Cat 3 memory → user-turn cutoff unreachable; all gates green the whole time — only the drive-through caught it) → `AD-Semantic-Compaction-User-Turn-Anchor`. Detail: `memory/project_phase57_109_compaction_cheap_tier.md` + CHANGE-076. Next: `claudedocs/1-planning/next-phase-candidates.md` (B4 per interleave). |
...
**Last Updated**: 2026-06-12 (Sprint 57.109 — C2 compaction cheap tier + `_compaction` ledger attribution SHIPPED, drive-through PASS; CHANGE-076); see `memory/` for sprint history
