...
| **Current Sprint** | Sprint 57.110 (PR-pending) — **B4: subagent child governance + failure policies — SHIPPED + drive-through ALL legs PASS** (closes proposal §2.5 B4 — the LAST mandatory harness-deepening slice; only optional A3 remains). Child loops inherit the tenant-composed Cat 9 engine via late-bound closure (**`loop.py` diff = 0** — ESCALATE-in-child fail-closes to BLOCK by the existing invariant) + truthful `child_guardrail_blocked` label + `GuardrailTriggered` joins the child relay (Tree row renders the fire; count 24, zero codegen) + `SubagentFailurePolicy` fail_fast/soft/partial rides `SubagentBudget` (fail_fast → `SubagentFailureEscalation` Cat 8 FATAL → `LoopTerminated`, exactly ONE spawn; fail_partial salvages partial child output) per-tenant via `HarnessPolicy.subagent_failure_policy` (PUT literal 422). Gate: mypy 0/359 · run_all 10/10 · full pytest 2502+4skip (+19, 0 del) · Vitest 837. Drive-through (zero dev-login, real Azure): child `subprocess` blocked by the INHERITED C3 RiskyActionDetector + Tree row visible; **D-DAY3-1: the child rewrote the blocked call as `os.popen` and EXECUTED it** (C3 deny-list gap — fix-forward +popen/spawn/exec, re-drive shows the child honestly giving up); fail_fast leg terminates the run (no re-spawn) + **D-DAY3-2: `LoopTerminated` is server-side-only** → `AD-LoopTerminated-Wire-Surface`. Detail: `memory/project_phase57_110_subagent_child_governance.md` + CHANGE-077. Next: optional A3 or the carryover pool (`claudedocs/1-planning/next-phase-candidates.md`). |
...
**Last Updated**: 2026-06-13 (Sprint 57.110 — B4 subagent child governance + failure policies SHIPPED, drive-through ALL legs PASS; CHANGE-077); see `memory/` for sprint history
