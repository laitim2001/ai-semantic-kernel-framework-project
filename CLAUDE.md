...
| **Current Sprint** | **Sprint 57.165 — LLM-assisted `--fix` for the tool-description lint — ✅ MERGED (PR #387, main `88e01514`)** (engine-debt **Tool range** final core slice → Tool range **4/4**). Closes ③2 `AD-Tool-Description-AutoFix` (57.144 carryover). The 57.144 lint reported flawed tool/param descriptions but did not draft fixes → opt-in `--fix` drafts one for each flagged tool/param via the neutral cheap-tier `ChatClient` (self-validated against the lint's OWN predicate → retry once, else `<needs-manual>`, never a lint-failing draft); `--fix --write` applies via position-based AST splicing (3 kinds: replace-desc / insert-tool-desc / insert-param-desc). **Apply-mode = Option B** (user AskUserQuestion): dry-run report default, `--write` to apply, `git diff` = 2nd gate. Folded into `check_tool_descriptions.py` (one module — a sibling would fail the importlib file-path test load; lazy backend adapter import → CI report path byte-identical + run_all 11/11 unchanged). NO agent_harness runtime / migration / wire / frontend / **NO drive-through** (dev/CI tooling) — instead a **real-Azure cheap-tier SMOKE**: a 5-gap fixture → 5/5 drafted + `--write` applied + re-lint `OK` (honest findings: drafts use smart quotes + insert param key first → the ⚠ banner + git-diff gate cover them). pytest **3255** (+24, fake-client unit tests, NO Azure) · run_all 11/11 · mypy `src` 400 unaffected (changes outside `src/`) · black/isort/flake8 clean. NEW calib `tool-autofix-spike` 0.60 (1st pt ~1.0-1.08 IN band). CHANGE-132 + design note 64. Carryover: ASCII-normalize drafts / param-key ordering / `AD-Tool-Autofix-Selection-Impact-Phase58` + the 57.164 follow-ons. (Prior: Sprint 57.164 MERGED PR #385, main `980b3357`.) Detail: `memory/project_phase57_165_tool_desc_autofix.md`. |
...
**Last Updated**: 2026-07-10 (Sprint 57.165 — LLM-assisted `--fix` for the tool-description lint: engine-debt Tool range final core slice, closes ③2 AD-Tool-Description-AutoFix → Tool range 4/4. Opt-in `--fix` drafts a description per flagged tool/param via the neutral cheap-tier ChatClient (self-validated against the lint's own predicate → else `<needs-manual>`); `--fix --write` applies via position-based AST splicing (Option B: dry-run default, `--write` to apply, git diff = 2nd gate). Folded into check_tool_descriptions.py (lazy backend import → CI report path byte-identical). NO agent_harness runtime / migration / wire / frontend / NO drive-through (dev/CI tooling) — real-Azure cheap-tier smoke 5/5 PASS. pytest 3255 (+24) · run_all 11/11 · mypy `src` 400 unaffected. CHANGE-132 + design note 64. MERGED PR #387, main `88e01514`. Prior Sprint 57.164 MERGED PR #385 main `980b3357`; see `memory/` for sprint history
