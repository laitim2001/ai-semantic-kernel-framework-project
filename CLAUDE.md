...
| **Current Sprint** | **Sprint 57.160 — tool-anchored observation masking (env-gated) + reduction/retention A/B — MERGED** (PR #376, main `ab9b4cf5`, CI all-green; branch from main `29da11e8`; engine-debt program, compaction range). Fixes `AD-Compaction-NoOp-On-Single-User-Turn-Chat-Path` (57.159 finding): the masker anchored its keep-window on **user-message count**, so the chat main flow (one user msg/send) never masked within a long single-user-turn tool run. Adds an **opt-in tool-result-recency anchor mode** (`DefaultObservationMasker(tool_anchor_keep=N)`: keep last N `role=="tool"` results, tombstone older) via env lever `CHAT_COMPACTION_TOOL_ANCHORED_MASKING` (default OFF → byte-identical; injected into Structural + PreClear). Rolled out evidence-first (env lever + A/B harness — AskUserQuestion pick "B"). **Backend-only, ZERO wire/codegen/frontend/migration** (57.159 marker already renders). parent-direct agent_factor 1.0. A/B (deterministic, real TiktokenCounter, 8 cases): **off 0.00% / on 60.83% / retention 100% / recommend_default_on True**. **Drive-through PASS** real chat-v2 + Azure gpt-5.2 + 6× knowledge_search in ONE user turn (lever + `PRECLEAR_RATIO=0.5` + budget 2500): 57.159 marker renders REAL reductions `13,615→7,933 (−42%)`, `16,199→7,984 (−51%)`; context bounded ~8-9k vs 57.159's 4k→35k; BOREALIS-9 + 6 topics retained. **KEY FINDING**: `StructuralCompactor.tokens_after` is a message-count ratio (`structural.py:192-193`) blind to in-place tombstoning → only `PreClearCompactor` real-counts → tool-anchored masking makes preclear EFFECTIVE on single-user-turn (closes mechanism half of `AD-Compaction-ToolAnchored-Preclear-Phase58`). pytest 3202 (+28) · mypy `src` 400 · run_all 11/11 · Vitest 927 / mockup 51 (FE untouched). NEW calib `compaction-tool-anchored-masking-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-127 + design note 62. Carryover `AD-Compaction-Structural-RealTokenCount`. (Prior: Sprint 57.159 compaction marker PR #374 merged, main `ae388e35`.) Detail: `memory/project_phase57_160_tool_anchored_masking.md`. |
...
**Last Updated**: 2026-07-07 (Sprint 57.160 — env-gated tool-anchored observation masking + reduction/retention A/B: fixes `AD-Compaction-NoOp-On-Single-User-Turn-Chat-Path` (57.159 finding — masker anchored on user-message count, so the chat main flow's single user turn per send never masked within a long tool run). Adds an opt-in tool-result-recency anchor mode (`DefaultObservationMasker(tool_anchor_keep=N)`: keep last N `role=="tool"` results, tombstone older) via env lever `CHAT_COMPACTION_TOOL_ANCHORED_MASKING` (default OFF → byte-identical; injected into Structural + PreClear). Evidence-first (env lever + A/B harness — AskUserQuestion pick "B"). Backend-only, ZERO wire/codegen/frontend/migration. parent-direct agent_factor 1.0. A/B (deterministic, 8 cases): off 0.00% / on 60.83% / retention 100% / recommend True. **Drive-through PASS** real chat-v2 + Azure gpt-5.2 + 6× knowledge_search in ONE user turn: 57.159 marker renders REAL reductions `13,615→7,933 (−42%)`, `16,199→7,984 (−51%)`, context bounded ~8-9k vs 57.159's 4k→35k, BOREALIS-9 + 6 topics retained. KEY FINDING: `StructuralCompactor.tokens_after` message-count ratio (`structural.py:192-193`) is blind to in-place tombstoning → only PreClearCompactor real-counts → tool-anchored masking makes preclear effective on single-user-turn (closes mechanism half of `AD-Compaction-ToolAnchored-Preclear-Phase58`). pytest 3202 (+28) · mypy `src` 400 · run_all 11/11 · Vitest 927 / mockup 51 (FE untouched). NEW calib `compaction-tool-anchored-masking-spike` 0.60 (1st pt ~1.0 IN band). CHANGE-127 + design note 62; carryover `AD-Compaction-Structural-RealTokenCount`; **MERGED PR #376 main `ab9b4cf5`** (branch from main `29da11e8`, engine-debt program compaction range); prior Sprint 57.159 compaction marker PR #374 merged main `ae388e35`; see `memory/` for sprint history
