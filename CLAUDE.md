...
| **Current Sprint** | Sprint 57.125 (PR-pending, NOT pushed) — **chat-v2 session history replay, arc slice 1/2: backend SSE transcript persistence + replay endpoint**. Day-0 re-scoped `AD-ChatV2-SessionList-Backend` (its session-LIST backend was already shipped by 57.107 B3; the residual gap is history-replay); user picked Option B (full SSE persist + replay, ~2-sprint arc). Writer `_persist_main_event` mirrors the 57.107 sidechain observer → persists the EXACT serialized main SSE payload to `message_events` (best-effort SAVEPOINT, `MAIN_TRANSCRIPT_OBSERVER` env-gate, monotonic `main_seq`, NO migration); reader `GET /sessions/{id}/events` ordered (cross-tenant/unknown → 200 `[]`). Backend-only (gate + probe, NOT a UI drive-through — UI is 57.126, NOT pre-written). Live probe: real-LLM chat 16 events, streamed==persisted TRUE. mypy 0/370 · run_all 10/10 · pytest 2711+5skip (+8) · FE unchanged. Detail: `memory/project_phase57_125_chatv2_session_transcript_persistence.md` + CHANGE-092 + design note 37. |
...
**Last Updated**: 2026-06-16 (Sprint 57.125 — chat-v2 session history replay, arc slice 1/2: backend SSE transcript persistence (`_persist_main_event` → `message_events`, mirrors the 57.107 sidechain observer, NO migration) + `GET /sessions/{id}/events` replay endpoint; Day-0 re-scoped `AD-ChatV2-SessionList-Backend` → `AD-ChatV2-Session-History-Replay-Phase58` (57.126 = frontend replay, NOT pre-written); gate + live probe PASS (streamed==persisted 16/16); CHANGE-092 + design note 37); see `memory/` for sprint history
