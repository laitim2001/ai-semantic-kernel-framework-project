...
| **Current Sprint** | Sprint 57.116 (PR-pending) — **Skills epic 1st UX affordance: force-load user-turn "⚡ {skill}" chip — drive-through ALL 3 cases PASS** (closes `AD-Skills-Inspector-Affordance`). 57.115's `/skill-name` force-load was invisible after send → now a **server-confirmed** `active_skill` ADDITIVE field on the opening `loop_start` SSE event (count stays 24; `sse.py` defaults null + `event_wire_schema` declares + codegen regen; the ROUTER injects the validated force-load name at `_stream_loop_events` — `loop.py`/`events.py`/`LoopStarted` dataclass/`read_skill`/resume-mirror UNTOUCHED, Cat-1 boundary clean) drives a chat-v2 `UserTurn.activeSkill` store stamp (truthy-guard last-user-turn, resume-safe) → a `.route-pill` chip (reuses 57.101 injected-tag, no new colour literal). Chip from the SERVER value not the sent field (AP-4 guard). Gate: mypy 0/370 · run_all 10/10 · backend pytest 2623+5skip (+7) · FE Vitest 869 (+6) / mockup 51. Drive-through (real chat-v2 + Azure gpt-5.2, acme-skills): A `/release-notes`→chip + output follows + **`read_skill` 0×** / B `/nonexistent`→no chip (router dropped) / C plain→no chip. `frontend-feature-with-event-wire-addition` 0.55 3rd pt ~1.1-1.2 IN band (KEEP, validated). NO design note (feature continuation). Detail: `memory/project_phase57_116_skills_inspector_affordance.md` + CHANGE-083. Next: carryover pool (`claudedocs/1-planning/next-phase-candidates.md` — Skills authoring/quota, Inspector-panel metadata row, MFA WebAuthn/recovery, SOC 2 + SBOM, FE mockup rebuild). |
...
**Last Updated**: 2026-06-14 (Sprint 57.116 — Skills epic 1st UX affordance: force-load user-turn skill chip via a server-confirmed `active_skill` additive field on `loop_start` (count 24, `loop.py`/`events.py` untouched), drive-through ALL 3 cases PASS, closes AD-Skills-Inspector-Affordance; CHANGE-083, no design note); see `memory/` for sprint history
