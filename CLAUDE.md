...
| **Current Sprint** | **Sprint 57.151 — cross-session conversation recall (session summaries) — PR-pending (NOT pushed; awaiting user push confirmation)** (branch `feature/sprint-57-151-memory-session-recall` from main `f664f34d`). Closes `AD-Memory-Formation-Session-Recall` (缺口 2, the memory-formation arc's conversation-recall slice). The arc (57.148 identity / 57.149 auto-extract / 57.150 dedup) recalled discrete user FACTS but NOT what was *discussed/decided* in a prior session. **Day-0 三-prong MAJOR catch** (`D-table-already-exists`): plan v0 invented a `memory_session` table; grep found `memory_session_summary` ALREADY designed + created (`0007` / `09-db-schema-design.md:481`, Layer 5 持久化, richer cols `session_id UNIQUE`+`summary`+`key_decisions`/`unresolved_issues` JSONB) → REUSE it (Check-Existing 鐵律, saved ~1.5 hr + avoided AP-3 duplicate-table + wrong-RLS migration). Fix: additive `updated_at` (migration **0033**, revision id ≤32 chars — Day-1 `D-revision-id-len`) + `DBSessionSummaryStore` (upsert on session_id UNIQUE = one rolling row, mirrors 57.150; `recent_for_user` JOINs `sessions` for tenant+user scope + exclude-current + set_config since sessions is FORCE RLS) + `SessionSummarizer` (cheap-tier, structured-JSON, rides the 57.149 post-send BackgroundTask seam, AP-8 allowlisted like extraction.py) + `MemoryRetrieval.recent_sessions()` (mirrors 57.148 `profile()`) injected into `PromptBuilder.build()`. One env flag `chat_session_summary` (default ON) gates BOTH formation + recall → byte-identical off. Backend-only, NO new table/wire(26)/frontend. **Drive-through STRONG PASS** (real chat-v2 + Azure gpt-5.2): Leg 1 dan's session A (billing MongoDB→Postgres / dual-write / invoices JSONB-vs-table) → `memory_session_summary` row all 3 cols populated; Leg 2 NEW session B recalls A's arc verbatim (Verification 0.98) + coexists w/ 57.149 Aurora user fact; Leg 3 priya (same tenant) 0 leak. pytest 3042 (+20) · mypy 0/396 · run_all 11/11 · migration up/down/up clean · FE untouched. NEW calib `memory-session-recall-spike` 0.60 (1st pt ~1.05 IN band). CHANGE-118 + design note 54. (Prior: Sprint 57.150 MERGED PR #354, main `32ae4489`.) Detail: `memory/project_phase57_151_memory_session_recall.md`. |
...
**Last Updated**: 2026-06-30 (Sprint 57.151 — cross-session conversation recall: fills the designed-but-unwired `memory_session_summary` table — rolling `SessionSummarizer` (cheap-tier, rides the 57.149 post-send seam) upserts `{summary, key_decisions, unresolved_issues}` per session; `MemoryRetrieval.recent_sessions()` (mirrors 57.148 profile()) injects the user's recent prior-session arcs into a NEW session (JOIN sessions for tenant+user scope, exclude current); migration 0033 adds `updated_at`; one `chat_session_summary` flag default ON byte-identical off; Day-0 三-prong MAJOR catch reused the designed table not a new one; drive-through STRONG PASS real chat-v2 + Azure gpt-5.2 — dan's billing-migration session A → new session B recalls it verbatim, priya 0 leak; CHANGE-118 + design note 54; PR-pending (NOT pushed) closes `AD-Memory-Formation-Session-Recall` 缺口 2); see `memory/` for sprint history
