...
- 2026-06-15（Sprint 57.121）：`page-chat.jsx` 加 `SkillMenu`（`/`-slash skill picker，浮於 Composer 上）+ `styles.css` 加 `.skill-menu*` class（CommandPalette-consistent：panel + subtle `--bg-hover` active + `.kbd` footer，token-only 無 literal、無 emoji）；Composer 改互動式（`/`-trigger / ↑↓/↵/ESC filter）以 demo。補上 production `SkillSlashMenu.tsx`（57.115 greenfield）唯一缺 mockup 來源的 chat-v2 元素 → 已 re-point 到本 mockup（CHANGE-088）
