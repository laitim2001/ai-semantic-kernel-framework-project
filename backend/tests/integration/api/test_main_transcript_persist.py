...
async def test_main_transcript_persists_ordered_events(
    ...
    # TurnStarted + LLMRequested + LoopCompleted persisted; Thinking skipped (None).
    ...
    assert events[0].event_data["turn_num"] == 0
...
async def test_main_and_sidechain_rows_separate_by_session_id(
    ...
    assert sorted(e.event_type for e in main_rows) == ["llm_request", "loop_end", "turn_start"]
    assert len(side_rows) >= 1
    assert all(e.event_type == "subagent_child" for e in side_rows)
    # No collision: main rows own a clean 1..3 sequence space of their own.
    assert {e.sequence_num for e in main_rows} == {1, 2, 3}
