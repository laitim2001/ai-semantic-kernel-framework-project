...
## Phase 45-47 Maturity Map Additions (2026-04-19 sync)

### New Modules — Maturity Classification

| Module | Path | Maturity | Evidence |
|--------|------|----------|----------|
| **Orchestration Pipeline** | `integrations/orchestration/pipeline/` | **REAL (PRODUCTION)** | `OrchestrationPipelineService` with real step execution, Redis Streams transcript, PG checkpoint, E2E tests passing (Sprint 158 `f1923d3`) |
| **Orchestration Dispatch** | `integrations/orchestration/dispatch/` | **REAL (PRODUCTION)** | 3 concrete executors (direct_answer/subagent/team); TeamExecutor delegates to PoC `run_parallel_team` which is real-LLM-backed |
| **Agent Expert Registry** | `integrations/orchestration/experts/` + `api/v1/experts/` | **REAL (PRODUCTION)** | YAML seeding + PG persistence via `AgentExpert` ORM; 6 builtin experts seeded on startup; three-tier fallback (DB → YAML → legacy worker_roles) |
| **Approval Service** | `integrations/orchestration/approval/service.py` | **REAL** | Integrates with Step 5 HITLGateStep; raises `HITLPauseException` with checkpoint |
| **Transcript Service** | `integrations/orchestration/transcript/service.py` | **REAL** | Redis Streams-backed per-session transcript |
| **Resume Service** | `integrations/orchestration/resume/service.py` | **REAL** | Checkpoint-based resume from paused step |
| **PoC Agent Team V4** | `integrations/poc/` + `api/v1/poc/` | **EXPERIMENTAL (PRODUCTION-ADJACENT)** | Real Claude SDK-backed agents, real Redis SharedTaskList, event-driven HITL. Flagged as PoC by directory name but invoked from production TeamExecutor |
| **AnthropicChatClient** | `integrations/agent_framework/clients/anthropic_chat_client.py` | **REAL** | Uses `AsyncAnthropic`; full MAF compatibility via `FunctionInvocationLayer` |
| **Pipeline Execution Log** | `integrations/orchestration/pipeline/persistence.py` + `infrastructure/database/models/orchestration_execution_log.py` | **REAL (PRODUCTION, Phase 47 W1)** | PG-backed persistence of full memory/knowledge text + step metadata |
| **IPA Checkpoint Storage** | `integrations/agent_framework/ipa_checkpoint_storage.py` | **REAL** | IPA-specific MAF checkpoint storage adapter |

### Memory Module Enhancements

| Sub-module | Path | Maturity |
|------------|------|----------|
| `memory/consolidation.py` | NEW | **REAL** — episodic → semantic consolidation |
| `memory/context_budget.py` | NEW | **REAL** — token budget allocation |
| `memory/extraction.py` | NEW | **REAL** — extraction from transcripts |

### Updated Maturity Summary

| Category | V9 Baseline | Post-Phase 47 |
|----------|-------------|---------------|
| **Fully Real (production)** | 15 modules | **~20 modules** (+pipeline, +dispatch, +experts, +approval/transcript/resume, +agent_framework/clients) |
| **Real with InMemory risk** | 8 modules | 8 modules (no change — InMemory issues in hybrid/orchestration legacy not yet fully migrated) |
| **Experimental (PoC)** | 0 | **1 module** (poc/ — real but labeled experimental) |
| **Stub** | 2 modules | 2 modules (messaging still STUB) |

### Flagged Items for Next Deep-Dive

- `poc/` module naming suggests experimental, but actual code is production-quality and used by `TeamExecutor`. Consider promoting to `integrations/agent_team/` in a future refactor.
- `execution_log_routes.py` and `execution_log_schemas.py` are Phase 47 W1 files; full schema column list not yet inventoried in V9 — manual follow-up recommended.
- Frontend `agent-team/` directory is real (Zustand-backed), but 15 components inherited from `agent-swarm/` via rename — behavioral equivalence not yet unit-tested end-to-end on new names.

---

*Generated by V9 Mock/Real Analysis — 2026-03-29*
*Phase 45-47 additions appended 2026-04-19 from actual source reading of new modules.*
