...
│               Front  API AG-UI Route Orch  MAF Claude MCP Integ Domain Infra│
│               ─────  ── ──── ───── ──── ─── ──── ─── ───── ───── ────     │
│  🛡️ Security   │    ██   │    ██   ██   │    │    ██   │    │    ██        │
│  JWT+RBAC      │   Auth  │  Risk  HITL  │    │  Perm   │    │   DB         │
│                │         │  Assess Approv│    │  Check  │    │              │
│                │         │              │    │         │    │              │
│  ⚡ Performance ██   ██   │    │    │    ██   │    │    │    │    ██        │
│  Cache+Pool   React Pool  │    │    │  Check  │    │    │    │  Redis       │
│               Lazy  Limit │    │    │  point  │    │    │    │  PG Pool     │
│                │         │              │    │         │    │              │
│  📊 Observability│   ██   ██   │    ██   │    │    ██   │    │    ██        │
│  Logs+Metrics  │  Route AG-UI  │  SSE    │    │  Audit  │    │  Monitor     │
│               │  Logs Events  │  Events │    │  Log    │    │              │
│                │         │              │    │         │    │              │
│  💾 Persistence│   │    │    │    │    ██   │    │    │    ██   ██          │
│  DB+Cache     │   │    │    │    │  Check  │    │    │   InMem  PG+Redis   │
│               │   │    │    │    │  point  │    │    │   ⚠Risk            │
│                │         │              │    │         │    │              │
│  🔄 Error      ██   ██   ██   │    ██   ██   ██   │    │    │    │         │
│  Handling     React HTTP SSE   │  Retry Adapt Fallbk  │    │    │         │
│                                                                             │
│  ██ = 該層有實作    │ = 該層未涉及                                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 安全防線多層架構 (Defense in Depth)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│              IPA Platform 安全防線 — 由外到內                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  外部請求                                                                   │
│     │                                                                       │
...
│  │  core/security/jwt.py — HS256 token 驗證                      │          │
│  │  每個 API 請求必須攜帶 valid JWT                               │          │
│  └───────────────────────────────────────────────────┬───────────┘          │
│                                                      │ ✓ 身份已驗證        │
...
│                                                      │ ✓ 權限已確認        │
│  ┌──── Layer 3: PromptGuard ─────────────────────────┤───────────┐          │
│  │  orchestration/prompt_guard/ — 提示注入偵測         │           │          │
│  │  分析用戶輸入是否含惡意 prompt injection             │           │          │
│  └───────────────────────────────────────────────────┤───────────┘          │
│                                                      │ ✓ 輸入已淨化        │
│  ┌──── Layer 4: ToolGateway ─────────────────────────┤───────────┐          │
...
│                                                      │ ✓ 工具已授權        │
│  ┌──── Layer 5: Sandbox ─────────────────────────────┤───────────┐          │
│  │  core/sandbox/ — 程式碼執行沙箱                    │           │          │
│  │  隔離環境 + 資源限制 + 超時控制                    │           │          │
│  └───────────────────────────────────────────────────┤───────────┘          │
│                                                      │ ✓ 執行已隔離        │
│  ┌──── Layer 6: Audit Trail ─────────────────────────┤───────────┐          │
│  │  mcp/security/audit.py — 12 事件類型 (5 類別)       │           │          │
│  │  + integrations/audit/ — 決策追蹤                  │           │          │
│  │  全程操作記錄 (但部分 InMemory ⚠)                  │           │          │
│  └───────────────────────────────────────────────────────────────┘          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
