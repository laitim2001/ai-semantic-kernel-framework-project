...
*Analysis generated by reading 89 source files from `backend/src/integrations/hybrid/`. All code references are based on the actual implementation as of the current branch (`feature/phase-42-deep-integration`).*

---

## Phase 45-47 Orchestration Additions (2026-04-19 sync)

> **Important**: Phase 45 introduces a **new unified orchestration pipeline** at `backend/src/integrations/orchestration/pipeline/` that runs alongside the legacy `hybrid/orchestrator/` described above. New `chat_routes.py` at `api/v1/orchestration/chat_routes.py` powers the new `OrchestratorChat` frontend page — the legacy `hybrid/` Mediator continues to serve existing endpoints. Both coexist in production as of commit `69b5fa2`.

### A. Unified Pipeline Architecture (Phase 45 Sprint 153-156)

**New directory**: `backend/src/integrations/orchestration/pipeline/` (14 files, ~2,971 LOC)

**Class hierarchy**:
```
OrchestrationPipelineService (pipeline/service.py:92-420)
  ├─ configure_steps(steps: List[PipelineStep])
  └─ run(context, event_queue?, start_from_step=0) -> PipelineContext

PipelineStep (ABC, pipeline/steps/base.py:23)
  ├─ MemoryStep       (step1_memory.py,       index=0)
  ├─ KnowledgeStep    (step2_knowledge.py,    index=1)
  ├─ IntentStep       (step3_intent.py,       index=2)  — 3-tier cascade
  ├─ RiskStep         (step4_risk.py,         index=3)
  ├─ HITLGateStep     (step5_hitl.py,         index=4)
  ├─ LLMRouteStep     (step6_llm_route.py,    index=5)  — fast-path for QUERY/REQUEST conf ≥ 0.92
  └─ PostProcessStep  (step8_postprocess.py,  index=7)  — checkpoint + memory extraction
     (step index 6 is not a numbered pipeline step; dispatch happens as part of Step 6 outcome)
```

**Key classes and support services**:
- `PipelineContext` (dataclass) at `pipeline/context.py`
- `DialogPauseException`, `HITLPauseException`, `PipelineError` at `pipeline/exceptions.py`
- `PipelineExecutionPersistenceService` at `pipeline/persistence.py` (Phase 47 W1 — persists to `orchestration_execution_log` table)
- `ApprovalService` at `approval/service.py` (integrates with Step 5)
- `TranscriptService` at `transcript/service.py` (Redis Streams per-session transcript, uses `TranscriptEntry`)
- `ResumeService` at `resume/service.py` (checkpoint-based resume)

**Pause/resume flow**:
1. `HITLGateStep` raises `HITLPauseException` on HIGH/CRITICAL risk → checkpoint saved → SSE emits `HITL_REQUIRED`
2. Frontend displays HITL approval UI; user approves/rejects
3. Frontend calls `POST /orchestration/chat/resume` with `ResumeRequest.approval_status`
4. `ResumeService` re-enters pipeline at `start_from_step` from saved checkpoint

### B. Dispatch Layer (Phase 45 Sprint 155)

**New directory**: `backend/src/integrations/orchestration/dispatch/` (12 files, ~2,210 LOC)

**`ExecutionRoute` enum** (`dispatch/models.py:13-25`):
| Value | Purpose | Executor |
|-------|---------|----------|
| `DIRECT_ANSWER` | Simple Q&A, no tool calls | `DirectAnswerExecutor` |
| `SUBAGENT` | Single specialized agent | `SubagentExecutor` — includes Sprint 166 `_infer_complexity()` with `MAX_SUBTASKS=10` cap |
| `TEAM` | Multi-agent team (default fallback) | `TeamExecutor` — delegates to PoC `run_parallel_team()` |

**`DispatchService`** (`dispatch/service.py:20-96`):
- `register_executor(route, executor)` — custom executor registration (supports extension)
- `dispatch(request, event_queue?)` — emits `DISPATCH_START` SSE event, then delegates to appropriate executor

**Integration adapters**:
- `EventQueueAdapter` (executors/event_adapter.py)
- `PipelineEmitterBridge` (executors/pipeline_emitter_bridge.py) — maps PoC `SWARM_*` events → pipeline `AGENT_*` events
- `TeamAgentAdapter` (executors/team_agent_adapter.py) — wraps per-agent for team runner
- `TeamToolRegistry` (executors/team_tool_registry.py) — per-agent tool scoping

### C. Agent Expert Registry (Phase 46 Sprint 158-163)

**New directory**: `backend/src/integrations/orchestration/experts/` + 6 YAML definitions at `experts/definitions/`

Provides `TeamExecutor` with expert definitions for task decomposition. See `mod-integration-batch2.md` Phase 45-47 section for full inventory; summary:

- `AgentExpertRegistry` (singleton, `registry.py`) with three-tier fallback: DB → YAML → legacy `worker_roles`
- `AgentExpertDefinition` dataclass (`registry.py:37`)
- `VALID_DOMAINS` frozenset: `{network, database, application, security, cloud, general, custom}`
- `TEAM_TOOLS` (6 collaboration tools) and `DOMAIN_TOOLS` dict (per-domain specialised + core tools)
- `resolve_tools(tools_list, domain)` resolves `"*"` (all tools) and `"@domain"` tokens

### D. Legacy vs New — Coexistence

The legacy `hybrid/orchestrator/` `OrchestratorMediator` (7-handler pipeline) remains active. The new Phase 45 pipeline is **additive**, not replacing:

| Entry point | Pipeline Used |
|-------------|---------------|
| `POST /orchestration/chat` (Phase 45 new) | **New** `OrchestrationPipelineService` |
| `POST /workflows/execute` and existing endpoints | **Legacy** `OrchestratorMediator` (7-handler pipeline) |
| `POST /poc/agent-team/test_*` | Direct to `run_parallel_team()` (bypasses both) |

> Reconciliation of legacy vs new orchestrator is planned for future phases but not completed in Phase 45-47.

---

*Phase 45-47 additions appended 2026-04-19 from `integrations/orchestration/pipeline/`, `dispatch/`, `experts/`, `approval/`, `transcript/`, `resume/` source reading.*
