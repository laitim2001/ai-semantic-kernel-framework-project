# Bash Execution Infrastructure

> Consolidated from deep analysis. Authoritative reference for Claude Code's shell execution model, pure-TS bash parser, AST-based security engine, command prefix extraction, PowerShell support, and platform differences.
> Source: `src/utils/bash/` (22 files), `src/utils/shell/` (10 files), `src/utils/powershell/` (3 files) = 35 files total
...
### `parseForSecurity()` -- The Core Function
...
### Post-Argv Semantic Checks (checkSemantics)
...
### Static Prefix (`prefix.ts` + `specPrefix.ts`)
...
### LLM-Powered Prefix (`shell/prefix.ts`)
...
### Embedded Search Tool Integration
...
### PowerShell Provider (`powershellProvider.ts`)
...
### Shell Resolution (`resolveDefaultShell.ts`)
...
### CommandElementType Classification
...
## 16. Cross-Cutting Design Themes
...
### Parser Differential Mitigation
...
### Placeholder-Based Variable Tracking
...
| Feature | Bash (Unix) | PowerShell (Windows) |
|---------|-------------|---------------------|
| Shell path | `/bin/bash` or `$SHELL` | Auto-detected |
| Spawn args | `['-c', '-l', cmd]` | Provider-specific |
| Output file | `O_NOFOLLOW` for security | String flags (no `O_NOFOLLOW`) |
| CWD tracking | File-based via `cwdFilePath` | Provider handles |
| Sandbox | Full macOS sandbox support | Limited |
| Process detach | `detached: true` | Provider-specific |
| Parser | Pure-TS (in-process, ~0ms) | Spawns `pwsh` (~450ms warm) |

---

## 18. Exported Functions Summary
