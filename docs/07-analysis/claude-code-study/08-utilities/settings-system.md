...
> Layered configuration pipeline with 6+ sources, Zod v4 schema validation, filesystem watching, and MDM enterprise integration.
> Source: `src/utils/settings/` (16 files) + `src/utils/settings/mdm/` (3 files) = 19 files total, ~2,800 LOC
...
- **Feature gates**: Fields conditionally included based on `feature()` flags
- **`z.preprocess()`**: Used for `strictPluginOnlyCustomization` to silently drop unknown surface names
...
|----------|-------------|
| `getInitialSettings()` | Returns merged settings snapshot (cached) |
| `getSettingsWithErrors()` | Merged settings + validation errors (cached) |
| `getSettingsWithSources()` | Effective settings + per-source breakdown (fresh read) |
| `getSettingsForSource(source)` | Single source settings (cached) |

### Settings Modification
| Function | Description |
|----------|-------------|
| `updateSettingsForSource(source, settings)` | Merge + write to source file |
| `applySettingsChange(source, setAppState)` | Apply change to AppState |

### Validation
| Function | Description |
|----------|-------------|
| `validateSettingsFileContent(content)` | Validate JSON against strict schema |
| `validatePermissionRule(rule)` | Validate single permission rule |
| `generateSettingsJSONSchema()` | Full JSON Schema from Zod |

### Change Detection
| Function | Description |
|----------|-------------|
| `settingsChangeDetector.initialize()` | Start filesystem watcher + MDM poll |
| `settingsChangeDetector.dispose()` | Stop watcher and timers |
| `settingsChangeDetector.subscribe(cb)` | Subscribe to settings changes |
| `resetSettingsCache()` | Invalidate all caches |
