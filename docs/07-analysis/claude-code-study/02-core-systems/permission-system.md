...
> **Consolidated from deep analysis** — Verified against 24 files in `utils/permissions/`, 6 files in `hooks/toolPermission/`, 8 files in `components/permissions/rules/`, plus `tools/BashTool/bashPermissions.ts` and `bashSecurity.ts`. Corrects errors from earlier analysis passes (M-10, M-11, M-12, L-03, L-04).

> Permission modes, rule engine, per-tool checks, classifier integration, and safety architecture.
...
The immutable permission context carried through every tool invocation.
...
type ToolPermissionContext = {
  ...
  readonly shouldAvoidPermissionPrompts?: boolean     // Background agents
  readonly awaitAutomatedChecksBeforeDialog?: boolean  // Coordinator workers
  readonly prePlanMode?: PermissionMode               // Saved mode before plan entry
}
```

Note: Uses direct `readonly` field modifiers, NOT a `DeepImmutable<>` wrapper. No `isAutoModeAvailable` field exists — only `isBypassPermissionsModeAvailable`.

## 14. Permission Decisions
...
### Decision Reasons (11 variants)

Every decision includes a reason for audit trailing:

| Reason Type | When |
|-------------|------|
| `rule` | Matched a permission rule |
| `mode` | Decision based on permission mode |
| `subcommandResults` | Compound command sub-decisions (Map of per-subcommand results) |
| `permissionPromptTool` | Decision from a permission prompt tool (includes `permissionPromptToolName` + `toolResult`) |
| `hook` | Hook script decided (includes `hookName`, `hookSource?`, `reason?`) |
| `asyncAgent` | Async agent restriction |
| `sandboxOverride` | Sandbox excluded command (`excludedCommand` or `dangerouslyDisableSandbox`) |
| `classifier` | Auto-mode classifier decided |
| `workingDir` | Working directory restriction |
| `safetyCheck` | Safety check triggered (with `classifierApprovable` boolean) |
| `other` | Miscellaneous |

## 15. Permission Updates
...
Destinations determine persistence: `userSettings`, `projectSettings`, `localSettings`, `session`, `cliArg`.
...
| File | Purpose |
|------|---------|
| `src/types/permissions.ts` | All permission type definitions |
| `src/utils/permissions/permissions.ts` | Core permission checking logic |
| `src/utils/permissions/permissionSetup.ts` | Permission context setup |
| `src/utils/permissions/permissionsLoader.ts` | Rule loading from sources |
| `src/utils/permissions/denialTracking.ts` | Denial tracking state |
| `src/utils/permissions/permissionRuleParser.ts` | Rule format parsing |
| `src/utils/permissions/shellRuleMatching.ts` | Shell command rule matching |
| `src/utils/permissions/shadowedRuleDetection.ts` | Unreachable rule detection |
| `src/utils/permissions/classifierDecision.ts` | Auto-mode classifier decisions |
| `src/utils/permissions/yoloClassifier.ts` | YOLO classifier implementation |
| `src/utils/permissions/permissionExplainer.ts` | Human-readable explanations |
| `src/utils/permissions/bypassPermissionsKillswitch.ts` | Killswitch controls |
| `src/utils/permissions/permissionLogging.ts` | Analytics and OTel events |
| `src/tools/BashTool/bashPermissions.ts` | Bash-specific permission logic |
| `src/tools/BashTool/bashSecurity.ts` | 23 security check IDs |
| `src/hooks/useCanUseTool.tsx` | React hook for permission UI |
| `src/hooks/toolPermission/` | Tool permission UI hooks and handlers |
| `src/components/permissions/` | Permission UI components |

---

*Analysis based on source files verified 2026-04-01. All function names, step numbers, and behaviors verified against actual source code.*
