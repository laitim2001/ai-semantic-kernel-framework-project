# Ink Terminal UI — Claude Code CLI

> Consolidated from base analysis + deep verification (2026-04-01, 9.1/10 quality).
> Source: `src/ink/` — Custom Ink reimplementation with React 19 reconciler, Yoga layout, ANSI parser, and terminal events.
...
```mermaid
graph TD
    React["React 19 Components"] --> Reconciler["Custom Reconciler"]
    Reconciler --> DOM["Virtual DOM (ink dom.ts)"]
    DOM --> Layout["Yoga Layout Engine"]
    Layout --> Render["render-node-to-output.ts"]
    Render --> Output["ANSI Output Buffer"]
    Output --> Screen["render-to-screen.ts"]
    Screen --> Terminal["Terminal (stdout)"]
```
...
### 9.1 Terminal Events (`src/ink/events/`)

Event system for terminal input:
- **Keypress parsing** (`parse-keypress.ts`): Converts raw terminal bytes to structured key events
- **Mouse events**: Click, scroll, hover detection
- **Focus tracking** (`terminal-focus-state.ts`): Terminal focus/blur events
- **Resize handling**: Dynamic terminal size adaptation

### 9.2 Terminal Querier (`src/ink/terminal-querier.ts`)

Queries terminal capabilities:
- Color depth detection
- Kitty keyboard protocol support
- Sixel/image protocol detection
- Terminal emulator identification

### 9.3 Termio Layer (`src/ink/termio/`, `src/ink/termio.ts`)
...
### 9.4 ANSI Parser (`src/ink/Ansi.tsx`)

Full ANSI escape sequence parser supporting:
- SGR (color/style) sequences
- 256-color and true-color (24-bit) support
- Hyperlink sequences (`\e]8;;url\e\\text\e]8;;\e\\`)
- Bidirectional text handling (`src/ink/bidi.ts`)

---

## 10. Component System

### 10.1 Built-in Ink Components (`src/ink/components/`)

| Component | Purpose |
|-----------|---------|
| `Box` | Flexbox container (maps to Yoga node) |
| `Text` | Styled text with color, bold, italic, etc. |
| `Static` | Non-rerendering content (previous messages) |
| `Transform` | Text transformation wrapper |
| `Spacer` | Flexible space filler |
| `Newline` | Line break element |

### 10.2 Hooks (`src/ink/hooks/`)

| Hook | Purpose |
|------|---------|
| `useInput` | Subscribe to keyboard input |
| `useStdin` | Raw stdin access |
| `useStdout` | Stdout dimensions and write access |
...
### 11.1 Hit Testing (`src/ink/hit-test.ts`)

Maps terminal coordinates (x, y) back to the virtual DOM tree for:
- Click handling on interactive elements
- Hover state management
- Selection tracking

### 11.2 Selection (`src/ink/selection.ts`)

Text selection support:
- Character-level selection tracking
- Copy-to-clipboard integration
- Selection highlighting in render output

### 11.3 Search Highlighting (`src/ink/searchHighlight.ts`)

In-terminal search with:
- Pattern matching across rendered output
- Highlighted matches with navigation
- Integration with transcript search

### 11.4 Terminal Notifications (`src/ink/useTerminalNotification.ts`)

OS-level notifications via:
- iTerm2 notification protocol
- Kitty notification protocol
- Ghostty notification protocol
- Terminal bell fallback

### 11.5 Hyperlink Support (`src/ink/supports-hyperlinks.ts`)

Detects terminal hyperlink capability and renders clickable links using OSC 8 sequences when supported.
...
### 14.2 Zero-Allocation Rendering
...
| File | Purpose |
|------|---------|
| `src/ink/reconciler.ts` | React 19 custom reconciler |
| `src/ink/dom.ts` | Virtual DOM node types |
| `src/ink/layout/` | Yoga layout integration |
| `src/ink/renderer.ts` | Main render coordinator |
| `src/ink/screen.ts` | Packed screen buffer with damage tracking |
| `src/ink/render-node-to-output.ts` | Node-to-ANSI conversion + viewport culling |
| `src/ink/render-to-screen.ts` | Diff-based screen updates |
| `src/ink/Ansi.tsx` | ANSI escape parser |
| `src/ink/events/dispatcher.ts` | Two-phase event dispatch |
| `src/ink/parse-keypress.ts` | Keypress parsing |
| `src/ink/terminal-querier.ts` | Terminal capability detection |
| `src/ink/hooks/` | React hooks for terminal interaction |
| `src/ink/components/ScrollBox.tsx` | Primary scroll container |
| `src/ink/components/` | Built-in UI components |
