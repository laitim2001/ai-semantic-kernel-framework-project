...
> **Scope**: `src/services/lsp/` (7 files) + `src/tools/LSPTool/` (6 files)
> **Date**: 2026-04-01
...
The `ScopedLspServerConfig` type (from missing `types.ts`, inferred from usage) includes:
...
4. **Missing types.ts**: The `types.ts` file referenced by imports is absent from the source snapshot (possibly generated or in a separate build step)
