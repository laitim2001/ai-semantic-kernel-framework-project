# Wave 52: coordinator/, assistant/, server/ Deep Analysis

> **Date**: 2026-04-01
> **Scope**: 5 files across 3 modules (coordinator/coordinatorMode.ts, assistant/sessionHistory.ts, server/*.ts)
> **Quality**: Deep read of all source lines

---

## 1. coordinator/coordinatorMode.ts (369 LOC)
...
## 2. assistant/sessionHistory.ts (88 LOC)
...
None directly. Depends on OAuth being configured (via `prepareApiRequest()` which requires valid OAuth tokens).

### How It's Loaded

- **Eager import** -- no lazy loading or feature gates. Available whenever the assistant module is used.
- Uses `axios` for HTTP (unlike most of the codebase which uses native `fetch`).
...
## 3. server/ (3 files, 271 LOC total)

### 3a. server/types.ts (57 LOC)
...
- Uses `lazySchema()` wrapper for the Zod schema -- **lazy initialization** to avoid import-time parsing cost.
- Types are pure TypeScript (zero runtime cost).

#### Architecture Role

These types define the **contract** between Direct Connect clients and servers. The `SessionIndex` type reveals that sessions persist across server restarts via `~/.claude/server-sessions.json`, supporting the `--resume` flow with `transcriptSessionId`.

### 3b. server/createDirectConnectSession.ts (89 LOC)
...
None. Available whenever the server module is imported.
...
- **Eager import**, no feature gates. Uses native `fetch` (not axios).
- Validates response with `connectResponseSchema().safeParse()`.

#### Architecture Role

This is the **client-side session factory** for Direct Connect mode. The flow:
1. POST to `/sessions` with `{ cwd, dangerously_skip_permissions? }`
2. Validate response against `connectResponseSchema`
3. Return `DirectConnectConfig` (serverUrl, sessionId, wsUrl, authToken)

The `dangerouslySkipPermissions` flag maps to the `--dangerously-skip-permissions` CLI flag, disabling the permission system entirely for automated/CI environments.

### 3c. server/directConnectManager.ts (214 LOC)
...
None directly. The entire server module is only activated when Claude Code is started in Direct Connect mode.
...
This is the **real-time communication backbone** for Direct Connect mode. It implements the full bidirectional protocol:

- **Client -> Server**: User messages, permission responses, interrupt signals
- **Server -> Client**: SDK messages (streamed responses), permission requests
