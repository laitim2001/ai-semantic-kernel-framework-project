# Wave 47: Deep Analysis of `utils/model/` — Model Selection & Routing Utilities

> Source: `src/utils/model/` (16 files, ~92 KB)
> Analysis date: 2026-04-01
> Verifies and supersedes: `model-selection.md` (Wave 23)
...
**Total**: ~2,525 lines of TypeScript
...
## 17. 3P Model Support Overrides (`modelSupportOverrides.ts`)

### 17.1 Override Capabilities
...
## 18. Model Validation (`validateModel.ts`)
...
## 19. Ant-Internal Model System (`antModels.ts`)
...
## 20. Model Picker Options (`modelOptions.ts`)

### 20.1 Tier-Specific Option Lists
...
## 21. Complete Exported Functions Index
...
### `configs.ts` (7 exports)

| Export | Type |
|--------|------|
| `ALL_MODEL_CONFIGS` | Registry constant |
| 11x `CLAUDE_*_CONFIG` | Individual model configs |
| `ModelKey` | Type |
| `CanonicalModelId` | Type |
| `CANONICAL_MODEL_IDS` | Runtime array |
| `CANONICAL_ID_TO_KEY` | Reverse lookup |
| `ModelConfig` | Type |

### `aliases.ts` (4 exports)

| Export | Type |
|--------|------|
| `MODEL_ALIASES` | Constant array |
| `ModelAlias` | Type |
| `isModelAlias(input)` | Type guard |
| `MODEL_FAMILY_ALIASES` | Constant array |
| `isModelFamilyAlias(model)` | Boolean check |

### `modelOptions.ts` (7 exports)

| Export | Purpose |
|--------|---------|
| `getModelOptions(fastMode)` | Complete model picker list |
| `getDefaultOptionForUser(fastMode)` | Default option for current tier |
| `getSonnet46_1MOption()` | Sonnet 4.6 1M option |
| `getOpus46_1MOption(fast)` | Opus 4.6 1M option |
| `getMaxSonnet46_1MOption()` | Max/subscriber Sonnet 1M option |
| `getMaxOpus46_1MOption(fast)` | Max/subscriber Opus 1M option |
| `ModelOption` | Type |

### `modelAllowlist.ts` (1 export)

| Export | Purpose |
|--------|---------|
| `isModelAllowed(model)` | 3-tier allowlist check |

### `modelCapabilities.ts` (3 exports)

| Export | Purpose |
|--------|---------|
| `getModelCapability(model)` | Cached capability lookup |
| `refreshModelCapabilities()` | Async fetch and cache update |
| `ModelCapability` | Type |

### `modelStrings.ts` (4 exports)

| Export | Purpose |
|--------|---------|
| `getModelStrings()` | Provider-specific model ID map |
| `ensureModelStringsInitialized()` | Async init (waits for Bedrock) |
| `resolveOverriddenModel(modelId)` | Reverse override lookup |
| `ModelStrings` | Type |
...
| Export | Purpose |
|--------|---------|
| `getAgentModel(agent, parent, tool, perm)` | Full subagent model resolution |
| `getDefaultSubagentModel()` | Returns `'inherit'` |
| `getAgentModelDisplay(model)` | Display string for agent model |
| `getAgentModelOptions()` | Available agent model options |
| `AGENT_MODEL_OPTIONS` | Constant array |
| `AgentModelAlias` / `AgentModelOption` | Types |

### `antModels.ts` (5 exports)

| Export | Purpose |
|--------|---------|
| `getAntModelOverrideConfig()` | Feature-flag config loader |
| `getAntModels()` | Ant model array |
| `resolveAntModel(model)` | Alias/substring match |
| `AntModel` / `AntModelOverrideConfig` | Types |

### `bedrock.ts` (9 exports)

| Export | Purpose |
|--------|---------|
| `getBedrockInferenceProfiles()` | Memoized profile listing |
| `findFirstMatch(profiles, sub)` | Substring profile match |
| `createBedrockRuntimeClient()` | Bedrock runtime client factory |
| `getInferenceProfileBackingModel(id)` | Profile → backing model |
| `isFoundationModel(modelId)` | `anthropic.*` prefix check |
| `extractModelIdFromArn(modelId)` | ARN → profile ID |
| `getBedrockRegionPrefix(modelId)` | Region prefix extraction |
| `applyBedrockRegionPrefix(modelId, prefix)` | Region prefix application |
| `BedrockRegionPrefix` | Type |

### `check1mAccess.ts` (2 exports)

| Export | Purpose |
|--------|---------|
| `checkOpus1mAccess()` | Opus 1M access gating |
| `checkSonnet1mAccess()` | Sonnet 1M access gating |

### `contextWindowUpgradeCheck.ts` (1 export)

| Export | Purpose |
|--------|---------|
| `getUpgradeMessage(context)` | Upgrade suggestion string |

### `deprecation.ts` (1 export)

| Export | Purpose |
|--------|---------|
| `getModelDeprecationWarning(modelId)` | Deprecation warning string |

### `modelSupportOverrides.ts` (2 exports)

| Export | Purpose |
|--------|---------|
| `get3PModelCapabilityOverride(model, cap)` | 3P capability check |
| `ModelCapabilityOverride` | Type |

### `providers.ts` (3 exports)

| Export | Purpose |
|--------|---------|
| `getAPIProvider()` | Provider detection |
| `getAPIProviderForStatsig()` | Analytics-typed provider |
| `isFirstPartyAnthropicBaseUrl()` | Base URL validation |

### `validateModel.ts` (1 export)

| Export | Purpose |
|--------|---------|
| `validateModel(model)` | API-probe model validation |
...
## 23. Verification Against Existing Analysis (`model-selection.md`)

The existing Wave 23 analysis is **accurate but incomplete**. Key gaps filled by this Wave 47 deep analysis:

| Aspect | Wave 23 | Wave 47 Addition |
|--------|---------|------------------|
| Model registry | 4 models shown | All 11 models with full 4-provider IDs |
| Canonical name resolution | Not covered | Complete 14-pattern matching chain |
| Display name system | Not covered | 4 layers: public, marketing, git, ant-masked |
| Allowlist | "Organization restrict" one-liner | Full 3-tier matching algorithm documented |
| Capabilities cache | Not covered | Schema, eligibility, fetch, cache file details |
| Bedrock integration | Not covered | Client creation, profiles, region prefix system |
| 1M access gating | Not covered | 12 overage disabled reasons, access logic |
| Subagent model resolution | Not covered | 4-priority chain, alias-to-parent matching, region inheritance |
| Ant model system | Not covered | Feature-flag-driven config, codename masking |
| Model validation | Not covered | API probe, cache, 3P fallback suggestions |
| Deprecation system | Not covered | Per-provider retirement dates |
| 3P support overrides | Not covered | Capability env var system |
| Environment variables | Partially | Complete 22-variable inventory |
| Exported functions | Not covered | Complete 70+ function/type inventory |
