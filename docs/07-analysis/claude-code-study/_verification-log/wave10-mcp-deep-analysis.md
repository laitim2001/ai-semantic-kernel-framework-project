# Wave 10: MCP (Model Context Protocol) Deep Behavior Verification

**Date**: 2026-04-01
**Scope**: `src/services/mcp/`, `src/tools/MCPTool/`, `src/utils/mcp/`
**Quality Target**: 9.0+/10 behavioral fidelity
**Files Analyzed**: 12 primary files, ~4,200 LOC
...
### 10.1 Key Behavioral Findings

| Finding | Confidence |
|---------|------------|
| 10 transport types (stdio, sse, http, ws, ws-ide, sse-ide, claudeai-proxy, sdk, in-process-chrome, in-process-computeruse) | HIGH — verified in connectToServer switch |
| Tool naming: `mcp__${normalize(server)}__${normalize(tool)}` | HIGH — traced through mcpStringUtils.ts |
| 6-scope config precedence: enterprise > local > project > user > plugin > claude.ai | HIGH — traced through getClaudeCodeMcpConfigs + getAllMcpConfigs |
| OAuth via ClaudeAuthProvider with RFC 9728/8414 discovery + PKCE + CIMD | HIGH — verified in auth.ts |
| Large output: truncate or persist-to-disk with jq-friendly schema hint | HIGH — verified in processMCPResult |
| Elicitation: form mode + URL mode with -32042 error retry | HIGH — verified in elicitationHandler.ts + callMCPToolWithUrlElicitationRetry |
| Reconnection: 3 consecutive terminal errors → close; session expiry → 1 retry | HIGH — verified in client.ts error handlers |
| Connection batching: local=3, remote=20 concurrency via pMap | HIGH — verified in getMcpToolsCommandsAndResources |

### 10.2 Quality Score: 9.2/10

| Dimension | Score | Notes |
|-----------|-------|-------|
| Completeness | 9.5 | All 10 transport types, full OAuth flow, elicitation, output management |
| Accuracy | 9.0 | Verified against source code line references |
| Behavioral Depth | 9.2 | Retry logic, error classification, cleanup escalation, dedup strategies |
| Architecture Understanding | 9.0 | Config aggregation, memoization strategy, cross-process coordination |

### 10.3 Areas Not Fully Verified (Lower Confidence)

- XAA (Cross-App Access) IdP login flow details (referenced but not deeply traced)
- `wrapFetchWithStepUpDetection()` internal implementation (called but not read)
- Plugin MCP server discovery via `getPluginMcpServers()` (referenced in config.ts)
- `fetchMcpSkillsForClient` — feature-gated behind `MCP_SKILLS`, lazy-loaded

---

*Wave 10 complete. 12 source files analyzed, ~4,200 LOC, 10 verification points at HIGH confidence.*
