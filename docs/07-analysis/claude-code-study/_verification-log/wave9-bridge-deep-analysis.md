# Wave 9: Bridge / Remote Control Protocol Deep Analysis

> **Scope**: `src/bridge/bridgeMain.ts`, `replBridge.ts`, `bridgeMessaging.ts`, `workSecret.ts`, `bridgePermissionCallbacks.ts`, `types.ts`, `jwtUtils.ts`, `replBridgeTransport.ts`, `src/cli/transports/HybridTransport.ts`, `SSETransport.ts`
>
> **Date**: 2026-04-01 | **Quality Target**: 9.0+/10 | **Verification Points**: ~180

---

## 1. Architecture Overview

The Bridge system (marketed as "Remote Control") connects a local Claude Code CLI process to the cloud-hosted claude.ai/code web interface. It has two independent execution modes:
...
## 2. Bridge Main Loop (`bridgeMain.ts`) - Complete Steps
...
## 3. REPL Bridge (`replBridge.ts`) - Complete Flow

### 3.1 `initBridgeCore(params: BridgeCoreParams)` Steps
...
### 3.2 `onWorkReceived` (transport creation)
...
## 4. v1 vs v2 Transport Comparison

| Aspect | v1 (HybridTransport) | v2 (SSETransport + CCRClient) |
|--------|---------------------|-------------------------------|
...
- `baseDelayMs = 500`, `maxDelayMs = 8,000`, `jitterMs = 1,000`
...
- `POST_MAX_DELAY_MS = 8,000`

---

## 5. Work Secret Structure and Decoding

### Wire Format

Base64url-encoded JSON in `WorkResponse.secret` field.

### Decoded Structure (`WorkSecret`)
...
## 7. Permission Forwarding Mechanism
...
## 8. JWT Token Refresh Scheduling (`jwtUtils.ts`)
...
## 9. Reconnection and Resume Logic
...
- Two strategies (detailed in Section 3.3)
...
## 11. Backoff Configuration (Exact Constants)
...
## 12. Protocol Types Summary (`types.ts`)

### Core Types

| Type | Fields | Purpose |
|------|--------|---------|
| `WorkData` | `type: 'session' \| 'healthcheck'`, `id: string` | Work item payload |
| `WorkResponse` | `id, type, environment_id, state, data, secret, created_at` | Poll response |
| `WorkSecret` | See Section 5 | Decoded secret with JWT + URLs |
| `BridgeConfig` | `dir, machineName, branch, gitRepoUrl, maxSessions, spawnMode, ...` | Bridge configuration |
| `SessionHandle` | `sessionId, done: Promise, kill(), forceKill(), activities[], ...` | Running session reference |
| `SessionSpawnOpts` | `sessionId, sdkUrl, accessToken, useCcrV2?, workerEpoch?` | Spawn parameters |
...
Sent as `metadata.worker_type` for web-side session filtering.

---

## 13. Keep-Alive and Heartbeat Mechanisms
...
## 14. Process Suspension Detection
...
|---|-------|--------|----------|
| 1 | Bridge has standalone + REPL modes | `bridgeMain.ts`, `replBridge.ts` | YES |
| 2 | Work secret is base64url-encoded JSON with version 1 | `workSecret.ts:6-32` | YES |
| 3 | v1 uses WS reads + POST writes | `HybridTransport.ts` class definition | YES |
| 4 | v2 uses SSE reads + CCRClient POST writes | `SSETransport.ts`, `replBridgeTransport.ts` | YES |
| 5 | JWT refresh buffer is 5 minutes | `jwtUtils.ts:52` | YES |
| 6 | Fallback refresh interval is 30 minutes | `jwtUtils.ts:55` | YES |
| 7 | Max 3 refresh failures | `jwtUtils.ts:58` | YES |
| 8 | BoundedUUIDSet capacity is 2000 | `replBridge.ts:518,527` | YES |
| 9 | SSE reconnect gives up after 10 minutes | `SSETransport.ts:19` | YES |
| 10 | SSE liveness timeout is 45 seconds | `SSETransport.ts:21` | YES |
| 11 | Permanent HTTP codes: 401, 403, 404 | `SSETransport.ts:27` | YES |
| 12 | HybridTransport stream buffer is 100ms | `HybridTransport.ts:7` | YES |
| 13 | POST timeout is 15 seconds | `HybridTransport.ts:9` | YES |
| 14 | Close grace period is 3 seconds | `HybridTransport.ts:14` | YES |
| 15 | Default session timeout is 24 hours | `types.ts:2` | YES |
| 16 | SpawnMode has 3 values | `types.ts:68-69` | YES |
| 17 | Server control request timeout is 10-14s | `bridgeMessaging.ts:151` (comment) | YES |
| 18 | Permission response has allow/deny behavior | `bridgePermissionCallbacks.ts:3-8` | YES |
| 19 | stopWorkWithRetry has 3 max attempts | `bridgeMain.ts:1636` | YES |
| 20 | Shutdown grace is 30 seconds default | `bridgeMain.ts:67` (comment) | YES |
| 21 | v2 worker registration has 10s timeout | `workSecret.ts:112` | YES |
| 22 | Standalone bridge conn backoff: 2s initial, 2min cap, 10min give-up | `bridgeMain.ts:73-79` | YES |
| 23 | REPL bridge poll error: 2s initial, 60s cap, 15min give-up | `replBridge.ts:244-246` | YES |
| 24 | SSE POST retry: 10 attempts, 500ms base, 8s max | `SSETransport.ts:30-32` | YES |
| 25 | Max environment recreations: 3 | `replBridge.ts:583`, `replBridge.ts:1920` | YES |
| 26 | Suspension detection threshold: 60s overrun | `replBridge.ts:2122` | YES |
| 27 | Keep-alive default interval: 120s (from GrowthBook) | `replBridge.ts:1534` (comment) | YES |
| 28 | Epoch mismatch fires onClose(4090) | `replBridgeTransport.ts:220` | YES |
| 29 | Init failure fires onClose(4091) | `replBridgeTransport.ts:365` | YES |
| 30 | SSE budget exhaustion fires onClose(4092) | `replBridgeTransport.ts:313` | YES |

**Verification Score**: 30/30 claims verified against source code.
