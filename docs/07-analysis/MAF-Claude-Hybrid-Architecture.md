...
### 6.5 Phase 28: 三層意圖路由架構 (📋 計劃中)

> **計劃日期**: 2026-01-15
> **預計時程**: Sprint 91-99 (8.5 週 + 1 週緩衝)
> **總 Story Points**: 235 pts

#### 架構擴展方向

**組件重命名**:
- `IntentRouter` → `FrameworkSelector` (技術框架選擇，保持現有功能)

**新增組件** (位於 `backend/src/integrations/orchestration/`):

| 組件 | 位置 | 職責 | 狀態 |
|------|------|------|------|
| **BusinessIntentRouter** | `intent_router/router.py` | 業務意圖分類 + 完整度評估 | 📋 計劃中 |
| **PatternMatcher** | `intent_router/pattern_matcher/` | Layer 1: 規則匹配 (< 10ms) | 📋 計劃中 |
| **SemanticRouter** | `intent_router/semantic_router/` | Layer 2: 語義路由 (< 100ms) | 📋 計劃中 |
| **LLMClassifier** | `intent_router/llm_classifier/` | Layer 3: LLM 分類 (< 2000ms) | 📋 計劃中 |
| **InputGateway** | `input_gateway/gateway.py` | 多來源輸入處理 + 格式標準化 | 📋 計劃中 |
| **GuidedDialogEngine** | `guided_dialog/engine.py` | 完整度檢查 + 引導式對話 | 📋 計劃中 |
| **RiskAssessor** (新) | `risk_assessor/assessor.py` | IT Intent → 風險等級映射 | 📋 計劃中 |
| **HITLController** | `hitl/controller.py` | 人機協作審批控制 | 📋 計劃中 |
...
InputGateway (來源識別)
    │
    ├─ 系統來源 → 映射表 + Pattern (簡化路徑)
    │
    └─ 用戶來源 → BusinessIntentRouter (完整流程)
                    │
                    ├─ Layer 1: Pattern Matcher (>0.9 置信度 → 輸出)
                    │
                    ├─ Layer 2: Semantic Router (>0.85 相似度 → 輸出)
                    │
                    └─ Layer 3: LLM Classifier (分類 + 完整度)
                                │
                                ▼
                        RoutingDecision
                        {intent, completeness, risk_level}
                                │
                    ┌───────────┴───────────┐
                    │ is_sufficient?        │
                    │                       │
            ...
            HITL (如需要)                   │
                    ...
                    └───────────┬───────────┘
                                ▼
                    FrameworkSelector (技術框架選擇)
                                │
                                ▼
                    HybridOrchestratorV2 執行
```

#### 關鍵設計決策

1. **完整度整合到 Router**: BusinessIntentRouter 同時輸出分類和完整度
2. **增量更新**: GuidedDialog 收集資訊後不重新分類，只更新欄位和細化 sub_intent
3. **系統來源簡化**: ServiceNow/Prometheus 等系統來源使用映射表 + Pattern，跳過 Semantic/LLM

#### 與現有組件關係

| 現有組件 | Phase 28 關係 |
|---------|--------------|
| IntentRouter | 重命名為 FrameworkSelector，保持原有功能 |
| RiskAssessor (hybrid/risk/) | 與新 RiskAssessor 共存，新版專注 IT Intent 映射 |
| HITLManager | 與新 HITLController 整合，提供統一審批入口 |
| HybridOrchestratorV2 | 整合 BusinessIntentRouter 和 GuidedDialogEngine |
...
| 1.2 | 2026-01-15 | 添加 Phase 28 三層意圖路由架構計劃章節 (📋 計劃中) |
