...
> **前版**: V7.0 (2026-02-11, Phase 29, 64 功能, 8+3 Agent 驗證)
...
│                     │ 7 Primary Builders, ALL COMPLIANT         │
├─────────────────────┼───────────────────────────────────────────┤
...
- **數據持久化**: InMemory
- **依賴關係**: ConcurrentBuilder (agent_framework), `from agent_framework import ConcurrentBuilder` (COMPLIANT)
...
- **依賴關係**: HandoffBuilder (agent_framework), `from agent_framework import HandoffBuilder` (COMPLIANT)
...
- **數據持久化**: InMemory
- **依賴關係**: GroupChatBuilder (agent_framework), `from agent_framework import GroupChatBuilder` (COMPLIANT)
...
- **依賴關係**: MagenticBuilder (agent_framework), `from agent_framework import MagenticBuilder` (COMPLIANT)
...
- **業務邏輯**: 7 個 Primary Builder Adapter 全部通過合規審計：ConcurrentBuilder, HandoffBuilder, GroupChatBuilder, MagenticBuilder, WorkflowExecutor, NestedWorkflow, PlanningAdapter。每個都正確 `from agent_framework import` 並使用 `self._builder = OfficialBuilder()`。另有 10 個 Extension Adapter (平台業務邏輯)、5 個 Migration Shim (Phase 2 向後相容)
- **數據持久化**: N/A (Adapter Pattern)
- **依賴關係**: agent_framework 官方套件
...
- **業務邏輯**: 使用真實 `from anthropic import AsyncAnthropic` SDK。Agentic loop (tool_use → execute → feed back)。Extended Thinking (beta header)。6 種 Smart Fallback 策略
- **數據持久化**: InMemory + Claude API
- **依賴關係**: anthropic SDK, mem0
- **問題**: Streaming 未在 Session.query() 中實現 (M-07); MCP tool 整合 registry 為 stub (M-08); ContextSynchronizer 非執行緒安全 (H-04)
- **驗證來源**: phase3c-claude-sdk
