...
- [x] **更新 `backend/src/adapters/_base/chat_client.py`** 補齊 4 abstract method
  - 預估：30 min
...
- [x] **建 `backend/src/adapters/_base/types.py`** 補完中性型別
  - 預估：30 min
...
- [x] **建 `backend/src/adapters/azure_openai/error_mapper.py`** AzureOpenAIErrorMapper
  - 預估：30 min
...
- [x] **建 `backend/src/adapters/azure_openai/tool_converter.py`** ToolSpec → Azure 格式
  - 預估：30 min
  - DoD：ToolSpec → `{"type": "function", "function": {"name", "description", "parameters"}}` 轉換 + 反向 Azure tool_calls → ToolCall 解析
- [x] **建 `backend/src/adapters/azure_openai/adapter.py`** AzureOpenAIAdapter
  - 預估：90 min
...
- [x] **建 `backend/src/adapters/_testing/mock_clients.py`** MockChatClient
  - 預估：30 min
  - DoD：實作 ChatClient ABC 全方法 + 可注入 responses dict + count_tokens 硬編值
- [x] **建 `backend/src/adapters/_testing/conftest.py`** 共用 fixture
  - 預估：15 min
  - DoD：mock_chat_client fixture + azure_adapter fixture（接 env var 或 mock）
- [x] **建 `backend/tests/unit/adapters/azure_openai/test_contract.py`** ≥ 7 contract tests
  - 預估：60 min
...
- [x] **建 `backend/tests/unit/adapters/azure_openai/test_token_counting.py`** + `test_pricing.py` + `test_error_mapper.py`
  - 預估：15 min
  - DoD：token counting 對 gpt-5.4 模型準確 / pricing 對 USD-per-1M / error mapper 8 case 全 PASS

### 1.4 Day 1 收尾（30 min）
- [x] **mypy strict + black + isort + flake8** 全綠
  - 預估：10 min
  - Command：`cd backend && black src/adapters && isort src/adapters && flake8 src/adapters && mypy src/adapters --strict`
- [x] **LLM SDK leak grep 驗證**：openai / anthropic 只在 `adapters/azure_openai/` 內可見
  - 預估：5 min
...
- [x] **Day 1 commit**：`feat(adapters-azure-openai, sprint-49-4): Day 1 ChatClient ABC 4 methods + Azure OpenAI adapter + 7 contract tests`
  - 預估：15 min
...
- [x] **建 spike branch / experimental folder**：`experimental/spike-celery/`
  - 預估：5 min
- [x] **Celery + Redis broker setup**：簡單 task + retry + result backend
  - 預估：45 min
  - DoD：sleep 5s → return result；測延遲 + 持久化（kill worker resume from checkpoint）
- [x] **Celery 量化指標**：latency P50 / P99、kill-worker resume 行為、Python ergonomics 評分（1-5）
  - 預估：40 min

### 2.2 Temporal Spike（90 min）
- [x] **Temporal server + Python SDK setup**
  - 預估：30 min
- [x] **Temporal workflow + activity**：sleep 5s → return result + checkpoint
  - 預估：45 min
  - DoD：kill worker → workflow 從 checkpoint resume；timer / signal / workflow vs activity 區分清楚
- [x] **Temporal 量化指標**：latency / 持久化 / re-entrant / Python ergonomics
  - 預估：15 min

### 2.3 決策報告（90 min）
- [x] **建 `docs/03-implementation/agent-harness-execution/phase-49/sprint-49-4/worker-queue-decision.md`**
  - 預估：60 min
  - DoD：5 軸對比表（latency / 持久化 / re-entrant / Python ergonomics / 運維成本） + 推薦（含理由 + 反方論點）+ carry-forward 條件（什麼情況需重新評估）
- [x] **49.3 retrospective Action item 9 勾掉**（worker queue 選型 PoC）
  - 預估：5 min
  - DoD：retrospective.md Action item 9 標 ✅ + 連結決策報告

### 2.4 agent_loop_worker 框架（90 min）
- [x] **建 `backend/src/runtime/__init__.py` + `backend/src/runtime/workers/__init__.py`**
  - 預估：10 min
- [x] **建 `backend/src/runtime/workers/queue_backend.py`** ABC
  - 預估：30 min
  - DoD：QueueBackend ABC（abstract submit / poll / cancel / get_status）+ MockQueueBackend 實作（in-memory）
- [x] **建 `backend/src/runtime/workers/agent_loop_worker.py`** stub
  - 預估：45 min
...
- [x] **建 `backend/tests/unit/runtime/workers/test_agent_loop_worker.py`** 4 unit tests
  - 預估：35 min
...
### 2.5 Day 2 收尾（30 min）
- [x] **mypy + lint + tests 全綠**
  - 預估：10 min
- [x] **Day 2 commit**：`feat(workers, sprint-49-4): Day 2 worker queue selection (Celery vs Temporal) + agent_loop_worker framework`
  - 預估：15 min
...
- [x] **更新 `backend/requirements.txt`** 鎖 OTel SDK 版本
  - 預估：15 min
...
- [x] **更新 `docker-compose.dev.yml`** 加 jaeger + prometheus 服務
  - 預估：15 min
...
- [x] **更新 `backend/src/agent_harness/observability/_abc.py`** Tracer ABC 完整化
  - 預估：30 min
  - DoD：Tracer ABC（start_span / set_attribute / record_metric / set_status / end）+ Span dataclass + signature 對齊 17.md §Contract 12
- [x] **建 `backend/src/agent_harness/observability/tracer.py`** OTelTracer 具體類
  - 預估：60 min
...
- [x] **建 `backend/src/agent_harness/observability/metrics.py`** MetricEvent + registrar
  - 預估：60 min
...
- [x] **建 `backend/src/agent_harness/observability/exporter.py`** Jaeger + Prometheus exporter
  - 預估：30 min
...
- [x] **建 `backend/src/platform_layer/observability/__init__.py`**
  - 預估：5 min
- [x] **建 `backend/src/platform_layer/observability/setup.py`** OTel global init
  - 預估：45 min
...
- [x] **建 `backend/src/platform_layer/observability/logger.py`** structured JSON logger
  - 預估：40 min
  - DoD：JSON formatter + PIIRedactor（email / phone / SSN regex）+ trace_id 自動注入 + tenant_id label

### 3.5 Tests + 對齊驗證（90 min）
- [x] **建 `backend/tests/unit/agent_harness/observability/test_tracer.py`** 3 tests
  - 預估：30 min
  - DoD：test_span_hierarchy / test_trace_context_propagation / test_attribute_set 全 PASS
- [x] **建 `backend/tests/unit/agent_harness/observability/test_metrics.py`** 2 tests
  - 預估：20 min
  - DoD：test_metric_emission / test_7_required_metrics_registered 全 PASS
- [x] **建 `backend/tests/unit/agent_harness/observability/test_logger.py`** 1 test
  - 預估：20 min
  - DoD：test_json_format_and_pii_redaction PASS
- [x] **手動驗證 Jaeger UI**：`curl /api/v1/health` → Jaeger UI :16686 看到 span（hookup 在 Day 5）
  - 預估：20 min（部分推 Day 5）

### 3.6 Day 3 收尾（30 min）
- [x] **mypy + lint + tests 全綠**
  - 預估：10 min
- [x] **`.claude/rules/observability-instrumentation.md` 對齊驗證**：5 處必埋點 + 7 必埋 metric + TraceContext 規範對齊
  - 預估：5 min
- [x] **Day 3 commit**：`feat(observability, sprint-49-4): Day 3 OpenTelemetry tracing+metrics+logging integration + Tracer ABC + 7 metrics + JSON logger`
  - 預估：15 min
...
- [x] **建 `scripts/lint/check_duplicate_dataclass.py`**
  - 預估：30 min
  - DoD：AST scan `agent_harness/**/*.py` 找 `@dataclass` + class name；同名跨目錄報 ERROR；輸出 file:line + 衝突清單
- [x] **建 `backend/tests/unit/scripts/lint/test_duplicate_dataclass.py`** 2 tests
  - 預估：15 min
...
- [x] **建 `scripts/lint/check_cross_category_import.py`**
  - 預估：45 min
...
- [x] **建 `backend/tests/unit/infrastructure/db/test_pg_partman.py`** 1 test
  - 預估：15 min
  - DoD：test_pg_partman_extension_loaded + test_partman_premake_6_partitions PASS

### 4.7 tool_calls.message_id FK 決策（30 min）
- [x] **嘗試 composite FK**：(message_id, message_created_at) FK to messages(id, created_at)
  - 預估：15 min
  - DoD：alembic revision + 測試是否通過 PG 16 partition table FK 限制
- [x] **建決策報告**（半頁，整合進 worker-queue-decision.md 同 sprint-49-4 folder）
  - 預估：10 min
  - DoD：FK 決策（接 / 推遲 / 替代方案）+ 寫入 carryover 列表
- [x] **49.3 retrospective Action item 4（tool_calls.message_id FK）勾掉**（標 ✅ 或 🚧 + 引用決策報告）
  - 預估：5 min

### 4.8 Day 4 收尾（30 min）
- [x] **`pre-commit run --all-files` 全綠**
  - 預估：10 min
- [x] **手動製造 4 種違反，CI 全擋驗證**
  - 預估：5 min
- [x] **Day 4 commit**：`feat(ci, sprint-49-4): Day 4 4 lint rules (duplicate-dataclass / cross-category-import / sync-callback / LLM SDK leak) + pg_partman + Dockerfile.postgres + 0010 migration + tool_calls.message_id FK decision`
  - 預估：15 min
...
- [x] **建 `backend/src/api/main.py`** FastAPI app
  - 預估：45 min
...
- [x] **建 `backend/src/api/v1/__init__.py`** + `backend/src/api/v1/health.py` /health endpoint
  - 預估：30 min
  - DoD：GET /api/v1/health 200 + 含 DB ping + Redis ping + RabbitMQ ping（用 anyio.to_thread.run_sync 避免 blocking）+ 結構化 JSON response
- [x] **建 `backend/tests/unit/api/test_health.py`** 2 tests
  - 預估：15 min
...
- [x] **更新 `13-deployment-and-devops.md`** 補 §Production App Role
  - 預估：30 min
...
- [x] **49.3 retrospective Action item 2（生產 app_role）勾掉** ✅
  - 預估：5 min（含 above）

### 5.3 全套整合 / 紅綠（90 min）
- [x] **`uvicorn backend.src.api.main:app --reload --host 0.0.0.0 --port 8000` 啟動驗證**
  - 預估：15 min
  - Command：`curl http://localhost:8000/api/v1/health` → 200 + DB/Redis/MQ 全 ✅
- [x] **Jaeger UI 手動驗證**：`curl /api/v1/health` 後 Jaeger UI :16686 看到 health span
  - 預估：10 min
- [x] **Prometheus 手動驗證**：`curl http://localhost:8000/metrics` 看到 7 metrics
  - 預估：10 min
- [x] **全套 alembic from-zero cycle**：downgrade base → upgrade head（驗 0010 + 0009 + 0001-0008 全可逆）
  - 預估：15 min
- [x] **全套 pytest run**：49.3 73 + 49.4 新增 ~25 = ~98 PASS / 0 SKIPPED
  - 預估：20 min
- [x] **全套 mypy strict / black / isort / flake8 / 4 lint rules / pre-commit 全綠**
  - 預估：15 min
- [x] **LLM SDK leak grep 驗證**：`agent_harness + business_domain + platform_layer` 全 0 import
  - 預估：5 min

### 5.4 49.3 Carryover 5 項驗證 + 標完成（30 min）
- [x] **(1) pg_partman extension** ✅ — 0010 migration 通過 + extension 裝起來
- [x] **(2) Production app_role guide** ✅ — 13.md §Production App Role 已寫
- [x] **(3) tool_calls.message_id FK 決策** ✅ / 🚧 — 決策報告 commit
- [x] **(4) CI .ini ASCII-only check** ✅ — 整合進 cross-category-import-check（複用同 lint）
- [x] **(5) state_snapshots STATEMENT TRUNCATE** ✅ — 49.3 已補裝 + 本 sprint 驗證 alembic cycle 通過
- [x] **49.3 retrospective Action items 全部勾掉**（updates 49.3 retrospective.md）
  - 預估：30 min（含 above 5 項）

### 5.5 Phase 49 收尾文件（60 min）
- [x] **建 `docs/03-implementation/agent-harness-execution/phase-49/sprint-49-4/progress.md`**
  - 預估：20 min
  - DoD：5 days estimate vs actual + ratio + daily highlights + surprises 列表
- [x] **建 `docs/03-implementation/agent-harness-execution/phase-49/sprint-49-4/retrospective.md`**
  - 預估：20 min
  - DoD：what went well / what surprised / improvements / Action items for Phase 50.1 / Approvals & sign-off
- [x] **更新 `phase-49-foundation/README.md`** 4/4 = 100%
  - 預估：10 min
  - DoD：Sprint 49.4 標 ✅ DONE + 累計交付（4/4 sprint）+ Phase 49 結束驗收 11 項全 ✅ + Phase 50.1 prerequisites unblocked 列表
- [x] **更新 `06-phase-roadmap.md`** Phase 49 標 ✅ DONE
  - 預估：10 min
  - DoD：Phase 49 標題 + 各 Sprint 49.1-49.4 全 ✅ + 累計 4/22 sprint = 18.2%

### 5.6 Sprint Checklist + Closeout commit（30 min）
- [x] **本 checklist 全 [x]**（or 🚧 + 理由）
  - 預估：10 min
- [x] **Day 5 closeout commit**：`docs(sprint-49-4, phase-49): Phase 49 Foundation closeout — 4/4 sprint complete + ~98 tests + 4 lint rules + OTel + pg_partman + FastAPI /health`
  - 預估：20 min
...
- [x] **AC-1**：ChatClient ABC 6 方法全 abstract + Contract Test ≥ 7 PASS
- [x] **AC-2**：AzureOpenAIAdapter 實作 ABC + 真實 Azure OpenAI 呼叫 PASS（or mock fallback）
- [x] **AC-3**：Worker queue 決策報告 + agent_loop_worker 框架 + 4 unit test PASS
- [x] **AC-4**：OTel SDK 鎖版本 + Jaeger / Prometheus exporter + Tracer / MetricEvent / Logger 6 unit test PASS
- [x] **AC-5**：4 Lint rules pre-commit + CI 強制 + 8 case test PASS
- [x] **AC-6**：pg_partman 裝起來 + 0010 migration + Dockerfile.postgres
- [x] **AC-7**：FastAPI app 啟動 + /health 200 + TenantContextMiddleware + OTel instrumentation
- [x] **AC-8**：49.3 carryover 5 項全清
- [x] **AC-9**：~98 PASS / 0 SKIPPED / mypy strict / 4 lint clean / LLM SDK leak 0
- [x] **AC-10**：Phase 49 收尾 4/4 = 100% + roadmap Phase 49 ✅ DONE + Phase 50.1 prerequisites unblocked

---

## Anti-Pattern Checklist（PR review 前自檢）

- [x] **AP-1** — 不是 Pipeline 偽裝 Loop（本 sprint 無 LLM loop；adapter 層不適用）N/A
- [x] **AP-2** — 不是 side-track（adapter / worker / OTel / lint 全主流量必需）✅
- [x] **AP-3** — 不跨目錄散落（adapters/azure_openai 集中 / observability 集中 / lint 在 scripts/lint/）✅
- [x] **AP-4** — 不是 Potemkin（每個交付都有 unit test + 主流量驗證 hookup）✅
- [x] **AP-5** — 不是無計畫 PoC（worker queue spike 有 deadline + 決策報告）✅
- [x] **AP-6** — 沒有「為未來預留」抽象（不接 Anthropic / OpenAI adapter；只實作真實使用）✅
- [x] **AP-7** — 處理 context rot（本 sprint 無長對話；不適用）N/A
- [x] **AP-8** — 透過 PromptBuilder（本 sprint 無 LLM 呼叫；不適用）N/A
- [x] **AP-9** — 有 verification（本 sprint 無 agent 輸出；不適用）N/A
- [x] **AP-10** — Mock 與 real 同 ABC（MockChatClient + AzureOpenAIAdapter 都繼承 ChatClient ABC）✅
- [x] **AP-11** — 無版本後綴 + 命名一致（無 `_v1` / `_v2`；adapter / worker / observability 命名一致）✅
