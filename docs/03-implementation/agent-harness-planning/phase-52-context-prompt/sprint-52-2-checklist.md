...
- [ ] **`git checkout -b feature/phase-52-sprint-2-cat5-prompt-builder`** from main HEAD `a45fd2f2`
...
- [ ] **修改 `agent_harness/prompt_builder/_abc.py`**
...
- [ ] **建 `prompt_builder/strategies/__init__.py`**（package marker，re-export PositionStrategy + 3 concrete + PromptSections）
- [ ] **建 `prompt_builder/strategies/_abc.py`**
...
- [ ] **建 `prompt_builder/strategies/naive.py`**
  - `NaiveStrategy.arrange()`：order = `[system, tools, memory(flatten), conversation, user]`
- [ ] **建 `prompt_builder/strategies/lost_in_middle.py`**
...
- [ ] **建 `prompt_builder/strategies/tools_at_end.py`**
...
- [ ] **建 `tests/unit/agent_harness/prompt_builder/test_strategies_naive.py`** — 3 tests
  - `test_order_system_first` / `test_tools_after_system` / `test_user_at_end`
- [ ] **建 `test_strategies_lost_in_middle.py`** — 5 tests
...
- [ ] **建 `test_strategies_tools_at_end.py`** — 3 tests
  - `test_system_first` / `test_tools_at_last` / `test_user_before_tools`
- [ ] **建 `test_builder_basic.py`** — 4 tests
...
- [ ] **建 `test_templates.py`** — 3 tests
  - `test_system_role_template_format` / `test_memory_section_header` / `test_format_memory_section`
- [ ] 其他 6 test files 建占位（@pytest.mark.skip）：`test_builder_memory_injection.py` / `test_builder_cache_integration.py` / `test_builder_position_strategy.py` / `test_builder_token_estimation.py` / `test_lint_rule.py` / `test_anthropic_mock_adapter.py`
...
- [ ] `test_5_layers_all_injected`（mock retrieval 回 5 layer hints → all 出現在 artifact）
- [ ] `test_3_time_scales_ordered_permanent_first`（permanent / quarterly / daily 依序）
- [ ] `test_memory_retrieval_failure_degrades_gracefully`（mock 拋 MemoryRetrievalError → artifact 仍可 build；layer_metadata["memory_layers_used"] = []）
- [ ] `test_layer_metadata_records_used_layers`（layer_metadata["memory_layers_used"] 含 list of layer names）
- [ ] **🛡️ red-team 1**：`test_cross_tenant_no_leak`（tenant_a memory 寫入 InMemoryStore；tenant_b PromptBuilder.build → grep tenant_a content in artifact = 0）
- [ ] **🛡️ red-team 2**：`test_cross_user_no_leak`（user_a tenant_a memory 寫入；user_b tenant_a build → user-scoped memory 不含）
- [ ] **🛡️ red-team 3**：`test_cross_session_no_leak`（session_a memory；session_b build → session-scoped memory 不含）
- [ ] **🛡️ red-team 4**：`test_expired_time_scale_filtered`（daily memory 過期；build → 不再注入）
...
- [ ] `test_default_policy_3_breakpoints`（system + tools + memory → 3 breakpoints）
- [ ] `test_cache_recent_turns_disabled_default`（default policy 不含 recent turns breakpoint）
- [ ] `test_content_hash_deterministic`（同 sections build 兩次 → 同 content_hash）
- [ ] `test_layer_metadata_cache_sections`（layer_metadata["cache_sections"] = ["system", "tools", "memory"]）
- [ ] `test_cache_manager_failure_degrades`（mock cache_manager 拋例外 → cache_breakpoints=[] + artifact 仍 build）
  - DoD：5 tests pass

### 2.5 `test_builder_position_strategy.py` + `test_builder_token_estimation.py` — 6 tests（45 min）
- [ ] **`test_builder_position_strategy.py`** — 3 tests
  - `test_default_lost_in_middle` / `test_override_to_naive` / `test_override_to_tools_at_end`
- [ ] **`test_builder_token_estimation.py`** — 3 tests
...
- [ ] **建 `tests/integration/agent_harness/prompt_builder/test_memory_5x3_matrix.py`**
...
- [ ] **更新 `17-cross-category-interfaces.md` §1.1 PromptArtifact row**
...
- [ ] **修改 loop 接受 `prompt_builder: PromptBuilder | None = None` 注入**（None 則 fallback 50.x 邏輯，backward-compat）
- [ ] **每 turn 開頭（compactor 之後）build artifact**：
...
- [ ] **替換既有 ad-hoc `messages = state.transient.messages.copy()` 組裝**為 artifact-based
- [ ] **call ChatClient 時傳 cache_breakpoints**：`chat(messages=artifact.messages, tools=tools, cache_breakpoints=artifact.cache_breakpoints, ...)`
- [ ] **emit `PromptBuilt` event**（payload 完整：messages_count / estimated_input_tokens / cache_breakpoints_count / memory_layers_used / position_strategy_used / duration_ms）
- [ ] **既有 50.x loop tests 不被破壞**（prompt_builder=None default）
...
- [ ] **跑 50.x loop tests + 52.1 30-turn integration**
...
- [ ] **修改 `agent_harness/_contracts/events.py`**
...
- [ ] **建 `adapters/_mock/anthropic_adapter.py`**
...
- [ ] **修改 `adapters/_testing/mock_clients.py`**
...
- [ ] `test_chat_returns_response`（基本 contract）
- [ ] `test_cache_control_marker_injected`（cache_breakpoints=[bp1] → messages[bp1.position] 含 cache_control）
- [ ] `test_no_breakpoints_no_marker`（cache_breakpoints=None → messages 無 cache_control）
- [ ] `test_does_not_import_anthropic_sdk`（grep file content = 0 anthropic imports）
  - DoD：4 tests pass

### 3.8 `test_loop_with_prompt_builder.py` integration（60 min）
- [ ] **建 `tests/integration/agent_harness/prompt_builder/test_loop_with_prompt_builder.py`**
...
- [ ] **建 `scripts/check_promptbuilder_usage.py`**
...
- [ ] `test_detects_chat_without_build`（fixture file 含 chat() 但無 build() → SystemExit(1)）
- [ ] `test_allows_chat_with_prior_build`（fixture file 含 build() → chat() → exit 0）
- [ ] `test_allowlist_tests_directory`（test_*.py 內 chat() 無 build() → exit 0）
- [ ] `test_allowlist_mock_clients`（_testing/mock_clients.py → exit 0）
- [ ] `test_dry_run_no_exit`（--dry-run + violation → 不 SystemExit）
  - DoD：5 tests pass

### 4.3 100% 主流量 e2e（45 min）
- [ ] **建 `tests/e2e/test_main_flow_via_prompt_builder.py`**
...
- [ ] **建 `tests/integration/agent_harness/prompt_builder/test_slo_latency.py`**
...
- [ ] **建 `tests/integration/agent_harness/prompt_builder/test_cache_hit_steady_state.py`**
...
- [ ] **`grep -r "import openai\|import anthropic" backend/src/agent_harness/prompt_builder/` → 0 actual imports**
- [ ] **`grep -r "import openai\|import anthropic" backend/src/adapters/_mock/` → 0**（mock anthropic adapter 不能 import 真 SDK）
- [ ] **5 V2 lints 全 pass**：
...
- [ ] **建 `docs/03-implementation/agent-harness-execution/phase-52/sprint-52-2/retrospective.md`**
...
- [ ] **建 `docs/03-implementation/agent-harness-execution/phase-52/sprint-52-2/progress.md`**
...
- [ ] **更新 `sprint-52-2-checklist.md` 全勾**（除 4.12 🚧）
