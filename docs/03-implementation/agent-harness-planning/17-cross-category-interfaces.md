...
| `ModelPolicy` | `27-per-tenant-model-policy-design.md` | adapters/_base | `@dataclass(frozen=True) {action_deployment, action_model, cheap_deployment, cheap_model: str\|None}` — Sprint 57.104 (C1). A provider-neutral **value object** carrying a tenant's OPTIONAL model overrides; resolves to a `ModelProfile`. All-None = byte-identical env-only path. Resolved per-request by `resolve_tenant_model_policy(db, tenant_id)` (`platform_layer/billing/model_policy.py`; TTL-cached, fail-open; `invalidate_tenant_model_policy` called by the admin PUT) in the router BEFORE the sync builders (mirrors `resolve_session_persona`), threaded into `build_azure_model_profile(policy)`. Admin write/read: `PUT`/`GET /api/v1/admin/tenants/{id}/model-policy` (pricing-validated 422 + `meta_data['model_policy']` JSONB + audit). See `CHANGE-071`. |
...
| `PromptCacheManager` | `01-eleven-categories-spec.md` | 範疇 4 | `get_cache_breakpoints(*, tenant_id, policy, trace_context=None) -> list[CacheBreakpoint]`（async） / `invalidate(*, tenant_id, reason, trace_context=None) -> None`（async）— **52.1 Day 1 簽名升級**（49.1 stub `plan_breakpoints(messages, provider_supports_caching)` 已淘汰；multi-tenant safety: tenant_id 強制隔離）+ **52.2 Day 2.3 W3-2 audit carryover**：加 `trace_context: TraceContext \| None = None` default kwarg（cross-cutting trace propagation；既有 51.1/52.1 callers default-arg 兼容） |
...
| `Verifier` | `01-eleven-categories-spec.md` | 範疇 10 | `verify(*, output, state: LoopState \| None = None, trace_context) -> VerificationResult`（**Sprint 57.111 A3**：`state` 由非 Optional → `LoopState \| None`，移除 4-site `cast(LoopState,None)` 型別謊言；in-loop gate 傳真 `trace_state`、3 個 Cat 9 fallback judge site 傳 `None`；`LLMJudgeVerifier` 讀 `state.transient.messages` 組 trace-aware `{trace}`）|
| `SubagentDispatcher` | `01-eleven-categories-spec.md` | 範疇 11 | `spawn()` / `wait_for()` — **Sprint 57.107 (B3)**: `handoff()` REMOVED（HANDOFF 不經 dispatcher：`handoff` tool_call 由 loop classifier 攔截 → `stop_reason="handoff"` → platform `HandoffService` boot；spec-only 工具見 §3.1 `handoff` row） |
...
| `TeammateChildLoopFactory` | `_contracts/subagent.py` | 範疇 11 | `Callable[[SubagentBudget, MessageInbox\|None], AgentLoop]` — Sprint 57.102 (B2a) TEAMMATE real child loop. Sibling of `ChildLoopFactory`; the teammate child ALSO takes the B1 `MessageInbox` (Sprint 57.101; live producer = B2b). Built at `build_real_llm_handler` (`_make_teammate_child_loop`), injected → dispatcher → `TeammateExecutor`. FORK's `ChildLoopFactory` byte-identical. See `CHANGE-069`. |
| `TeammateInboxScope` | `_contracts/subagent.py` | 範疇 11 → api | `Callable[[UUID], AbstractAsyncContextManager[MessageInbox\|None]]` — Sprint 57.103 (B2b). Replaces B2a's sync `inbox_factory`: an async CM keyed by `subagent_id` that REGISTERS the teammate's `InjectionRegistry` queue on enter + UNREGISTERS on exit (success/timeout/exception), so a chat-user inject reaches a LIVE teammate (`put()` succeeds) + a put onto a finished one returns False → 409 (no Potemkin dead inbox). Concrete CM = `make_teammate_inbox_scope(registry, tenant)` in `api/v1/chat/injection_registry.py`; the `TeammateExecutor` brackets the child drive in `async with`. Producer endpoint `POST /chat/{id}/subagents/{sid}/inject`; the inject **UI** is deferred to §2.5 (the FE never observes a teammate "running" under the buffered relay). See `CHANGE-070`. |
...
| `MessageInbox` | `_contracts/inbox.py` | 範疇 1 | `async drain() -> list[Message]` — Sprint 57.101 B1 between-turns injection. Loop drains it at the `_run_turns` top; injected content runs the Cat 9 **INPUT** guardrail (a non-PASS injection is dropped + `GuardrailTriggered(input)`, run continues). Optional ctor dep; `None` = no-op. Chat backing = `QueueMessageInbox` over the module `InjectionRegistry` (`api/v1/chat/injection_registry.py`). **Sprint 57.102 (B2a)**: the teammate child loop reuses the SAME ABC + `QueueMessageInbox` over the `InjectionRegistry` keyed by the child's `subagent_id`; the per-request `MailboxStore` is the child→parent `send_to_parent` channel (not the inbox — it can't bridge the separate inject request). **Sprint 57.103 (B2b)**: the queue is registered ONLY while the teammate runs via a `TeammateInboxScope` async CM (`make_teammate_inbox_scope`); producer endpoint `POST /chat/{id}/subagents/{sid}/inject` (the inject UI awaits §2.5). See `26-between-turns-injection-design.md` / `CHANGE-069` / `CHANGE-070`. |
...
| `task_spawn` | **範疇 11 (Subagent)** | Fork / Teammate subagent（54.2+；**57.64 Day 2** wired on chat path 經 `make_task_spawn_tool(dispatcher, parent_session_id)`；`SubagentResult` 由 handler 內部 merge — **無獨立 `SubagentResultReducer`**） | SEQUENTIAL / AUTO / MEDIUM | LLM via 範疇 2 Registry |
| `handoff` | **範疇 11 (Subagent)** | Handoff 控制權（54.2+；**57.107 B3 spec-only**：`make_handoff_spec(suggested_targets)` — handler 為 defensive raise，loop classifier 在執行前攔截（`stop_reason="handoff"`）→ router post-loop hook → `HandoffService.boot_handoff(allowed_targets=tenant policy)`；註冊 gate = `harness_policy.handoff_enabled`（None/True=on, False=absent）；allowlist = `harness_policy.handoff_target_allowlist`） | SEQUENTIAL / AUTO / MEDIUM | LLM via 範疇 2 Registry（parent executor only — child/teammate 不註冊） |
| `send_to_parent` | **範疇 11 (Subagent)** | TEAMMATE child→parent report (57.102 B2a; `make_send_to_parent_tool(mailbox, parent_session_id)`; registered ONLY on the teammate child via `make_default_executor(teammate_mailbox=)`; the `TeammateExecutor` drains the parent mailbox + folds reports into `SubagentResult.summary`) | SEQUENTIAL / AUTO / LOW | teammate child LLM via 範疇 2 Registry |
...
| `ApprovalRequested` | §HITL 中央化 | HITL 觸發等待；+`kind` (tool/input/between_turns/output/verification, Sprint 57.100) → chat-v2 card branches REJECT on `kind="verification"` (coach-one-turn vs terminate) |
| `ApprovalReceived` | §HITL 中央化 | HITL 結果回到 loop |
| `LoopCompleted` | 範疇 1 | Loop 終止（Sprint 57.2 加 accumulator-sourced `total_tokens` / `input_tokens` / `output_tokens` / `provider` / `model`；**Sprint 57.65 A-2 Tier2 加 `cached_input_tokens` + 衍生 `cache_hit_rate`**（= cached / input，div-by-0 → 0.0；Cat 12 prompt-cache-hit-rate metric，無 Tracer/MetricsRegistry，metric 隨 event 欄位下游消費）；**Sprint 57.82 B-8 leg-1 加 `verification_input_tokens` / `verification_output_tokens` / `verification_model`** — Cat 10 **in-loop verify gate（Sprint 57.98 起；前為 correction-loop wrapper）** 跨 verifier+attempt 累加的 judge LLM token（`_run_turns` 內 `verif_*` local 累加並 stamp terminal LoopCompleted），chat router billing observer 消費為獨立 `_verification` cost-ledger sub_type + 計入 quota actual；server-side only，不上 SSE wire（與 loop input/output_tokens 一致）；**Sprint 57.88 地基 A 加 terminal `stop_reason="awaiting_approval"`**（`TerminationReason.AWAITING_APPROVAL`，parallel to 57.68 `handoff`）— chat-path tool ESCALATE 在 `hitl_deferred` 模式下 checkpoint loop（`pending_approval` + `resume_messages` 進 `state_snapshots` JSONB，免 migration）+ emit `ApprovalRequested` + 終止以**釋放 SSE 連線**；human approve（hours/days 後）→ `POST /chat/{id}/resume` 驅動 NEW `AgentLoopImpl.resume()` 續到 `end_turn`；見 `19-pause-resume-design.md`；**Sprint 57.91 Slice 3 leg 1（CHANGE-058）同一 terminal 新增 origin**：input-guardrail ESCALATE 也經 generalized `_emit_deferred_pause` primitive 觸發 `awaiting_approval`（`pending_approval.kind="input"`，無 tool_call；resume 續到第一個 LLM turn，不執行 tool）；**Sprint 57.92 Slice 3 leg 2（CHANGE-059）同一 terminal 第三 origin**：between-turns guardrail ESCALATE 在 `_run_turns` loop 頂端（≥1 turn 後、下一 LLM call 前）觸發 `awaiting_approval`（`pending_approval.kind="between_turns"`，無 tool_call；resume 經 `skip_between_turns_once` 續到下一 turn 的 LLM，不重新 escalate）；新增 `GuardrailType.BETWEEN_TURNS` enum value（single-source `guardrails/_abc.py`；無新 LoopEvent contract）；**Sprint 57.93 Slice 3 output-guardrail leg（CHANGE-060）同一 terminal 第四 origin**：output-guardrail ESCALATE 在 FINAL answer 上於 `_run_turns` parse 後、`LLMResponded` **之前**（pre-delivery gate，gated on `is_final_answer` + full deferred-HITL wiring）觸發 `awaiting_approval`（`pending_approval.kind="output"`，無 tool_call，carries held-answer `response_snapshot`）；reuse 既有 `GuardrailType.OUTPUT` + `check_output`（無新 enum value）；resume output kind 為 TERMINAL（不 drive `_run_turns`）— APPROVED re-emit held `LLMResponded`+`END_TURN`（無 LLM re-call），REJECTED → `GUARDRAIL_BLOCKED`；無新 LoopEvent contract；**Sprint 57.98 A1（CHANGE-065）新增 terminal `stop_reason="verification_failed"`**：in-loop Cat 10 gate 在 self-correction budget（`max_correction_attempts`，default 2）耗盡時終止（raw string，無新 `TerminationReason` enum，無 consumer switch）；verifier registry 由 `build_handler` 注入 `AgentLoopImpl` ctor（前為 `run_with_verification` wrapper 包覆 `loop.run()`，已退役刪除）；`resume()` 經共用 `_run_turns` 自動驗證 resumed 答案（關閉 pre-57.98 漏洞）；durable `verification_attempts` 騎 checkpoint `metadata`（無新 DurableState 欄位）；`VerificationPassed/Failed` contract 不變；**Sprint 57.99 A2（CHANGE-066）同一 `verification_failed` terminal 加條件性人類 ESCALATE 替代**：`chat_verification_escalate_on_max` toggle（default OFF = A1 byte-identical）ON 且 full deferred-HITL wiring 在場時，FAIL==max swap-point 改 `_cat10_verification_escalate_pause()` 觸發 `awaiting_approval` **第五 origin**（`pending_approval.kind="verification"`，carries held failed-answer `response_snapshot` + durable `verification_escalated` flag 騎 `metadata`，無 migration）而非直接 terminal；`resume()` 新增 `kind="verification"`：APPROVE → `_replay_approved_output` 交付 held 答案（人類覆蓋 verifier，TERMINAL，不 re-verify）/ REJECT-with-note → 註記回注成 `user` correction + 恰一個 human-coached turn（`verification_attempts`=max + durable `verification_escalated` → 2nd fail 綁回 A1 terminal，無第二次 pause）；無新 LoopEvent / event-schema（reuse `ApprovalRequested`/`ApprovalReceived`） |
