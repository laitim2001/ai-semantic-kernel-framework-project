...
- **`AD-Resume-Continuation-Fidelity`** (NEW) — `_resume_continuation` (`loop.py:1992`) is a SECOND, reduced copy of run()'s loop body: real while-true + PromptBuilder + max_turns/token_budget, BUT deliberately omits run()'s Cat 8 retry / Cat 9 guardrail+tripwire / Cat 4 compaction / per-turn checkpoint+span. OK for the spike happy-path (approved tool → LLM 收尾, no second escalating tool). Production needs run()'s core refactored into a shared re-enterable loop (or resume re-arms full machinery). Also: the continuation cannot itself pause again (one-approval-per-run this slice).
