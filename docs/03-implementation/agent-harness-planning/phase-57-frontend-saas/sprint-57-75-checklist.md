...
- [ ] **Extend SpanStarted/SpanEnded** in `_contracts/events.py`
...
- [ ] **Emit SpanStarted/Ended at 6 loop sites** (`loop.py` 797/877/912/960/1021/1289)
...
- [ ] **Add 2 serialize branches** (SpanStarted/SpanEnded) in `sse.py` `_serialize_inner`
  - Verify nested `{type, data:{...}}` envelope against an existing branch (57.67 flat-vs-nested lesson)
- [ ] **Add 2 WIRE_SCHEMA entries** (`event_wire_schema.py`, 18→20) + regenerate codegen
...
- [ ] **Parent re-verify (Before-Commit item 7)** — read loop emit + serialize; re-run mypy + pytest (subset) + `run_all.py`
...
- [ ] **Extend MemoryAccessed** in `_contracts/events.py`
  - +`summary: str = ""` +`time_scale: str = ""` (keep layer/operation/key)
- [ ] **Enrich build() metadata** in `prompt_builder/builder.py`
...
- [ ] **Emit MemoryAccessed per-hint** in `loop.py:~966` (post-build, inside `prompt_builder is not None` branch, next to PromptBuilt)
...
- [ ] **Add 1 serialize branch** (MemoryAccessed) in `sse.py` + **1 WIRE_SCHEMA entry** (20→21) + regen codegen
  - Command: `python scripts/codegen/generate_event_schemas.py`

### 2.3 Backend Memory tests + parity (US-6)
- [ ] **MemoryAccessed emit test** (real_llm path emits ≥1; echo_demo emits 0)
- [ ] **Parity test** `test_event_wire_schema_parity.py` for 3 new types (18→21)
  - Command: `pytest backend/tests/unit/api/v1/chat/test_event_wire_schema_parity.py`
- [ ] **Parent re-verify** — mypy 0; pytest green; `run_all.py` 10/10 (incl. check_event_schema_sync + check_llm_sdk_leak)
...
- [ ] **chatStore.ts** — 2 derived slices `spans: SpanNode[]` + `memoryOps: MemoryOp[]` in ChatStoreState + `_initial()`
- [ ] **3 mergeEvent cases** — span_started (open) / span_ended (close+duration) / memory_accessed (push op); mirror `subagents` slice
  - DoD: exhaustive switch stays complete; rawEvents still pushed

### 3.2 InspectorTrace (US-1)
- [ ] **InspectorTrace.tsx** (NEW) — consume `s.spans`; waterfall by parent_span_id (indent + tree-glyph); colored duration bar by span_type (var(--*) tokens); right `{d}s`; empty-state
  - Mockup L434-466 verbatim CSS classes; no shadcn residue; no new oklch/hex literal

### 3.3 InspectorMemory (US-2)
- [ ] **InspectorMemory.tsx** (NEW) — consume `s.memoryOps`; ops list (`<Badge tone="memory">{op}</Badge>` + scope + `.subtle` timestamp + `{key} = {summary}`); empty-state
  - Mockup L468-487 verbatim CSS classes

### 3.4 ChatInspector swap + Vitest (US-1/US-2)
- [ ] **ChatInspector.tsx** — swap 2 `ComingSoonInspectorTab` (:98-113) → `<InspectorTrace/>` / `<InspectorMemory/>` (Turn/Tree untouched)
- [ ] **Vitest** — `{InspectorTrace,InspectorMemory}.test.tsx` (NEW: waterfall nesting / ops render / empty-state) + chatStore.test (new slices)
...
- [ ] **e2e** — `*chat*inspector*.spec.ts` (NEW or extend): SSE mock with span/memory events → Trace waterfall + Memory ops render; echo_demo → Memory empty
- [ ] **Mockup-fidelity DoD**
...
- [ ] **Full sweep (parent re-verify, Before-Commit item 7)**
...
### 4.2 Closeout docs
- [ ] **CHANGE-043** in `claudedocs/4-changes/feature-changes/`
- [ ] **progress.md** Day 0-4 + **retrospective.md** Q1-Q7
- [ ] **Checklist** all `[x]` (no deletion of unchecked)
- [ ] **Calibration** record (mixed-multidomain-bundle 0.65 + agent_factor 0.45; CAVEAT 12+ consecutive)
- [ ] **AD closure**: `AD-ChatV2-Inspector-Trace-Phase2` + `-Memory-Phase2` → CLOSED; new carryovers (subagent-boundary spans / memory write-evict emit) → next-phase-candidates.md
- [ ] **MEMORY subfile + pointer** + **CLAUDE.md lean** (Current Sprint + Last Updated only)
- [ ] **No design note** (feature-continuation: extends A-5 chain 57.66/67/72 + existing event types)

### 4.3 Ship
- [ ] **Commit mapping** Day-0 / Track A backend / Track B frontend / closeout
