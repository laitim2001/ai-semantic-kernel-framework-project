...
- [x] **Prong 1 (path)** — confirm: `infrastructure/db/models/memory.py`, `migrations/versions/` (0023 head), `agent_harness/memory/layers/{user,tenant,role,system,session}_layer.py`, `api/v1/chat/memory.py` (matrix endpoint), `infrastructure/db/models/base.py` (TenantScopedMixin), `infrastructure/db/models/audit.py`.
- [x] **Prong 2 (content)** — D-DAY0-1..7 in plan §0 (no persistence confirmed; layer signatures; Risk-C session; matrix deps trio).
- [x] **Prong 3 (schema, PRIMARY)** —
...
- [x] **go/no-go** — schema confirmed; no >20% drift

### 0.2 Branch + decisions
- [x] **Branch created** `feature/sprint-57-76-memory-ops-history`
- [x] **Decisions locked**: Memory ops-history Option B (dedicated memory_ops table); backend-only this sprint (FE → 57.77); emit user/tenant/role write/evict (system raises, session in-memory skipped); no hash-chain; agent-delegated Track A backend + parent re-verify.
...
### 1.1 MemoryOp ORM
- [x] **`MemoryOp(Base, TenantScopedMixin)`** in `models/memory.py`
...
- [x] **`0024_memory_ops.py`** migration
...
- [x] **Day-0/1 verify** `check_rls_policies.py` green for new table
...
- [x] **`_record_memory_op(session, *, tenant_id, user_id, scope, key, operation, time_scale, value_snapshot, actor)`** (Cat 3, in `memory/`)
  - `session.add(MemoryOp(...))` — takes the LAYER's session (Risk C: same txn, no separate session)

### 2.2 Write emit (user/tenant/role)
- [x] **write() emit** in `{user,tenant,role}_layer.py`
...
- [x] **evict() emit** in `{user,tenant,role}_layer.py`
...
### 3.1 GET /memory/ops
- [x] **`GET /memory/ops?limit=&before=`** in `api/v1/chat/memory.py`
...
### 3.2 Tests
- [x] **emit tests** (`test_ops_emit.py`) — write→WRITE row; evict→EVICT row (old value); system write raises→0 rows; Risk-C rollback (layer write rollback → op row rolled back)
- [x] **RLS test** (`test_memory_ops_rls.py`) — tenant A ops invisible to B (SET app.tenant_id; mirror test_rls_policy_enforced)
- [x] **endpoint test** (`test_memory_ops_endpoint.py`) — tenant-scoped time-ordered; pagination cursor; require_audit_role 403; cross-tenant empty
- [x] **Parent re-verify (Before-Commit item 7)** — read emit + endpoint; mypy + pytest (subset) + run_all
...
- [x] **Backend gates** — `mypy src/` 0 + `pytest` (new + regression) + `scripts/lint/run_all.py` 10/10 (esp. check_rls_policies + check_llm_sdk_leak) + Alembic up/down round-trip
- [x] **Read all agent-changed code** — emit in same txn (no double-commit), SELECT-before-DELETE, RLS mirror correct, no fabrication, no LLM import

### 4.2 Closeout docs
- [x] **CHANGE-044** in `claudedocs/4-changes/feature-changes/`
- [x] **progress.md** Day 0-4 + **retrospective.md** Q1-Q7
- [x] **Checklist** all `[x]` (no deletion of unchecked)
- [x] **Calibration** record (medium-backend 0.80 + agent_factor 0.45; CAVEAT 14th consecutive)
- [x] **AD status**: `AD-Memory-OpsHistory-Backend` backend half done → frontend half = Sprint 57.77 carryover; next-phase-candidates.md
- [x] **MEMORY subfile + pointer** + **CLAUDE.md lean**
- [x] **Design note?** — likely NO (feature-continuation: new table reuses 0023 RLS + TenantScopedMixin + matrix-endpoint pattern, like 57.70 agent_catalog); confirm at Day 4 retro

### 4.3 Ship
- [x] **Commit mapping** Day-0 / schema+migration / emit / endpoint+tests / closeout
